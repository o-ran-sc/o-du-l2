...
void addDlHqProcInUe(SlotTimingInfo dlMsgTime, MacUeCb *ueCb, DlMsgSchInfo schedInfo)
{
   ...
   DU_LOG("\nHLAL ADDED HQ Proc Id [%d] [SFN:SLOT] [%d:%d] NumTb [%d] TbSz1 [%d] TbSz2 [%d] UECB [%p]", \
      hqProcId, dlMsgTime.sfn, dlMsgTime.slot, hqProcCb->numTb, hqProcCb->tbInfo[0].tbSize, hqProcCb->tbInfo[1].tbSize, ueCb);
   return;
}
...
uint8_t updateNewTbInDlHqProcCb(SlotTimingInfo slotInfo, MacUeCb *ueCb, uint32_t tbSize, uint8_t *txPdu)
{
   ...
   DU_LOG("\nHLAL Inside updateNewTbInDlHqProcCb ueCb [%p]", ueCb);

   /* Search HARQ Proc Cb in DL HARQ Ent */
   for(hqProcIdx = 0; hqProcIdx < MAX_NUM_HARQ_PROC; hqProcIdx++)
   {
      DU_LOG("\nHLAL hqProcIdx [%d] numTb [%d]", hqProcIdx, dlHqEnt->harqProcCb[hqProcIdx].numTb);
      ...
      for(tbIdx =0; tbIdx < dlHqEnt->harqProcCb[hqProcIdx].numTb; tbIdx++)
      {
          ...
          */
         DU_LOG("\nHLAL [SFN:SLOT] [%d:%d] tbSize [%d]", \
         dlHqEnt->harqProcCb[hqProcIdx].tbInfo[tbIdx].txTime.sfn, \
         dlHqEnt->harqProcCb[hqProcIdx].tbInfo[tbIdx].txTime.slot, \
         dlHqEnt->harqProcCb[hqProcIdx].tbInfo[tbIdx].tbSize);
               ...
               (dlHqEnt->harqProcCb[hqProcIdx].tbInfo[tbIdx].tbSize == tbSize))
         {
            ...
            DU_LOG("\nHLAL UPDATED HQ Proc Id [%d] [SFN:SLOT] [%d:%d] NumTb [%d] TbSz1 [%d] Tb1 [%x] TbSz2 [%d] Tb2 [%x]", \
                  dlHqEnt->harqProcCb[hqProcIdx].procId, dlHqEnt->harqProcCb[hqProcIdx].tbInfo[tbIdx].txTime.sfn, \
                  dlHqEnt->harqProcCb[hqProcIdx].tbInfo[tbIdx].txTime.slot, dlHqEnt->harqProcCb[hqProcIdx].numTb, \
                  dlHqEnt->harqProcCb[hqProcIdx].tbInfo[0].tbSize, dlHqEnt->harqProcCb[hqProcIdx].tbInfo[0].tb, \
                  dlHqEnt->harqProcCb[hqProcIdx].tbInfo[1].tbSize, dlHqEnt->harqProcCb[hqProcIdx].tbInfo[1].tb);
            break;
         }
      }
...
uint8_t* fetchTbfromDlHarqProc(SlotTimingInfo slotInfo, MacUeCb *ueCb, uint8_t hqProcId, uint32_t tbSize)
{
   ...
   {
      /* Search TB Info in a HARQ Proc Cb */
      for(tbIdx =0; tbIdx < hqProcCb->numTb; dlHqEnt++)
      {
         if(hqProcCb->tbInfo[tbIdx].tbSize == tbSize)
         {
            ...
            DU_LOG("\nHLAL FETCHED HQ Proc Id [%d] [SFN:SLOT] [%d:%d] TbSz [%d] Tb [%x]", \
                  hqProcId, slotInfo.sfn, slotInfo.slot, hqProcCb->tbInfo[tbIdx].tbSize, hqProcCb->tbInfo[tbIdx].tb);
...
uint8_t MacProcSchRlsHqProc(Pst *pst, SchRlsHqInfo *hqInfo)
{
   ...
   for(hqInfoIdx = 0; hqInfoIdx < hqInfo->numUes; hqInfoIdx++)
   {
      ...
      if(ueCb->raCb)
      {
         if(ueCb->raCb->msg4HqInfo.procId == hqProcId)
         {
      ...
      else
      {
         ...
         if(hqProcCb->procId == hqProcId)
         {
            ...
            DU_LOG("\nHLAL RELEASED HQ Proc Id [%d] NumTb [%d] TbSz1 [%d] Tb1 [%x] TbSz2 [%d] Tb2 [%x]", \
                  hqProcId, hqProcCb->numTb, hqProcCb->tbInfo[0].tbSize, hqProcCb->tbInfo[0].tb, \
