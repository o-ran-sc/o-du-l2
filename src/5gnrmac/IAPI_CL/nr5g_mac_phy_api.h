...
#ifndef INTEL_L1_V23_03
  uint8_t nPrimeRUPortIndex;           /* the Port index in L1 XML file */
  uint8_t nSecondRUPortIndex;          /* the Port index in L1 XML file, is used only for nRUPortNum is 2.*/
  uint8_t nRsv;                        
...
typedef struct tDlBchPdu
{
    ...
    int16_t    nBeamId[4];
#else
    /*max support 2 ssb with ssb beamid,nBeamId is the beamid of the first ssb in the slot ,nBeamId1 is the beamid of the second ssb in the slot*/
    int16_t    nBeamId;
    int16_t    nBeamId1;
#endif
#endif
...
#ifdef INTEL_L1_V22_11
    uint8_t  nConfLvl;      // the level of confidence for Timing advance measure. value is 0-2. 0 is low, 1 is high. 
...
#elif INTEL_L1_V22_03
	/**** word 5 *****/
	int16_t  nSigPwr[MAX_UL_REAL_PORTS];  // 8 received signal power from antennas in 1/256 dBm steps 
#endif
...
#elif INTEL_L1_V22_03
	/**** word 3*****/  
	int16_t    nIntfNoisePwr[(int)(MAX_5GNR_PRB/4+1)]; // Subband granularity for RBG size of 4
#endif
#ifdef INTEL_L1_V22_03
    uint8_t    rsv2[2];	
#endif
