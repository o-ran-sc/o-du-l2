...
S16 rgAddToL2MeasPerQci ARGS ((RgCellCb  *cell,U8 qci));
#endif
 ...
 * APIs exposed by LMM
 */
S16 rgLMMStartTmr ARGS((Inst inst,S16 tmrEvnt, U32 tmrVal, PTR cb));
...
/* APIs Exposed by UIM */
S16 rgUIMRguBndCfm ARGS ((Inst inst,SuId suId, U8 status));
...
S16 rgUIMCrgBndCfm ARGS ((Inst inst,SuId suId,U8 status));
S16 rgUIMCrgCfgCfm ARGS ((Inst inst,CrgCfgTransId transId,U8 status));
...
S16 rgDBMInitInactvCellLst ARGS((U16 numBins));
...
S16 rgDBMInitCellLst ARGS(( U16 numBins));
...
S16 rgDHMHqEntInit ARGS((Inst inst,RgDlHqEnt *hqE, U8 maxHqProcs));
...
S16 rgDHMRlsHqProcTB ARGS((RgCellCb *cell, RgDlHqProcCb *hqP,U8 tbIndex));
S16 rgDHMGetHqProcFrmId ARGS((RgUeCb *ue, U8 idx, RgDlHqProcCb **hqP));
/* Changes for MIMO feature addition */
/* Removed dependency on MIMO compile-time flag */
...
Void rgFillDgnParams ARGS((Inst inst,RgUstaDgn *dgn,U8 dgnType));
Void rgUpdtRguDedSts ARGS((Inst inst,RgUpSapCb *rguSap,U8 stsType, RgRguDedDatReq *datReq));
Void rgUpdtRguCmnSts ARGS((Inst inst,RgUpSapCb *rguSap,U8 stsType));
Void rgUpdtCellCnt   ARGS((Inst inst,U8 updtType));
Void rgUpdtUeCnt     ARGS((Inst inst,U8 updtType));
