...
void setNumCs(uint8_t *numCs, MacUlSlot *currUlSlot)
{
   uint8_t idx;
   if(currUlSlot != NULLP)
	{
	   for(idx = 0; idx < MAX_ZERO_CORR_CFG_IDX; idx++)
		{
...
void fillPrachPdu(fapi_ul_tti_req_pdu_t *ulTtiReqPdu, MacCellCfg *macCellCfg, MacUlSlot *currUlSlot, uint32_t *msgLen)
{
   if(ulTtiReqPdu != NULLP)
   {
      ...
      setNumCs(&ulTtiReqPdu->u.prach_pdu.numCs, currUlSlot); 
...
#else
         MAC_ALLOC(ulTtiReq, sizeof(fapi_dl_tti_req_t));
#endif
         if(ulTtiReq != NULLP)
         {
            ...
            if(ulTtiReq->nPdus > 0)
            {
               ...
               if(ulTtiReq->rachPresent)
               {
                 ...
                 LwrMacSendToPhy(ulTtiReq->header.message_type_id, msgLen, (void *)ulTtiReq);
					  MAC_FREE(ulTtiReqPdu, (ulTtiReq->nPdus * \
