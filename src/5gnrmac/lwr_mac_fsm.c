 ...
 *
 * ****************************************************************/
S16 lwr_mac_procInvalidEvt(void *msg)
{
...
S16 lwr_mac_procParamReqEvt(void *msg)
{
#ifdef INTEL_FAPI
   /* startGuardTimer(); */
...
S16 lwr_mac_procParamRspEvt(void *msg)
{
#ifdef INTEL_FAPI
   /* stopGuardTimer(); */
   uint8_t index;
...
S16 lwr_mac_procConfigReqEvt(void *msg)
{
#ifdef INTEL_FAPI
   uint8_t idx = 0;
   uint8_t index = 0;
   ...
   macCfgParams = macCb.macCell->macCellCfg;
...
S16 lwr_mac_procConfigRspEvt(void *msg)
{
#ifdef INTEL_FAPI
 ...
 *
 * ****************************************************************/
S16 lwr_mac_procStartReqEvt(void *msg)
{
#ifdef INTEL_FAPI
   uint32_t msgLen = 0;
...
S16 lwr_mac_procStopReqEvt(void *msg)
{
#ifdef INTEL_FAPI
   uint32_t msgLen = 0;
 ...
 *
 * ****************************************************************/
uint16_t procDlTtiReq(SlotIndInfo currTimingInfo)
{
#ifdef INTEL_FAPI
   uint8_t idx;
   uint8_t nPdu = 0;
   ...
   if(clGlobalCp.phyState == PHY_STATE_RUNNING)
   {
      ...
      macCellCfg = macCb.macCell->macCellCfg;
...
uint16_t sendTxDataReq(SlotIndInfo currTimingInfo, DlSchedInfo *dlInfo)
{
   ...
   if(nPdu > 0)
   {
      ...
      if(dlInfo->brdcstAlloc.sib1Trans)
      {
	 txDataReqMsgSize += macCb.macCell->macCellCfg.sib1Cfg.sib1PduLen;
      }
      if(dlInfo->rarAlloc != NULLP)
      {
	 ...
	 fillSib1TxDataReq(txDataReq->pdu_desc,
	       &macCb.macCell->macCellCfg, pduIndex);
	 pduIndex++;
	 txDataReq->num_pdus++;
      }
      if(dlInfo->rarAlloc != NULLP)
 ...
 *
 ******************************************************************/
uint16_t procUlTtiReq(SlotIndInfo currTimingInfo)
{
#ifdef INTEL_FAPI
   uint8_t    pduIdx = -1;
   uint32_t   msgLen = 0;
