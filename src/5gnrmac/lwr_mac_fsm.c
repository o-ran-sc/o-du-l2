...
S16 lwr_mac_handleConfigReqEvt(void *msg)
{
   ...
   if(configReq != NULL)
   {
      ...
      LwrMacSendToPhy(configReq->header.msg_id, configReqSize, (void *)configReq);
   }
   else
   {
 ...
 *
 ******************************************************************/
S16 fillPdcchPdu(fapi_dl_tti_req_pdu_t *dlTtiReqPdu, DlSchedInfo *dlInfo, uint32_t *msgLen, RntiType rntiType, uint8_t
coreSetType)
{
   if(dlTtiReqPdu != NULLP)
   {
      ...
      BwpCfg *bwp = NULLP;
#if 0
      dlTtiReqPdu->pdu.pdcch_pdu.dlDci = (fapi_dl_dci_t *)(dlTtiReqPdu + \
         (sizeof(fapi_dl_tti_req_pdu_t) - sizeof(dlTtiReqPdu->pdu)) + \
			...
			if(dlTtiReq->nPdus > 0)
			{
				...
				if(currDlSlot->dlInfo.rarAlloc != NULLP)
				{
					...
					fillPdcchPdu(&dlTtiReq->pdus[numPduEncoded], \
							&currDlSlot->dlInfo, &msgLen, rntiType, CORESET_TYPE0);
					numPduEncoded++;
					fillPdschPdu(&dlTtiReq->pdus[numPduEncoded],
...
uint16_t sendTxDataReq(SlotIndInfo currTimingInfo, DlSchedInfo *dlInfo)
{
   ...
   if(nPdu > 0)
   {
      ...
      if(dlInfo->rarAlloc != NULLP)
      {
         fillRarTxDataReq(txDataReq->pdu_desc, &dlInfo->rarAlloc->rarInfo, &msgLen, pduIndex);
         pduIndex++;
         txDataReq->num_pdus++;
      ...
      LwrMacSendToPhy(txDataReq->header.msg_id, sizeof(fapi_tx_data_req_t), \
         (void *)txDataReq);
   }
#endif
   return ROK;
