 ...
 *
 ******************************************************************/
void fillBsrUlDciPdu(fapi_dl_dci_t *ulDciPtr, DciInfo *schDciInfo)
{
   if(ulDciPtr != NULLP)
   {
      uint8_t numBytes;
      ...
      timeDomResAssign = schDciInfo->format.format0_0.timeAlloc.rowIndex;
...
uint8_t fillUlDciPdcchPdu(fapi_dci_pdu_t *ulDciReqPdu, DlSchedInfo *dlInfo, \
   RntiType rntiType, uint8_t coreSetType)
{
   if(ulDciReqPdu != NULLP && dlInfo->ulGrant != NULLP)
   {
      memset(&ulDciReqPdu->pdcchPduConfig, 0, sizeof(fapi_dl_pdcch_pdu_t));
      if(rntiType == C_RNTI_TYPE)
      {
         fillBsrUlDciPdu(ulDciReqPdu->pdcchPduConfig.dlDci, dlInfo->ulGrant);
      }
      else
      {
