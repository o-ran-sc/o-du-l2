...
uint8_t lwr_mac_procConfigReqEvt(void *msg)
{
   ...
   mibPdu[0] = macCfgParams.ssbCfg.mibPdu[0];
   mibPdu[0] = mibPdu[0] << 24;
   mibPdu[1] = macCfgParams.ssbCfg.mibPdu[1];
   mibPdu[1] = mibPdu[1] << 16;
   mibPdu[2] = macCfgParams.ssbCfg.mibPdu[2];
   mibPdu[2] = mibPdu[2] << 8;
   mib = mibPdu[0] | mibPdu[1] | mibPdu[2];
   ...
   FILL_FAPI_LIST_ELEM(headerElem, cfgReqQElem, FAPI_MSG_HEADER_IND, 1, \
...
uint8_t lwr_mac_procStartReqEvt(void *msg)
{
   ...
   FILL_FAPI_LIST_ELEM(headerElem, startReqElem, FAPI_MSG_HEADER_IND, 1, \
      sizeof(fapi_msg_header_t));
   msgHeader = (fapi_msg_header_t *)(headerElem + 1);
