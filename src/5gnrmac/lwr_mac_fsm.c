...
S16 lwr_mac_handleConfigReqEvt(void *msg)
{
   ...
   if(configReq != NULL)
   {
      ...
      fillTlvs(&configReq->tlvs[index++], FAPI_NUM_UNUSED_ROOT_SEQUENCES_TAG,  sizeof(uint8_t), macCfgParams.prachCfg.fdm[0].numUnusedRootSeq, &msgLen);
	   //LWR_MAC_ALLOC(macCfgParams.prachCfg.fdm[0].unsuedRootSeq , sizeof(uint8_t)*macCfgParams.prachCfg.fdm[0].numUnusedRootSeq);
...
#ifndef INTEL_WLS
      free(macCfgParams.prachCfg.fdm[0].unsuedRootSeq);
