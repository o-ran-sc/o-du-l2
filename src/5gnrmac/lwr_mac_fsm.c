...
EXTERN void sendToPhy ARGS((uint16_t msgType, uint32_t msgLen, void *msg));
...
U16 getParamValue(fapi_uint16_tlv_t *tlv, uint16_t type)
{
    uint16_t valueLen;
    void *temp, *posPtr;
    ...
    {
       temp = (uint16_t *)malloc(valueLen * sizeof(U16));
       memcpy(temp, posPtr, valueLen);
       return(*(uint16_t *)temp);
    }
    else if(type == FAPI_UINT_32)
    {
       temp = (uint32_t *)malloc(valueLen * sizeof(U32));
       memcpy(temp, posPtr, valueLen);
...
S16 handleDlTtiReq(CmLteTimingInfo *dlTtiReqtimingInfo)
{
   ...
   if(dlTtiReqtimingInfo != NULLP)
   {
      ...
      if(dlTtiReq != NULLP)
      {
         ...
         dlTtiReq->nPdus = 0;
         dlTtiReq->nGroup = 0;
         if(macGlobalCb.macCell->dlSlot[0].cellBroadcastInfo.ssbTrans)
         {
