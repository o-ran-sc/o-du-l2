...
Msg4Info *msg4Info)
{
   if(dlDciPtr != NULLP)
   {
      ...
      /* TODO: Fill values of coreset0Size, rbStart and rbLen */
      coreset0Size = msg4PdcchInfo->coresetCfg.coreSetSize;
...
BsrInfo *bsrInfo)
{
   if(ulDciPtr != NULLP)
   {
      ...
      coreset1Size = bsrPdcchInfo->coresetCfg.coreSetSize;
   ...
   RntiType rntiType, uint8_t coreSetType)
{
   if(ulDciReqPdu != NULLP)
   {
	 ...
	 return RFAILED;
      ...
      ulDciReqPdu->pdcchPduConfig.startSymbolIndex  = pdcchInfo->coresetCfg.startSymbolIndex;
      ulDciReqPdu->pdcchPduConfig.durationSymbols   = pdcchInfo->coresetCfg.durationSymbols;
      memcpy(ulDciReqPdu->pdcchPduConfig.freqDomainResource, pdcchInfo->coresetCfg.freqDomainResource, 6);
      ulDciReqPdu->pdcchPduConfig.cceRegMappingType = pdcchInfo->coresetCfg.cceRegMappingType;
      ulDciReqPdu->pdcchPduConfig.regBundleSize     = pdcchInfo->coresetCfg.regBundleSize;
      ulDciReqPdu->pdcchPduConfig.interleaverSize   = pdcchInfo->coresetCfg.interleaverSize;
      ulDciReqPdu->pdcchPduConfig.coreSetSize       = pdcchInfo->coresetCfg.coreSetType;
      ulDciReqPdu->pdcchPduConfig.shiftIndex        = pdcchInfo->coresetCfg.shiftIndex;
      ulDciReqPdu->pdcchPduConfig.precoderGranularity = pdcchInfo->coresetCfg.precoderGranularity;
