...
uint32_t *msgLen, uint8_t ssbIdxCount) 
{
   ...
   if(dlTtiReqPdu != NULL)
   {
      ...
      dlTtiReqPdu->u.ssb_pdu.ssbBlockIndex = macCb.macCell->dlSlot[0].cellBroadcastInfo.ssbInfo[ssbIdxCount].ssbIdx;
...
uint32_t *msgLen)
{
    if(dlTtiReqPdu != NULLP)
    {
       ...
       dlTtiReqPdu->u.pdcch_pdu.subCarrierSpacing = sib1PdcchInfo->sib1CmCfg.subcarrierSpacing; 
       dlTtiReqPdu->u.pdcch_pdu.cyclicPrefix = sib1PdcchInfo->sib1CmCfg.cyclicPrefix; 
...
uint32_t *msgLen)
{
    ...
    if(dlTtiReqPdu != NULLP)
    {
       ...
       dlTtiReqPdu->u.pdsch_pdu.subCarrierSpacing = sib1PdschInfo->sib1CmCfg.subcarrierSpacing;
       dlTtiReqPdu->u.pdsch_pdu.cyclicPrefix = sib1PdschInfo->sib1CmCfg.cyclicPrefix;
...
uint8_t calculatePduCount(DlBrdcstAlloc *cellBroadcastInfo)
{
   ...
   {
      count = 0;
      for(idx =0; idx < macCb.macCell->dlSlot[0].cellBroadcastInfo.ssbIdxSupported; idx++)
      {
         count++;
      }
   }
   ...
   if(clGlobalCp.phyState == PHY_STATE_RUNNING)
   {
      ...
      if(dlTtiReqtimingInfo != NULLP)
      {
         ...
         if(dlTtiReq != NULLP)
         {
            ...
            dlTtiReq->nPdus = calculatePduCount(&(macCb.macCell->dlSlot[0].cellBroadcastInfo));  /* get total Pdus */
            dlTtiReq->nGroup = 0;
            if(dlTtiReq->nPdus > 0)
            {
               ...
               if(macCb.macCell->dlSlot[0].cellBroadcastInfo.ssbTrans)
               {
                 if(dlTtiReqPdu != NULLP)
                 {
                    for(idx =0; idx < macCb.macCell->dlSlot[0].cellBroadcastInfo.ssbIdxSupported; idx++)
                    {
                       fillSsbPdu(dlTtiReqPdu, &macCellCfg, &msgLen, idx);
                       dlTtiReq->pdus = dlTtiReqPdu;
                       dlTtiReq->pdus++;
                    }
                 }
               }
               else if(macCb.macCell->dlSlot[0].cellBroadcastInfo.sib1Trans)
               {
                  /* Filling SIB1 param */
                  if(dlTtiReqPdu != NULLP)
                  {
                     fillPdcchPdu(dlTtiReqPdu, &macCb.macCell->dlSlot[0].cellBroadcastInfo.sib1Alloc.sib1PdcchCfg, &msgLen);
                     dlTtiReq->pdus = dlTtiReqPdu;
                     dlTtiReq->pdus++;
                     fillPdschPdu(dlTtiReqPdu, &macCb.macCell->dlSlot[0].cellBroadcastInfo.sib1Alloc.sib1PdschCfg, &msgLen);
                     dlTtiReq->pdus = dlTtiReqPdu;
                  }
               }
               ...
               MAC_FREE(dlTtiReq->pdus->u.pdcch_pdu.dlDci, sizeof(fapi_dl_pdcch_pdu_t));
