...
uint8_t lwr_mac_procConfigReqEvt(void *msg)
{
   ...
   memset(&lwrMacCb.cellCb[lwrMacCb.numOfCells], 0, sizeof(LwrMacCellCb));
   lwrMacCb.cellCb[lwrMacCb.numOfCells].cellId = macCfgParams.cellId;
   lwrMacCb.cellCb[lwrMacCb.numOfCells].phyCellId = macCfgParams.phyCellId; 
   lwrMacCb.numOfCells++;
...
uint8_t lwr_mac_procConfigRspEvt(void *msg)
{
   ...
   if(configRsp != NULL)
   {
	 ...
	 lwrMacCb.cellCb[lwrMacCb.numOfCells - 1].state = PHY_STATE_CONFIGURED;
	 /* TODO : 
	  * Store config response into an intermediate struture and send to MAC
	  * Support LC and LWLC for sending config rsp to MAC 
	  */
	 fapiMacConfigRsp(lwrMacCb.cellCb[lwrMacCb.numOfCells - 1].cellId);
      }
      else
      {
