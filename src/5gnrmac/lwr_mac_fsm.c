...
uint8_t lwr_mac_procStopReqEvt(void *msg)
{
   ...
   vendorMsg->stop_req_vendor.sfn = 0;
   vendorMsg->stop_req_vendor.slot = 0;
   ...
   FILL_FAPI_LIST_ELEM(headerElem, stopReqElem, FAPI_MSG_HEADER_IND, 1, \
   ...
   /* TODO : uncomment when L1-L2 Integration Fixes are submitted */
...
uint16_t fillDlTtiReq(SlotIndInfo currTimingInfo)
{
   ...
   if(lwrMacCb.phyState == PHY_STATE_RUNNING)
   {
	 ...
	 FILL_FAPI_LIST_ELEM(headerElem, dlTtiElem, FAPI_MSG_HEADER_IND, 1, \
	       sizeof(fapi_msg_header_t));
	 msgHeader = (fapi_msg_header_t *)(headerElem + 1);
	 msgHeader->num_msg = 1;
	 msgHeader->handle = 0;
...
uint16_t sendTxDataReq(SlotIndInfo currTimingInfo, DlSchedInfo *dlInfo)
{
   ...
   if(nPdu > 0)
   {
	 ...
	 return RFAILED;
      ...
      FILL_FAPI_LIST_ELEM(headerElem, txDataElem, FAPI_MSG_HEADER_IND, 1, \
	    sizeof(fapi_msg_header_t));
      msgHeader = (fapi_msg_header_t *)(headerElem + 1);
      msgHeader->num_msg = 1;
      msgHeader->handle = 0;
...
uint16_t fillUlTtiReq(SlotIndInfo currTimingInfo)
{
   ...
   if(lwrMacCb.phyState == PHY_STATE_RUNNING)
   {
	 ...
	 FILL_FAPI_LIST_ELEM(headerElem, ulTtiElem, FAPI_MSG_HEADER_IND, 1, \
	       sizeof(fapi_msg_header_t));
	 msgHeader = (fapi_msg_header_t *)(headerElem + 1);
	 msgHeader->num_msg = 1;
	 msgHeader->handle = 0;
         ...
         if(ulDciElem)
	 {
	    ...
	    fillMsgHeader(&ulDciReq->header, FAPI_UL_DCI_REQUEST, sizeof(fapi_ul_dci_req_t));
		  ...
		  return RFAILED;
	       ...
	       FILL_FAPI_LIST_ELEM(headerElem, ulDciElem, FAPI_MSG_HEADER_IND, 1, \
	          sizeof(fapi_msg_header_t));
	       msgHeader = (fapi_msg_header_t *)(headerElem + 1);
	       msgHeader->num_msg = 1;
	       msgHeader->handle = 0;
