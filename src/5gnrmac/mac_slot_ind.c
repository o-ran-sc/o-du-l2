...
extern uint16_t procUlTtiReq(SlotIndInfo currTimingInfo);
extern uint16_t procDlTtiReq(SlotIndInfo currTimingInfo);
...
int sendSlotIndMacToSch(SlotIndInfo *slotInd)
{
   ...
   FILL_PST_TO_SCH(pst, EVENT_SLOT_IND_TO_SCH);
...
int sendSlotIndMacToDuApp(SlotIndInfo *slotInd)
{
   ...
   /* Fill Pst */
   FILL_PST_TO_DUAPP(pst, EVENT_MAC_SLOT_IND);
...
uint8_t macProcSlotInd(SlotIndInfo slotInd)
{
   ...
   macCb.macCell->currTime.slot = slotInd.slot;
   macCb.macCell->currTime.sfn = slotInd.sfn;

   /* Trigger for DL TTI REQ */
   procDlTtiReq(slotInd);

   /* Trigger for UL TTI REQ */
   procUlTtiReq(slotInd);
