...
void fillMsg4Pdu(uint16_t cellId, DlMsgAlloc *msg4Alloc)
{
   ...
   {
      msg4Alloc->dlMsgInfo.dlMsgPduLen = macCb.macCell[cellIdx]->macRaCb[ueIdx -1].msg4TbSize;
      MAC_ALLOC(msg4Alloc->dlMsgInfo.dlMsgPdu, msg4Alloc->dlMsgInfo.dlMsgPduLen);
      if(msg4Alloc->dlMsgInfo.dlMsgPdu != NULLP)
      {
	 memcpy(msg4Alloc->dlMsgInfo.dlMsgPdu, macCb.macCell[cellIdx]->macRaCb[ueIdx -1].msg4TxPdu, \
