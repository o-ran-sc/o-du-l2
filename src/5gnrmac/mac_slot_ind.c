   ...
   if(dlSchedInfo != NULLP)
   {
      ...
      for(ueIdx=0; ueIdx<MAX_NUM_UE; ueIdx++)
      {
         ...
         if(dlSchedInfo->dlMsgAlloc[ueIdx] != NULLP)
         {
            ...
            if(dlSchedInfo->dlMsgAlloc[ueIdx]->dlMsgPdcchCfg->coresetCfg.coreSetType == CORESET_TYPE0)
            {
            ...
            else
            {
               ...
               else
               {
                   ...
                   */
                  for(cwIdx = 0; \
                        cwIdx < dlSchedInfo->dlMsgAlloc[ueIdx]->dlMsgPdcchCfg->dci.pdschCfg.numCodewords;\
                        cwIdx++)
                  {
                     /* Fetch TB to be retransmitted */
                     txPduLen = dlSchedInfo->dlMsgAlloc[ueIdx]->dlMsgPdcchCfg->dci.pdschCfg.codeword[cwIdx].tbSize;
...
uint8_t MacProcDlPageAlloc(Pst *pst, DlPageAlloc *dlPageAlloc)
{
   ...
   if(dlPageAlloc != NULLP)
   {
      ...
      memcpy(&currDlSlot->pageAllocInfo->pagePdcchCfg.dci.pdschCfg, &currDlSlot->pageAllocInfo->pagePdschCfg, sizeof(PdschCfg));
   }
   else
   {
...
void buildAndSendMuxPdu(SlotTimingInfo currTimingInfo)
{
   ...
   for(ueIdx=0; ueIdx<MAX_NUM_UE; ueIdx++)
   {
      ...
      {
         if(currDlSlot->dlInfo.dlMsgAlloc[ueIdx]->dlMsgPdcchCfg && currDlSlot->dlInfo.dlMsgAlloc[ueIdx]->dlMsgPdschCfg &&
               currDlSlot->dlInfo.dlMsgAlloc[ueIdx]->dlMsgPdcchCfg->coresetCfg.coreSetType == CORESET_TYPE0)
         {
