...
uint8_t MacProcCellCfgReq(Pst *pst, MacCellCfg *macCellCfg)
{
   ...
   * sizeof(Snssai*));
   ...
   {
      MAC_ALLOC(macCb.macCell[cellIdx]->macCellCfg.rrmPolicy, sizeof(MacRrmPolicy));
      if(!macCb.macCell[cellIdx]->macCellCfg.rrmPolicy)
      {
         DU_LOG("\nERROR  --> MAC: Memory allocation failed at MacProcCellCfgReq");
         return RFAILED;
      }
      memcpy(macCb.macCell[cellIdx]->macCellCfg.rrmPolicy, macCellCfg->rrmPolicy, sizeof(MacRrmPolicy));
   }
...
uint8_t MacSchCellCfgReq(Pst *pst, MacCellCfg *macCellCfg)
{
   ...
   if(macCellCfg->snssai) 
   {
      ...
      MAC_ALLOC(schCellCfg.snssai, schCellCfg.numSliceSupport * sizeof(SchSnssai*));
      ...
      for(sliceIdx=0; sliceIdx<schCellCfg.numSliceSupport; sliceIdx++)
      {
         if(macCellCfg->snssai[sliceIdx])
         {
            MAC_ALLOC(schCellCfg.snssai[sliceIdx], sizeof(SchSnssai));
            if(!schCellCfg.snssai[sliceIdx])
            {
               DU_LOG("\nERROR  --> MAC: Memory allocation failed at MacSchCellCfgReq");
               return RFAILED;
            }
            memcpy(schCellCfg.snssai[sliceIdx], macCellCfg->snssai[sliceIdx],  sizeof(SchSnssai));
         }
      }
   }
   if(macCellCfg->rrmPolicy)
      ...
      if(schCellDelRsp->rsp == RSP_OK)
      {
         ...
         if(macCb.macCell[cellIdx])
         {
            if(macCb.macCell[cellIdx]->cellId == schCellDelRsp->cellId)
            {
               ...
               MAC_FREE(macCb.macCell[cellIdx]->macCellCfg.rrmPolicy, sizeof(MacRrmPolicy));
