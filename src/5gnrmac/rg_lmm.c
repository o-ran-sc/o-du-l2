   ...
   {   
      case STRGUSAP:
         if ((cfg->s.rguSap.spId > LRG_MAX_RGU_SAPS) &&
             (cfg->s.rguSap.selector != SELECTOR_TC) &&
             (cfg->s.rguSap.selector != SELECTOR_LC))
         {
         ...
         break;
      case STCRGSAP:
         if ((cfg->s.crgSap.selector != SELECTOR_TC) &&
             (cfg->s.crgSap.selector != SELECTOR_LC))
         {
            ret = LCM_REASON_INVALID_PAR_VAL;
      ...
      case STTFUSAP:
#ifndef CL_MAC_LWLC 
         if ((cfg->s.tfuSap.selector != SELECTOR_TC) &&
             (cfg->s.tfuSap.selector != SELECTOR_LC))
         {
            ret = LCM_REASON_INVALID_PAR_VAL;
...
#endif
{
   ...
   if ((cfg->s.genCfg.lmPst.selector != SELECTOR_TC) &&
       (cfg->s.genCfg.lmPst.selector != SELECTOR_LC))
   {
      RLOG0(L_ERROR, "unsupported Selector value for RGU");
