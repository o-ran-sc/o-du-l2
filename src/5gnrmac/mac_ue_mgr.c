   ...
   if(schUeDelRsp)
   {
      if(schUeDelRsp->rsp == RSP_OK)
      {
         ...
         if(cellCb)
         {
            ...
            {
               deleteMacUeCb(&ueCb);
               if(MacSendUeDeleteRsp(schUeDelRsp->cellId, schUeDelRsp->crnti, SUCCESS) != ROK)
               {
            ...
            {
               if(MacSendUeDeleteRsp(schUeDelRsp->cellId, schUeDelRsp->crnti, UEIDX_INVALID) != ROK)
               {
                  DU_LOG("\nERROR  -->  MAC: MacProcSchUeDeleteRsp(): Failed to send UE delete response");
                  ret = RFAILED;
               }
         ...
         {
            if(MacSendUeDeleteRsp(schUeDelRsp->cellId, schUeDelRsp->crnti, CELLID_INVALID) != ROK)
            {
               DU_LOG("\nERROR  -->  MAC: MacProcSchUeDeleteRsp(): Failed to send UE delete response");  
               ret = RFAILED;
            }
...
uint8_t MacProcUeDeleteReq(Pst *pst, MacUeDelete *ueDelete)
{
   ...
   if(ueDelete)
   {
      ...
      if(cellCb)
      {
         ...
         {
            MacSendUeDeleteRsp(ueDelete->cellId, ueDelete->crnti, UEIDX_INVALID);
            MAC_FREE_SHRABL_BUF(pst->region, pst->pool, ueDelete, sizeof(MacUeDelete));
      ...
      {
         MacSendUeDeleteRsp(ueDelete->cellId, ueDelete->crnti, CELLID_INVALID);
         MAC_FREE_SHRABL_BUF(pst->region, pst->pool, ueDelete, sizeof(MacUeDelete));
