...
uint8_t fillMacLcCfgList(MacUeCb *ueCb, MacUeCfg *ueCfg)
{
   ...
   for(lcIdx = 0; lcIdx < ueCfg->numLcsToAdd; lcIdx++)
   {
      ...
      {
      ueCb->dlInfo.lcCb[ueCb->dlInfo.numDlLc].lcId = ueCfg->lcCfgToAddList[lcIdx].lcId;
      ...
      else
      {
         DU_LOG("\nMAC : LC to be configured[%d] for lcIdx [%d] has reached MAX limit[%d]",\
          ueCb->dlInfo.numDlLc, lcIdx,  MAX_NUM_LC);
         return RFAILED;
      }
   }
   /*LC to be Modified */
   if(ueCfg->numLcsToMod > MAX_NUM_LC)
   {
      DU_LOG("\nMAC : Number of LC to configure[%d] exceeds limit[%d] while modifying",\
	    ueCfg->numLcsToMod, MAX_NUM_LC);
      return RFAILED;
   }
   else if(ueCfg->numLcsToMod > 0)
   {
      for(lcIdx = 0; lcIdx < ueCb->dlInfo.numDlLc; lcIdx++)
      {
         ...
         {
            if(ueCb->dlInfo.lcCb[lcIdx].lcId == ueCfg->lcCfgToModList[ueLcIdx].lcId) //searching for Lc to be mod
            {
	       lcId = ueCfg->lcCfgToModList[ueLcIdx].lcId;
               ueCb->dlInfo.lcCb[lcIdx].lcId = ueCfg->lcCfgToModList[ueLcIdx].lcId;
               ueCb->dlInfo.lcCb[lcIdx].lcState = MAC_LC_STATE_ACTIVE;
   ...
   else if(ueCfg->numLcsToDel > 0)
   {
      for(lcIdx = 0; lcIdx < ueCb->dlInfo.numDlLc; lcIdx++)
      {
         for(ueLcIdx = 0; ueLcIdx < ueCb->dlInfo.numDlLc; ueLcIdx++)
         {
            if(ueCb->dlInfo.lcCb[lcIdx].lcId == ueCfg->lcCfgToDelList[ueLcIdx].lcId) //searching for Lc to be del
            {
	       lcId = ueCfg->lcCfgToDelList[ueLcIdx].lcId;
               memset(&ueCb->dlInfo.lcCb[lcIdx], 0, sizeof(LcCfg));
	       (ueCb->dlInfo.numDlLc)--;
...
uint8_t fillSchLcCfgList(SchUeCfg *schUeCfg, MacUeCfg *ueCfg)
{
   ...
   if(schUeCfg->numLcToAdd > MAX_NUM_LC)
   {
      DU_LOG("\nMAC : Number of Logical channels %d exceeds max limit %d while adding",\
	    schUeCfg->numLcToAdd, MAX_NUM_LC);
   }  
   for(idx = 0; ((idx < schUeCfg->numLcToAdd) && (idx < MAX_NUM_LC)); idx++)
   {
   ...
   if(schUeCfg->numLcToMod > MAX_NUM_LC)
   {
      DU_LOG("\nMAC : Number of Logical channels %d exceeds max limit %d while modifying",\
	    schUeCfg->numLcToMod, MAX_NUM_LC);
   }
   for(idx = 0; ((idx < schUeCfg->numLcToMod) && (idx < MAX_NUM_LC)); idx++)
   {
   ...
   if(schUeCfg->numLcToDel > MAX_NUM_LC)
   {
      DU_LOG("\nMAC : Number of Logical channels %d exceeds max limit %d while deleting",\
	    schUeCfg->numLcToDel, MAX_NUM_LC);
   }
   for(idx = 0; ((idx < schUeCfg->numLcToDel) && (idx < MAX_NUM_LC)); idx++)
   {
