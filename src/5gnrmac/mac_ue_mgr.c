...
uint8_t createUeCb(MacUeCfg *ueCfg)
{
   ...
   ueCb->state = UE_STATE_ACTIVE;
	...
	/* Fill BSR info */
	ueCb->ulInfo.bsrInfo.periodicTimer = ueCfg->macCellGrpCfg.bsrCfg.periodicTimer;
	ueCb->ulInfo.bsrInfo.retxTimer = ueCfg->macCellGrpCfg.bsrCfg.retxTimer;
	if(ueCfg->macCellGrpCfg.bsrCfg.srDelayTimer)
	{
	   //TODO: fill srDelayTimer in ueCb. Currently this is NULL
	}
