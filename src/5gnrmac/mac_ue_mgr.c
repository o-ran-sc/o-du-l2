...
uint8_t MacSendUeDeleteRsp(uint16_t cellId, uint16_t crnti, CausesOfResult cause)
{
...
uint8_t MacProcSchUeDeleteRsp(Pst *pst, SchUeDeleteRsp *schUeDelRsp)
{
   ...
   if(schUeDelRsp)
   {
      if(schUeDelRsp->rsp == RSP_OK)
      {
         ...
         cause = schUeDelRsp->cause;
      
      if(MacSendUeDeleteRsp(schUeDelRsp->cellId, schUeDelRsp->crnti, cause) != ROK)
      {
...
uint8_t MacProcUeDeleteReq(Pst *pst, MacUeDelete *ueDelete)
{
   uint8_t ret = ROK;
   uint8_t cellIdx=0;
   CausesOfResult cause=SUCCESSFUL;
   MacUeCb  *ueCb = NULLP;
   ...
   if(ueDelete)
   {
      ...
      if(cause!= SUCCESSFUL)
      {
         MacSendUeDeleteRsp(ueDelete->cellId, ueDelete->crnti, cause);
         MAC_FREE_SHRABL_BUF(pst->region, pst->pool, ueDelete, sizeof(MacUeDelete));
         ret = RFAILED;
      }
   }
...
uint8_t MacSendUeResetRsp(uint16_t cellId, uint16_t ueId, CausesOfResult cause)
{
...
uint8_t MacProcUeResetReq(Pst *pst, MacUeResetReq *ueReset)
{
   ...
   if(ueReset)
   {
      ...
      MacSendUeResetRsp(ueReset->cellId, ueReset->ueId, cause);
