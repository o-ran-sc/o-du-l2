...
uint8_t fillInitalUlBwpPuschCfg(PuschCfg macPuschCfg, SchPuschCfg *schPuschCfg)
{
	...
	for(idx = 0; idx < schPuschCfg->numTimeDomRsrcAlloc; idx++)
	{
		...
		schPuschCfg->timeDomRsrcAllocList[idx].startSymbol = macPuschCfg.timeDomRsrcAllocList[idx].startSymbol; 
...
uint8_t fillInitDlBwpPdschCfg(PdschConfig macPdschCfg, SchPdschConfig *schPdschCfg)
{
	...
	for(idx = 0; idx < schPdschCfg->numTimeDomRsrcAlloc; idx++)
	{
		...
		schPdschCfg->timeDomRsrcAllociList[idx].startSymbol = macPdschCfg.timeDomRsrcAllociList[idx].startSymbol;
		schPdschCfg->timeDomRsrcAllociList[idx].symbolLength = macPdschCfg.timeDomRsrcAllociList[idx].symbolLength;
	}
