...
uint8_t sendUeDelReqToSch(MacUeDelete *ueDelete)
{
   if(ueDelete != NULLP)
   {
      Pst schPst;
...
void deleteMacUeCb(MacUeCb  *ueCb)
{
   ...
   memset(&ueCb[ueCb->ueIdx-1], 0, sizeof(MacUeCb));
}
/*******************************************************************
*
   ...
   if(schUeDelRsp)
   {
      if(schUeDelRsp->rsp == RSP_OK)
      {
         ...
         if(cellCb)
         {
            ...
            {
               deleteMacUeCb(ueCb);
               if(MacSendUeDeleteRsp(SUCCESS, schUeDelRsp->cellId, schUeDelRsp->crnti) != ROK)
               {
...
uint8_t MacProcUeDeleteReq(Pst *pst, MacUeDelete *ueDelete)
{
   ...
   if(ueDelete)
   {
      ...
      if(cellCb)
      {
         ...
         {
            ret = sendUeDelReqToSch(ueDelete);
            if(ret != ROK)
            {
