 ...
 *
 * ****************************************************************/
uint16_t MacHdlDlCcchInd(Pst *pst, DlCcchInd *dlCcchInd)
{
...
{
   Pst pst;
   UlCcchInd *ulCcchInd = NULLP;

   MAC_ALLOC_SHRABL_BUF(ulCcchInd, sizeof(UlCcchInd));
   if(!ulCcchInd)
   {
	...
	ulCcchInd->cellId = cellId;
	ulCcchInd->crnti  = crnti;
	memcpy(ulCcchInd->ulCcchMsg, rrcContainer, strlen((const
					char*)rrcContainer));
	...
	if(MacDuAppUlCcchInd(&pst, ulCcchInd) != ROK)
	{
		DU_LOG("\nMAC: Failed to send slot indication to DU APP");
		MAC_FREE_SHRABL_BUF(MAC_MEM_REGION, MAC_POOL, ulCcchInd, sizeof(UlCcchInd));
	}
