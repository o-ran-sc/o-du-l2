 ...
 *
 * ****************************************************************/
uint8_t fillPucchF0F1Pdu(UciPucchF0F1 *pduInfo, fapi_uci_o_pucch_f0f1_t *fapiPduInfo)
{
   ...
   if(fapiPduInfo->pduBitmap & SR_PDU_BITMASK)
   {
   ...
   if(fapiPduInfo->pduBitmap & HARQ_PDU_BITMASK)
   {
      ...
      {
         pduInfo->harqInfo.harqValue[idx] = fapiPduInfo->harqInfo.harqValue[idx];
      }
   }
   return ROK;
}
...
uint8_t procUciInd(fapi_uci_indication_t  *fapiUciInd)
{
   ...
   for(pduIdx = 0; pduIdx < uciInd.numUcis; pduIdx++)
   {
      ...
      switch(uciInd.pdus[pduIdx].pduType)
      {
         ...
         case UCI_IND_PUCCH_F0F1:
         {
            ...
            ret = fillPucchF0F1Pdu(pduInfo, &fapiUciInd->uciPdu[pduIdx].uci.uciPucchF0F1);
         }
         break;
   ...
   if(ret == ROK)
   {
      ...
      (*sendUciIndOpts[pst.selector])(&pst, &uciInd);
   }
   else
   {
