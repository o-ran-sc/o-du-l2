...
/*******************************************************************
 *
 * @brief Pack and Send UCI Ind from MAC to SCH
 *
 * @details
 *
 *    Function : packMacSchUciInd
 *
 *    Functionality:
 *       Pack and Send UCI Ind from MAC to SCH
 *
 * @params[in] 
 * @return ROK     - success
 *         RFAILED - failure
 *
 * ****************************************************************/
uint8_t packMacSchUciInd(Pst *pst, SrUciIndInfo *uciInd)
{
   if((pst->selector == ODU_SELECTOR_LC) || (pst->selector == ODU_SELECTOR_LWLC))
   {
      /* TODO */
