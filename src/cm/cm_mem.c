...
uint32_t getT2kMemLeakIndex(U64 address, Region region)
{
...
void InsertToT2kMemLeakInfo(U64 address, uint32_t size, uint32_t lineNo, char* fileName, Region region)
{
...
void RemoveFromT2kMemLeakInfo(U64 address, char *file, uint32_t line,Region region)
{
   T2kMeamLeakInfo *leakInfo;
...
/* cm_mem_c_001.main_12 - addition for initializing the hash table */
    ...
    if(cmHashListInit(&regCb->brdcmSsiLstCp, 1000, offset, FALSE, 
       CM_HASH_KEYTYPE_uint32_tMOD, region, 0) != ROK)
    {
       return RFAILED;
    }
#endif /* SSI_DEBUG_LEVEL1 */
...
/*cm_mem_c_001.main_23 Fix for specifier mismatch warnings in 64BIT compilation*/          
      ...
      *ptr = (Data *)(bkt->nextBlk) + (sizeof(CmMmBlkHdr)); /* ccpu00125353: warning fix */
      ...
      cmMemset((uint8_t *)*ptr, 0xAB, *size);
#endif
...
#endif
{
    ...
    retVal = cmHashListFind(&regCb->brdcmSsiLstCp,(uint8_t *)&ptrHdr,
                   (uint16_t)sizeof(U64), 0, (PTR *)&ptrHdr);                              
#else
    retVal = cmHashListFind(&regCb->brdcmSsiLstCp,(uint8_t *)&ptrHdr,
...
/*cm_mem_c_001.main_27 SSI-4GMX specfic changes*/   
   ...
   cmMemset((uint8_t*)funcNm, 0, (sizeof(uint32_t) * CM_MAX_STACK_TRACE));
#else
...
/*cm_mem_c_001.main_27 SSI-4GMX specfic changes*/   
   ...
   cmMemset((uint8_t*)allocInfo, 0, sizeof(MemAllocInfo));
#else
...
#endif
{
   ...
   for(memMdl = 0; memMdl < CM_MEM_USR_MDL; memMdl++)
   {
       ...
       cmHashListInit(&memLkCb.memUsrMdl[memMdl][hashIdx].memHashCp,
                      500, 0, FALSE, CM_HASH_KEYTYPE_uint32_tMOD, 0, 0);
