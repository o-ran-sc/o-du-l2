...
* @details
*
*    Function : packRrcDeliveryMsgToDu
*
*    Functionality:
*      Unpacks the DL RRC Message info received at RLC from DU APP
*
...
*
* ****************************************************************/
uint8_t packRrcDeliveryMsgToDu(Pst *pst, RrcDeliveryReportInfo *rrcDeliveryMsg)
{
    Buffer *mBuf = NULLP;
    ...
    if(pst->selector == ODU_SELECTOR_LWLC)
    {
       if (ODU_GET_MSG_BUF(pst->region, pst->pool, &mBuf) != ROK)
       {
          DU_LOG("\nRLC : Memory allocation failed at packRrcDeliveryMsgToDu");
          return RFAILED;
       }
       /* pack the address of the structure */
    ...
    {
       DU_LOG("\nRLC: Only LWLC supported for packRrcDeliveryMsgToDu");
    }
    return RFAILED;
 }
...
* @details
*
*    Function : unpackRrcDeliveryMsgToDu
*
*    Functionality:
...
*
* ****************************************************************/
uint8_t unpackRrcDeliveryMsgToDu(RlcRrcDeliveryMsgToDuFunc func, Pst *pst, Buffer *mBuf)
{
    if(pst->selector == ODU_SELECTOR_LWLC)
    {
