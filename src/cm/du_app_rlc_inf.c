 ...
 *
 * ****************************************************************/
uint8_t packDlDataMsgToRlc(Pst *pst, RlcDlDataMsgInfo *dlDataMsg)
{
   Buffer *mBuf = NULLP;
   ...
   if(pst->selector == ODU_SELECTOR_LWLC)
   {
      if (ODU_GET_MSG_BUF(pst->region, pst->pool, &mBuf) != ROK)
      {
         DU_LOG("\nRLC : Memory allocation failed at packRlcDlDataMsgToRlc");
         return RFAILED;
      }
      /* pack the address of the structure */
   ...
   {
      DU_LOG("\nRLC: Only LWLC supported for packDlDataMsgToRlc");
   }
   return RFAILED;
}
 ...
 *
 * ****************************************************************/
uint8_t unpackDlDataMsgToRlc(DuDlDataMsgToRlcFunc func, Pst *pst, Buffer *mBuf)
{
   if(pst->selector == ODU_SELECTOR_LWLC)
   {
      RlcDlDataMsgInfo *dlDataMsgInfo;
      /* unpack the address of the structure */
   ...
   {
      /* Nothing to do for other selectors */
      DU_LOG("\nRLC: Only LWLC supported for DL Data Message transfer ");
      ODU_PUT_MSG_BUF(mBuf);
   }
   return RFAILED;
}
