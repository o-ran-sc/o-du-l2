...
* @details
*
*    Function : packDuRlcUeReestablishmentReq 
*
*    Functionality:
...
uint8_t packDuRlcUeReestablishmentReq(Pst *pst, RlcUeReestablishmentReq *ueDelete)
{
   Buffer *mBuf = NULLP;
   ...
   if(pst->selector == ODU_SELECTOR_LWLC)
   {
      if (ODU_GET_MSG_BUF(pst->region, pst->pool, &mBuf) != ROK)
      {
         DU_LOG("\nERROR  --> RLC : Memory allocation failed at packDuRlcUeReestablishmentReq");
         return RFAILED;
      }
      /* pack the address of the structure */
      CMCHKPK(oduPackPointer,(PTR)ueDelete, mBuf);
   ...
   {
      DU_LOG("\nERROR  -->  RLC: Only LWLC supported for packDuRlcUeReestablishmentReq");
      return RFAILED;
   }
...
* @details
*
*    Function : unpackRlcUeReestablishmentReq 
*
*    Functionality:
...
uint8_t unpackRlcUeReestablishmentReq(DuRlcUeReestablishmentReq func, Pst *pst, Buffer *mBuf)
{
    if(pst->selector == ODU_SELECTOR_LWLC)
    {
       RlcUeReestablishmentReq *ueDelete;
       /* unpack the address of the structure */
       CMCHKUNPK(oduUnpackPointer, (PTR *)&ueDelete, mBuf);
       ODU_PUT_MSG_BUF(mBuf);
       return (*func)(pst, ueDelete);
...
* @details
*
*    Function : packRlcDuUeReestablishmentRsp
*
*    Functionality:
...
*
* @params[in] Post structure pointer
*             RlcUeReestablishmentRsp *ueReestablishment
*
* @return ROK     - success
*         RFAILED - failure
*
...
uint8_t packRlcDuUeReestablishmentRsp(Pst *pst, RlcUeReestablishmentRsp *ueReestablishment)
{
   Buffer *mBuf = NULLP;
   ...
   if(pst->selector == ODU_SELECTOR_LWLC)
   {
      if (ODU_GET_MSG_BUF(pst->region, pst->pool, &mBuf) != ROK)
      {
         DU_LOG("\nERROR  --> RLC : Memory allocation failed at packRlcDuUeReestablishmentRsp");
         return RFAILED;
      }
      /* pack the address of the structure */
      CMCHKPK(oduPackPointer,(PTR)ueReestablishment, mBuf);
   }
   else
   {
      DU_LOG("\nERROR  -->  RLC: Only LWLC supported for packRlcDuUeReestablishmentRsp");
      return RFAILED;
   }
...
* @details
*
*    Function : unpackRlcUeReestablishmentRsp
*
*    Functionality:
...
uint8_t unpackRlcUeReestablishmentRsp(RlcDuUeReestablishmentRsp func, Pst *pst, Buffer *mBuf)
{
    if(pst->selector == ODU_SELECTOR_LWLC)
    {
       ...
       /* unpack the address of the structure */
       CMCHKUNPK(oduUnpackPointer, (PTR *)&ueReestablishmentRsp, mBuf);
       ODU_PUT_MSG_BUF(mBuf);
       return (*func)(pst, ueReestablishmentRsp);
    }
    else
    {
       /* Nothing to do for other selectors */
