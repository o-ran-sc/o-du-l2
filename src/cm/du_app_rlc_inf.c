...
{
   static uint8_t rlcLcId = MIN_DRB_LCID;
   
   if(rlcLcId >= MIN_DRB_LCID && rlcLcId < MAX_DRB_LCID)
   {
      rlcLcId = rlcLcId % MAX_DRB_LCID;
   }
   else
   {
      rlcLcId = MIN_DRB_LCID;
   }
   return rlcLcId++;
}
...
uint8_t packDlRrcMsgToRlc(Pst *pst, RlcDlRrcMsgInfo *dlRrcMsgInfo)
{
   ...
   if(pst->selector == ODU_SELECTOR_LWLC)
   {
      if (ODU_GET_MSG_BUF(pst->region, pst->pool, &mBuf) != ROK)
      {
         DU_LOG("\nRLC : Memory allocation failed at packDlRrcMsgToRlc");
         return RFAILED;
      }
      /* pack the address of the structure */
