...
{
   uint8_t sst[SST_SIZE]; 
   uint8_t sd[SD_SIZE];
}Snssai;

/*
typedef struct qosCha
{
   uint16_t fiveQI;
   uint16_t avgWindow;
...
typedef uint16_t (*DuMacUeCreateReq) ARGS((
   Pst           *pst,
   MacUeCfg      *macUeCfg ));
...
extern uint16_t packDuMacUeCreateReq(Pst *pst, MacUeCfg *macUeCfg);
extern uint16_t MacHdlUeCreateReq(Pst *pst, MacUeCfg *macUeCfg);
