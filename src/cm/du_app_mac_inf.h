...
#if 0
typedef struct macStatsModifyGrpInfo
{
   uint8_t   groupId;
   uint16_t  periodicity;  /* In milliseconds */
   uint8_t   numStats;
   MacMeasurementType statsList[MAX_NUM_STATS];
}MacStatsModifyGrpInfo;

typedef struct macStatsModificationReqReq
{
   uint64_t    subscriptionId;
   uint8_t     numStatsGroup;
   MacStatsModifyGrpInfo statsGrpList[MAX_NUM_STATS_GRP];
}MacStatsModificationReq;

typedef struct macStatsModificationRsp
{
   uint64_t             subscriptionId;
   uint8_t              numGrpAccepted;
   uint8_t              statsGrpAcceptedList[MAX_NUM_STATS_GRP];
