...
)
{
   ...
   {
      DU_LOG("\nERROR  --> CM: cmDynAlloc(): regionCb is null");
      return RFAILED;
   }
#endif
...
/*  cm_mem_c_001.main_15 : Additions */
   ...
   if ((regCb == NULLP) || (size == NULLP) || !(*size) || (ptr == NULLP))
   {
      DU_LOG("\nERROR  --> CM : cmAlloc(): regionCb is null");
      return RFAILED;
   }
#endif
...
/* cm_mem_c_001.main_12 - addition for checking memType parameter */
      ...
      {
         DU_LOG("\nERROR  --> CM : cmAlloc(): memType is invalid");
         return RFAILED;
      }
...
/*cm_mem_c_001.main_23 Fix for specifier mismatch warnings in 64BIT compilation*/          
   ...
   if ( *size <= regCb->bktMaxBlkSize)
   {
      ...
      { 
         /* Some fatal error in the map table initialization. */
         DU_LOG("\nERROR  --> CM : cmAlloc(): bktIdx is null");
         return RFAILED;
      }
#endif
...
/*cm_mem_c_001.main_23 Fix for specifier mismatch warnings in 64BIT compilation*/          
               ...
               else
               {
                  ...
                  (Void) SUnlock(&(bkt->bktLock));
#endif
                  DU_LOG("\nERROR  --> CM : cmAlloc(): cmMmBktSanityChk return RTRAMPLINGNOK");
                  /* return RFAILED */
                  return RFAILED;
               }
         }
...
/* cm_mem_c_001.main_12 - addition for passing an extra parameter */
   ...
   /* No memory available */
   DU_LOG("\nERROR  --> CM : cmAlloc(): regCb->heapFlag is null");
   return RFAILED;
#else /* use pure is on */
...
)
{
   ...
   {
      DU_LOG("\nERROR --> CM : cmDynFree(): regCb is null");
      return RFAILED;
   }
   ...
   {
      DU_LOG("\nERROR --> CM : cmDynFree(): Memory region is not valid");
      return RFAILED;
   }
	/* cm_mem_c_001.main_20 Addition */
   ...
   if (size > bkt->size)
   {
      DU_LOG("\nERROR --> CM : cmDynFree(): Size = %d bucket size = %d\n", size, bkt->size);
      exit(-1);
      bkt = &(regCb->bktTbl[bktIdx = regCb->mapTbl[++idx].bktIdx]);
   }
   if(size > bkt->size)
   {
      DU_LOG("\nERROR --> CM : cmDynFree(): 2nd time Size = %d bucket size = %d\n", size, bkt->size);
      exit(-1);
   ...
   if(dynMemElem == NULLP)
   {
      DU_LOG("\nERROR --> CM : cmDynFree(): dynMemElem is not valid");
      return RFAILED;
   }
...
/*  cm_mem_c_001.main_15 : Additions */
   ...
   if (ptr >= ((CmMmRegCb *)regCb)->regInfo.start +
               ((CmMmRegCb *)regCb)->regInfo.size) 
   {
      DU_LOG("\nERROR --> CM : cmFree(): Memory block region is not correct");
      return RFAILED;
   }
	/* cm_mem_c_001.main_20 Addition */
	if (ptr < regCb->regInfo.start)
	{
     DU_LOG("\nERROR --> CM : cmFree(): ptr is less than regCb->regInfo.start");
	  return RFAILED;
	}
   ...
   if (ptr < (regCb->regInfo.start + regCb->bktSize))
   {
      ...
      { 
         /* Some fatal error in the map table initialization. */
         DU_LOG("\nERROR --> CM : cmFree(): bktIdx is null");
         return RFAILED;
      }
#endif
           ...
           else
           {
               ...
               */
               ...
               (Void) SUnlock(&(bkt->bktLock));
                ...
                DU_LOG("\nERROR  --> CM : cmFree(): cmMmBktSanityChk returns  RTRAMPLINNOK");    
                return (RTRAMPLINGNOK);
           }
      }
