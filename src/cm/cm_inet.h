...
#define CM_INET_NTOH_uint32_t(_long)  ntohl(_long)
#define CM_INET_HTON_uint32_t(_long)  htonl(_long)
#define CM_INET_NTOH_uint16_t(_word)  ntohs(_word)
#define CM_INET_HTON_uint16_t(_word)  htons(_word)

/* peeks a uint8_t from the given position */
#define CM_INET_PEEK_uint8_t(_sockFd, _fromAddr, _info, _pos, _octet, _ret)  \
 ...
 */
#define CM_INET_PEEK_uint16_t(_sockFd, _fromAddr, _info, _pos, _word, _ret)  \
   {  \
      uint8_t _tempWord[2];  \
      \
 ...
 * represented in big endian representation within the data stream)
 */
#define CM_INET_PEEK_uint32_t(_sockFd, _fromAddr, _info, _pos, _long, _ret)  \
   {  \
      uint8_t _tempLong[4];  \
      \
...
{                                                       \
   (Void)cmMemcpy((uint8_t *)_addrToFill, (uint8_t *)_fromAddr, sizeof(CmInetIpAddr6)); \
}
...
#define CM_COPY_VWIPADDR(vwIpAddr, addr) \
   { \
      (Void)cmMemcpy((uint8_t *)addr, (uint8_t *)&vwIpAddr, sizeof(S32)); \
   }
#endif
