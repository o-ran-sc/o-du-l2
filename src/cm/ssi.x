...
S16 SChekMemUtilization ARGS((Region region,Bool *memAlarm));
 ...
 S16 oduUnpackUInt8 ARGS((U8 val,Buffer *mBuf));
 S16 SPkS16 ARGS((S16 val,Buffer *mBuf));
 S16 oduUnpackUInt16 ARGS((U16 val,Buffer *mBuf));
 S16 SPkS32 ARGS((S32 val,Buffer *mBuf));
 S16 oduUnpackUInt32 ARGS((U32 val,Buffer *mBuf));
/* ssi_x_001.main_49 : added packing of F32 and F64*/
 ...
 S16 oduPackUInt8 ARGS((U8 *val,Buffer *mBuf));
 S16 SUnpkS16 ARGS((S16 *val,Buffer *mBuf));
 S16 oduPackUInt16 ARGS((U16 *val,Buffer *mBuf));
 S16 SUnpkS32 ARGS((S32 *val,Buffer *mBuf));
 S16 oduPackUInt32 ARGS((U32 *val,Buffer *mBuf));
...
#define SPutDBuf(region,pool,buf) SPutDBufNew(region,pool,buf,file,line)
 S16 SGetDBufNew ARGS((Region region,Pool pool,Buffer * *bufPtr,char*,U32));
 S16 SPutDBufNew ARGS((Region region,Pool pool,Buffer *buf,char*,U32));
...
#define SPutZbcDBuf(region,buf) SPutZbcDBufNew(region,buf,__FILE__,__LINE__)
 ...
 S16 SGetDBufNew ARGS((Region region,Pool pool,Buffer * *bufPtr,U32 line,U8 *fnName));
 S16 SPutDBufNew ARGS((Region region,Pool pool,Buffer *buf, U32 line, U8 *fnName));
#else
...
/* ssi_x_001.main_59 : Added compile time flag */
 ...
 S16 SGetMsgNew ARGS((Region region, Pool pool, Buffer * *mBufPtr, U32 line, U8 *fnName));
 S16 SGetSBufNew ARGS((Region region,Pool pool, Data * *bufPtr, Size size, U32 line, U8* fnName));
 S16 SPutMsgNew ARGS((Buffer *mBuf, U32 line,  U8 *fnName));
 S16 SPutSBufNew ARGS((Region region, Pool pool, Data *buf, Size size, U32 line, U8 *fnName));
#else /*SS_LIGHT_MEM_LEAK_STS */
#ifdef SS_HISTOGRAM_SUPPORT 
 S16 SPutMsgNew ARGS((Buffer *mBuf, U32 line,  U8 *fileName));
 S16 SGetMsgNew ARGS((Region region, Pool pool, Buffer * *mBufPtr, U32 line,  U8 *fileName));
 S16 SGetSBufNew ARGS((Region region,Pool pool, Data * *bufPtr, Size size, U32 line, U8 *fileName));
 S16 SPutSBufNew ARGS((Region region, Pool pool, Data *buf, Size size, U32 line, U8 *fileName));
#else
...
#define SGetMsg(region,pool,mBuf) SGetMsgNew(region,pool,mBuf,__FILE__,__LINE__)
 S16 SPutMsgNew ARGS((Buffer *mBuf,char*, U32));
 S16 SGetMsgNew ARGS((Region region, Pool pool, Buffer * *mBufPtr, char*,U32));
#else
...
#define SGetSBuf(region,pool,bufPtr,size) SGetSBuf1(region,pool,bufPtr,size,__FILE__,__LINE__)
 S16 SGetSBuf1 ARGS((Region region,Pool pool, Data * *bufPtr, Size size, char* file, U32 line));
...
#define SPutSBuf(region,pool,buf,size) SPutSBuf1(region,pool,buf,size,__FILE__,__LINE__)
 S16 SPutSBuf1 ARGS((Region region, Pool pool, Data *buf, Size size, char*, U32));
#else
...
S16 SPutSBufWls1(Region region, Pool pool, Data *ptr, Size size,char* file, U32 line);
S16 SGetSBufWls1(Region region, Pool pool, Data **ptr, Size size,char* file, U32 line);  
...
S16 SAttachWlsPtrToMBuf1(Region region, Pool pool, Data *ptr, Data *readPtr, MsgLen totalLen, MsgLen ptrLen, Buffer** mBuf,char* file, U32 line);
...
#endif
#endif
 S16 SGetStaticBuffer ARGS((Region region,Pool pool, Data * *bufPtr, Size size, U8 memType));
 S16 SPutStaticBuffer ARGS((Region region, Pool pool, Data *buf, Size size, U8 memType));
...
#define SAddPstMsgMult(src, cnt, mBuf) SAddPstMsgMult1(src, cnt, mBuf, __FILE__,__LINE__)
 ...
 S16 SAddPreMsgMult1 ARGS((Data *src,MsgLen cnt,Buffer *mBuf, char *file, U32 line));
 S16 SAddPstMsgMult1 ARGS((Data *src,MsgLen cnt,Buffer *mBuf, char *file, U32 line));
#else
...
#define SSegMsg(mBuf1, idx, mBuf2) SSegMsgNew(mBuf1, idx, mBuf2, __FILE__, __LINE__)
 S16 SSegMsgNew ARGS((Buffer *mBuf1,MsgLen idx,Buffer **mBuf2,char*,U32));
#else
...
Void SIncMsgLen ARGS((Buffer *mBuf));
 ...
 S16 SChkResUtl ARGS((Region region,U8 *wSum));
...
/* ssi_x_001.main_48: Added Timestamp changes */
 ...
 S16 SGetRefTime ARGS((U32 refTime, U32 *sec, U32 *usec));
...
S16 SSetAffinity ARGS((SSTskId *tskId, SsAffinityMode mode, U32 coreId, SSTskId *tskAssociatedTskId));
S16 SGetAffinity ARGS((SSTskId *tskId, U32 *coreId));
...
S16 SCacheFlush ARGS( (U16 cache_type, Data *addr, Size size));
S16 SCacheInvalidate ARGS( (U16 cache_type, Data *addr, Size size));
S16 SAlignDBufEven ARGS((Buffer *dBuf));
S16 SAlignDBuf ARGS((Buffer *dBuf, U32 align));
S16 SInitNxtDBuf ARGS((Buffer *mBuf));
S16 SGetNxtDBuf ARGS((Buffer *mBuf, Buffer **dBuf));
S16 SChkNxtDBuf ARGS((Buffer *mBuf));
S16 SSetIntPend ARGS((U16 id, Bool flag));
S16 SChkMsg ARGS((Buffer *mBuf));
 ...
 S16 SPutIsrDpr ARGS((VectNmb vectNmb, Void *context, PIF isrFnct, 
                             PIF dprFnct));
 S16 SSyncInt   ARGS((U16 adapterNmb, PFVOID syncFnct, 
                             Void *syncContext));
#endif
...
/*ssi_x_001.main_62-prototype for SAlloc and SFree, gaurded under flag*/
 ...
 /* ssi_x_001.main_57 : Additions */
#ifdef SS_HISTOGRAM_SUPPORT 
S16 SAlloc ARGS((Region region, Size *size, U32 flags, Data **ptr,U32 line, U8 *fileName, U8 entId));
S16 SFree ARGS((Region region, Data *ptr, Size size, U32 line, U8 *fileName, U8 entId));
#else
#ifdef T2K_MEM_LEAK_DBG
...
S16 SAllocNew ARGS((Region region, Size *size, U32 flags, Data **ptr,char*,U32));
S16 SFreeNew ARGS((Region region, Data *ptr, Size size,char*,U32));
#elif defined(SS_LIGHT_MEM_LEAK_STS)
S16 SAlloc ARGS((Region region, Size *size, U32 flags, Data **ptr,U32 line, U8 *fnName));
S16 SFree ARGS((Region region, Data *ptr, Size size, U32 line, U8 *fnName));
#else
S16 SAlloc ARGS((Region region, Size *size, U32 flags, Data **ptr));
...
#ifdef SS_MULTIPLE_PROCS
S16 SAddProcIdLst ARGS((U16 numPIds, ProcId *pIdLst));
S16 SRemProcIdLst ARGS((U16 numPIds, ProcId *pIdLst));
S16 SGetProcIdLst ARGS((U16 *numPIds, ProcId *pIdLst));
#endif /* SS_MULTIPLE_PROCS */
...
#ifdef SS_WIN
S16 WTInitLock     ARGS((SLockId *lock,U8 type));
...
S16 SRegLogCfg ARGS(( U8 mode, S8 *path, U32 size, S8 *IPA, U16 port));
...
S16 SGetEntInd ARGS((Ent *entId, U8 *fileName));
#endif /* SS_HISTOGRAM_SUPPORT */
...
S16 SLockNew ARGS((SLockInfo *LockId, U8 lockType));
S16 SInitLockNew ARGS((SLockInfo *LockId, U8 lockType));
S16 SUnlockNew ARGS((SLockInfo *LockId, U8 lockType));
S16 SDestroyLockNew ARGS((SLockInfo *LockId, U8 lockType));
...
S16 SGetReadPtr ARGS((Buffer *mBuf, U8** data, MsgLen *len));
...
Void rbCallstackShowForCore ARGS((U32 coreId));
U32 MacGetTick ARGS ((void));
#endif /* MSPD */
#ifdef LTE_L2_MEAS
extern U64  glblTtiCnt;
#endif

S16 SStartTask ARGS((VOLATILE U32 *startTime, U32 tarkId));
S16 SStopTask ARGS((VOLATILE U32 startTime,U32 taskId));
#ifdef MSPD_MLOG_NEW
