...
#ifdef ANSI
S16 cmPkRlcuFlowCntrlIndInfo
(
RlcuFlowCntrlIndInfo *param,
Buffer              *mBuf
)
#else
S16 cmPkRlcuFlowCntrlIndInfo(param, mBuf)
RlcuFlowCntrlIndInfo  *param;
Buffer               *mBuf;
#endif
{
   TRC3(cmPkRlcuFlowCntrlIndInfo);

   CMCHKPK(oduUnpackUInt32, param->pktAdmitCnt, mBuf);   
   CMCHKPK(cmPkLteRlcId, &param->rlcId, mBuf);

   return ROK;
} /* cmPkRlcuFlowCntrlIndInfo */

#ifdef ANSI
S16 cmUnpkRlcuFlowCntrlIndInfo
(
RlcuFlowCntrlIndInfo *param,
Buffer              *mBuf
)
#else
S16 cmUnpkRlcuFlowCntrlIndInfo(param, mBuf)
RlcuFlowCntrlIndInfo  *param;
Buffer               *mBuf;
#endif
{
   TRC3(cmUnpkRlcuFlowCntrlIndInfo);
   
   CMCHKUNPK(cmUnpkLteRlcId, &param->rlcId, mBuf);
   CMCHKUNPK(oduPackUInt32, &param->pktAdmitCnt, mBuf);
  
   return ROK;
} /* cmUnpkRlcuFlowCntrlIndInfo */
...
Pst            *pst,
SuId           suId,
RlcuFlowCntrlIndInfo *flowCntrlIndInfo
)
#else
...
#endif
{
   ...
   switch(pst->selector)
   {
      case ODU_SELECTOR_LC:
      {
         cmPkRlcuFlowCntrlIndInfo((flowCntrlIndInfo), mBuf);

         if (SPutStaticBuffer(pst->region, 
                      pst->pool, 
                      (Data *)flowCntrlIndInfo,
                      sizeof(RlcuFlowCntrlIndInfo),0) != ROK)
         {
            SPutMsg(mBuf);
            return RFAILED;
         }
...
{
   SuId   suId;
   RlcuFlowCntrlIndInfo *flowCntrlInfo = NULLP;
   ...
   switch(pst->selector)
   {
      ...
      case ODU_SELECTOR_LC:
      {
         ...
         cmUnpkRlcuFlowCntrlIndInfo(flowCntrlInfo, mBuf);
      }
      break;
