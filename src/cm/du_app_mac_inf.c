 ...
 *
 * ****************************************************************/
uint16_t packMacUlCcchInd(Pst *pst, UlCcchInd *ulCcchInd)
{
   Buffer *mBuf = NULLP;
	...
	if(pst->selector == DU_SELECTOR_LWLC)
	{
		...
		/* pack the address of the structure */
		CMCHKPK(cmPkPtr,(PTR)ulCcchInd, mBuf);
	}
	else
	{
...
uint16_t unpackMacUlCcchInd(DuMacUlCcchInd func, Pst *pst, Buffer *mBuf)
{
   if(pst->selector == DU_SELECTOR_LWLC)
   {
      ...
      /* unpack the address of the structure */
		CMCHKUNPK(cmUnpkPtr, (PTR *)&ulCcchInd, mBuf);
		SPutMsg(mBuf);
		return (*func)(pst, ulCcchInd);
	}
	else
	{
 ...
 *
 * ****************************************************************/
uint16_t packMacDlCcchInd(Pst *pst, DlCcchInd *dlCcchInd)
{
	Buffer *mBuf = NULLP;
	...
	if(pst->selector == DU_SELECTOR_LWLC)
	{
		...
		/* pack the address of the structure */
		CMCHKPK(cmPkPtr,(PTR)dlCcchInd, mBuf);
	}
	else
	{
...
uint16_t unpackMacDlCcchInd(DuMacDlCcchInd func, Pst *pst, Buffer *mBuf)
{
	if(pst->selector == DU_SELECTOR_LWLC)
	{
		...
		/* unpack the address of the structure */
		CMCHKUNPK(cmUnpkPtr, (PTR *)&dlCcchInd, mBuf);
		SPutMsg(mBuf);
		return (*func)(pst, dlCcchInd);
	}
	else
	{
		/* Nothing to do for other selectors */
