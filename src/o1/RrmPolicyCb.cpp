...
bool RrmPolicyCb::updateParams(string leafNode, string val)
{
	    ...
	    O1_LOG("\nO1 RrmPolicyCb::updateParams sst val = %d",
         ...
         else if(leafNode == "sd") 
         {
            memset(rrmPolicy[policyNum].rRMPolicyMemberList[memberNum].sd, \
	           '\0', SD_LEN);
            strncpy(rrmPolicy[policyNum].rRMPolicyMemberList[memberNum].sd, \
                    val.c_str(), SD_LEN);
            O1_LOG("\nO1 RrmPolicyCb::updateParams rrmPolicy[%d]."
		   "rRMPolicyMemberList[%d].sd[5] = %c", policyNum, memberNum, \
...
changed val=%s", change->new_val()->val_to_string().c_str());
	       string val = change->new_val()->val_to_string();
               ...
               if(!updateParams(leafNode,val.c_str())){
                  O1_LOG("\nO1 RrmPolicy : LeafNode  [%s] not handled", leafNode.c_str());
		  //later error handling can be done
	       }
            }
         }
	 uint8_t memberList = policyMemberListNum/MAX_POLICY_LIST;
#ifndef ODU_TEST_STUB
	 if(ROK != updateRrmPolicy(rrmPolicy, policyIndex, memberList))
	 {
