                                       ...
                                       void *private_data) 
{
    ...
    //read the data from map
    map<int,Alarm>::const_iterator it;

    AlarmManager& alrmMgr = AlarmManager::instance();
    const map<int,Alarm>& alrmList = alrmMgr.getAlarmList();

    for(it = alrmList.begin(); it !=alrmList.end(); it++)
    {
        O1_LOG("\nAlarm ID %d", it->second.getAlarmId());
        alarm.reset(new libyang::Data_Node(alarms, mod, "alarm"));
        id.reset(new libyang::Data_Node(alarm, mod, "alarm-id", to_string(it->second.getEventType()).c_str()));
