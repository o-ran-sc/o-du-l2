...
/* This file contains DU-RU connection Handler for CLA use case. It handles
...
int CellOpState::oper_get_items(sysrepo::S_Session session, \
                                       ...
                                       void *private_data)
{
   O1_LOG("O1 CellOpState : Callback called for path=%s on get request", path);
   ...
   {
      O1_LOG("\nO1 CellOpState : cellId = %d, opState=%d, cellState=%d\n", \
...
void CellOpState::print_change(sysrepo::S_Change change) {
   switch(change->oper()) {
      case SR_OP_CREATED:
         if (nullptr != change->new_val()) {
            O1_LOG("O1 CellOpState : CREATED: %s", \
                    change->new_val()->to_string().c_str());
         }
         break;
      case SR_OP_DELETED:
         if (nullptr != change->old_val()) {
             O1_LOG("O1 CellOpState : DELETED:  %s", \
                     change->old_val()->to_string().c_str());
         }
         break;
      case SR_OP_MODIFIED:
         if (nullptr != change->old_val() && nullptr != change->new_val()) {
            O1_LOG("O1 CellOpState : MODIFIED: old value %s :new value %s", \
                    change->old_val()->to_string().c_str(), \
                    change->new_val()->to_string().c_str());
        }
        break;
     case SR_OP_MOVED:
        if (nullptr != change->old_val() && nullptr != change->new_val()) {
           O1_LOG("O1 CellOpState : MOVED: %s :after %s ", \
                    change->new_val()->xpath(), \
                    change->old_val()->xpath());
        }
        else if (nullptr != change->new_val()) {
           O1_LOG("O1 CellOpState : MOVED: %s : first\n", \
                   change->new_val()->xpath());
        }
        break;
    }
...
const char *CellOpState::ev_to_str(sr_event_t ev) {
   switch (ev) {
      case SR_EV_CHANGE:
         return "change";
      case SR_EV_DONE:
...
int CellOpState::module_change(sysrepo::S_Session sess, \
                                    const char *module_name, \
                                    const char *xpath, \
                                    sr_event_t event, \
                                    uint32_t request_id, \
                                    void *private_data)
{
   ...
   try {
      O1_LOG("O1 CellOpState : Notification %s\n", ev_to_str(event));
      ...
      while (auto change = sess->get_change_next(it)) {
         ...
         if(nullptr != change->new_val())
         {
            O1_LOG("O1 CellOpState : Parameter value has been \
changed val=%s\n", change->new_val()->val_to_string().c_str());
            ...
            for(it = mCellOpStateMap.begin(); it !=mCellOpStateMap.end(); it++)
            {
               ...
               O1_LOG("O1 CellOpState : created xpath = %s", \
                      xpath.str().c_str());
...
#ifndef ODU_TEST_STUB
                  O1_LOG("O1 CellOpState : Update admin state for \
cellId =%d with admin-state value=%s\n", it->first, val.c_str());
                 if(!updateAdminState(it->first, newVal)) {
                    O1_LOG("O1 CellOpState : Could not change \
parameter value =%s\n", change->new_val()->val_to_string().c_str());
                    return SR_ERR_OK;
                 }
#endif
               ...
               {
                  O1_LOG("O1 CellOpState : Non administrative-state \
...
AdminState CellOpState::adminStateToEnum(string val)
{
   ...
   if( val == "LOCKED")
      ...
      O1_LOG("O1 CellOpState : %s admin state not handled\n", \
              val.c_str());
...
string CellOpState::enumToCellStateString(CellState val)
{
   string ret = "IDLE";
   switch(val)
   {
      ...
      case IDLE:
         ...
         break;
      default :
         O1_LOG("O1 CellOpState : %d cell state not handled\n", val);
...
string CellOpState::enumToOperationalStateString(OpState val)
{
   string ret = "DISABLED";
   switch(val)
   {
      ...
      case ENABLED:
         ...
         break;
      default :
         O1_LOG("O1 CellOpState : %d operational state not handled\n", val);
...
bool CellOpState::updateCellOpState(uint16_t cellId, \
                                    OpState opState, \
                                    CellState cellState)
{
   O1_LOG("\nO1 CellOpState : before setting cellId = %d, opState=%d, \
