...
*
* ****************************************************************/
std::string SliceMeasurementEventStdDef::getMeasPath()
{
   O1_LOG("\n O1 SliceMeasurementEventStdDef cellLocalId :%d",cellParams.cellLocalId);
   std::ostringstream os;
   os<< "/o-ran-sc-du-hello-world:network-function/distributed-unit-functions\
	   [id='"<< ODU_HIGH <<"']/cell[id='"<<cellParams.cellLocalId <<"']\
	   /supported-measurements[performance-measurement-type='user-equipment-average-throughput-downlink']";
   return os.str();
...
bool SliceMeasurementEventStdDef::prepareEventFields(const Message* msg)
{
   ...
   if(JsonHelper::addNodeToObject(stndDefinedFields, "stndDefinedFieldsVersion", STND_DEFINED_FEILD_VERSION) == 0) {
      ret = false;
   }
   else if(JsonHelper::addNodeToObject(stndDefinedFields, "schemaReference", SLICE_SCHEMA) == 0) {
      ret = false;
   }
   ...
   else if(JsonHelper::addNodeToObject(data, "start-time", getISOEventTime().c_str()) == 0) {
      ret = false;
   }
   else if(JsonHelper::addNodeToObject(data, "administrative-state", ADMINISTRATIVE_STATE) == 0) {
      ret = false;
   }
   else if(JsonHelper::addNodeToObject(data, "operational-state", OPERATIONAL_STATE) == 0) {
      ret = false;
   }
   ...
   else if(JsonHelper::addNodeToObject(data, "granularity-period", GRANULARITY_PERIOD) == 0) {
      ret = false;
   }
   ...
   cJSON_AddItemToObject(data, "measurements", measure);

   cJSON* arr = JsonHelper::createNode();
   cJSON_AddItemToArray(measure,arr);
  
   if(JsonHelper::addNodeToObject(arr, MEAS_REF, getMeasPath().c_str()) == 0) {
      ret = false;
   }

   cJSON *value = cJSON_CreateNumber(RANDOM_NUM);
