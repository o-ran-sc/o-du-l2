...
bool HttpClient::readConfigFile()
{
   cJSON *json = JsonHelper::readConfig(SMO_CONFIG);
   if(json == NULL) {
...
bool HttpClient::parseConfig(cJSON *json)
{
   cJSON *rootNode = NULL;
   rootNode = cJSON_GetObjectItem(json, "smoConfig");
   if(rootNode) {
      O1_LOG("O1 VES : Reading smoConfig.json file\n");
      mSmoIp = JsonHelper::getObject(rootNode, "smoV4IpAddress");
      mSmoPort = JsonHelper::getObject(rootNode, "smoPort");
      mSmoUsername = JsonHelper::getObject(rootNode, "username");
      mSmoPassword = JsonHelper::getObject(rootNode, "password");
   }
   else {
...
bool HttpClient::prepareHttpHeader(struct curl_slist *header, CURL *curl)
{
   if(!readConfigFile())
   {
      O1_LOG("O1 VES : Could not read config file\n");
      //return false;
   }
...
bool HttpClient::setUserPassword(CURL *curl)
{
   ...
   char *credentials = 0;
   if((mSmoUsername.c_str()) && (mSmoPassword.c_str())) {
      asprintf(&credentials, "%s:%s", mSmoUsername.c_str(), mSmoPassword.c_str());
      if(credentials == 0) {
...
bool HttpClient::createUrl(CURL *curl)
{
   ...
   if((mSmoIp.c_str()) && (mSmoPort.c_str())) {
      oss <<"https://"<<mSmoIp<<":"<<mSmoPort<<"/eventListener/v7";
      string url = oss.str();
      O1_LOG("O1 VES : URL=%s\n", url.c_str());
...
bool HttpClient::sendVesEvent(const char *sendData) {
   assert(sendData);   
