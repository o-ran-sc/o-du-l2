...
void NrCellDuCb::updateParamas(string parent, string leaf, string val)
{
   if(parent.find("attribute") != string::npos)
   {
      if(leaf == "cellLocalId"){
         ...
         cellParams.administrativeState = administrativeStateToEnum(val.c_str());
   ...
   else if(parent.find("pLMNInfoList") != string::npos)
   {
	 int memberNum = plmnListNum/MAX_PLMN_MEMBER;
         if(leaf == "mcc") 
         {
            memset(cellParams.plmnList[memberNum].mcc, \
	           '\0', MCC_LEN);
            strncpy((char *)cellParams.plmnList[memberNum].mcc, \
			    val.c_str(), MCC_LEN);
	    ...
	    memset(cellParams.plmnList[memberNum].mnc, \
	           '\0', MNC_LEN);
	    strncpy((char *)cellParams.plmnList[memberNum].mnc, \
	            val.c_str(), MNC_LEN);
         ...
         else if(leaf == "sd") 
         {
            memset(cellParams.plmnList[memberNum].sd, \
	           '\0', SD_LEN);
            strncpy(cellParams.plmnList[memberNum].sd, \
                    val.c_str(), SD_LEN);
            O1_LOG("\nO1 NrCellDuCb : cellParams.plmnList[%d].sd[5]  = %c",
...
bool NrCellDuCb::configureCell()
{
   ...
   if( setCellParam(cellParams) != ROK)
   {
      return false;
   }
   return true;
