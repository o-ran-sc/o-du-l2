...
uint8_t duHandleSlotInd(Pst *pst, uint16_t *cellId)
{
   ...
   if(*cellId <=0 || *cellId > MAX_NUM_CELL)
   {
      DU_LOG("\nDU APP : Invalid Cell Id %d in duHandleSlotInd()", *cellId);
      return RFAILED;
   }

   if(duGetCellCb(*cellId, &cellCb) != ROK)
      return RFAILED;

   if((cellCb != NULL) && (cellCb->cellStatus == ACTIVATION_IN_PROGRESS))
   {
      DU_LOG("\nDU APP : 5G-NR Cell %d is UP", *cellId);
      cellCb->cellStatus = ACTIVATED;

#ifdef O1_ENABLE
      DU_LOG("\nDU APP : Raise cell UP alarm for cell id=%d", *cellId);
      raiseCellAlrm(CELL_UP_ALARM_ID, *cellId);
#endif      
   }

   if((pst->selector == ODU_SELECTOR_LWLC) || (pst->selector == ODU_SELECTOR_TC)) 
      DU_FREE_SHRABL_BUF(pst->region, pst->pool, cellId, sizeof(uint16_t));
   return ROK;
}
