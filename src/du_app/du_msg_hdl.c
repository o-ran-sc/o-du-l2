...
uint8_t duSendEgtpTnlMgmtReq(uint8_t action, GtpTnlCfg *ueCbTnlCfg)
{
   uint8_t tnlIdx = 0;
   Pst pst;
   EgtpTnlEvt tnlEvt;
...
uint8_t DuProcRlcUlUserDataTrans(Pst *pst, RlcUlUserDatInfo *ulUserData)
{
   ...
   {
      if(duCb.upTnlCfg[rbIdx])
      {
         if(duCb.upTnlCfg[rbIdx]->drbId == ulUserData->rbId)
         {
            egtpMsg.msgHdr.teId = duCb.upTnlCfg[rbIdx]->tnlCfg[0].teId; /*As we are supporting only 1 tunnel per DRB*/
         }
      }
   }
