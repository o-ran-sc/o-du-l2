...
S16 duBuildRlcCfg(Inst inst)
{
   ...
   genCfg->lmPst.selector  = SELECTOR_LC;
   ...
   kwMngmt.hdr.response.selector   = SELECTOR_LC;
...
S16 duBuildRlcLsapCfg(Ent ent, Inst inst, U8 lsapInst)
{
   ...
   if (ent == ENTRG)
   {
      ...
      lSap->selector    = (inst == RLC_UL_INST) ? SELECTOR_LWLC : SELECTOR_TC;
...
S16 duBuildMacGenCfg()
{
   ...
   rgMngmt.hdr.response.selector   = SELECTOR_LC;
...
S16 duSendSchCfg()
{
   ...
   cfg->genCfg.lmPst.selector  = SELECTOR_LC;
...
uint16_t duHandleSlotInd(Pst *pst, SlotInfo *slotInfo)
{
   ...
   if((pst->selector == SELECTOR_LWLC) || (pst->selector == SELECTOR_TC)) 
