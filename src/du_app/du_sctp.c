...
uint8_t duSctpAssocReq(uint8_t itfType)
{
   ...
   switch(itfType)
   {
      ...
      default:
      {
         DU_LOG("\nERROR  -->  SCTP : Invalid Interface Type");
         break;
      }
   }  
   if(ret != ROK)
...
{
   Pst pst;
   DU_LOG("\nINFO   -->  SCTP : Forwarding received message to duApp");
...
uint8_t sctpNtfyHdlr(CmInetSctpNotification *ntfy, uint8_t *itfState)
{
   ...
   switch(ntfy->header.nType)
   {
      ...
      case CM_INET_SCTP_PARTIAL_DELIVERY_EVENT:
         ...
         break;
      default:
         DU_LOG("\nERROR  -->  SCTP : Invalid sctp notification type\n");
         break;
   }
      ...
      else
      {
         if((((pollParams->flag & CM_INET_SCTP_MSG_NOTIFICATION) != 0)) && ret == ROK)
         {
            if(pollParams->port == f1Params.destPort)
            {
               ...
               DU_LOG("\nINFO   -->  SCTP : AssocId assigned to F1Params from PollParams [%d]\n", f1Params.assocId);
            ...
            else if(pollParams->port == ricParams.destPort)
            {
               ...
               DU_LOG("\nINFO   -->  SCTP : AssocId assigned to ricParams from PollParams [%d]\n", ricParams.assocId);
...
uint8_t sctpSend(Buffer *mBuf, uint8_t itfType)
{
   ...
   {
      DU_LOG("\nINFO   -->  SCTP : sending the message to DuApp");
   ...
   {
      DU_LOG("\nINFO   -->  SCTP : sending the message to ric");
