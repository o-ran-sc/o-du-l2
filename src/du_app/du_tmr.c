...
bool duChkTmr(PTR cb, int16_t tmrEvnt)
{
   switch (tmrEvnt)
   {
      ...
      case EVENT_RIC_SERVICE_UPDATE_TMR:
         {
            if(((E2apDb *)cb)->e2TimersInfo.e2Timers.ricServiceUpdateInfo.timer.tmrEvnt == EVENT_RIC_SERVICE_UPDATE_TMR)
            {
               DU_LOG("\nERROR  -->  DU_APP : duChkTmr: Timer already running for event [%d]", tmrEvnt);
               return TRUE;
            }
      ...
      case EVENT_RIC_SERVICE_UPDATE_TMR:
         {
            ...
            arg.timers = &e2apDb->e2TimersInfo.e2Timers.ricServiceUpdateInfo.timer;
      ...
      case EVENT_RIC_SERVICE_UPDATE_TMR:
         {
            ...
            memcpy(&ricServiceUpdate, &e2apDb->e2TimersInfo.e2Timers.ricServiceUpdateInfo.timer, sizeof(RicServiceUpdateTimer));
            BuildAndSendRicServiceUpdate(ricServiceUpdate.ricService);
            break;
         }
