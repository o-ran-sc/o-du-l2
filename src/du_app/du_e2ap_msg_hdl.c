 ...
 *
 * ****************************************************************/
uint8_t assignRanFuncRevisionId(RanFunction *ranFuncDb)
{
   uint8_t currId = ranFuncDb->revisionCounter;
 ...
 *
 ******************************************************************/
uint8_t fillRicReportStyle(RanFunction *ranFuncDb, struct E2SM_KPM_RANfunction_Description__ric_ReportStyle_List *ricReportStyle, ConfigType action)
{
   ...
   for(styleIdx =0;styleIdx<ricReportStyle->list.count; styleIdx++)
   {
      ...
      if(action == CONFIG_ADD)
         measurmentCount = measInfoList.count - 1; 
      else
...
uint8_t BuildRanFunctionAddOrModifyList(RANfunctions_List_t *ranFunctionsList, ConfigType action)
{
   ...
   for(ranFuncIdx = 0; ranFuncIdx< ranFunctionsList->list.count; ranFuncIdx++)
   {
      ...
      ranFuncItem->ranFunctionRevision = assignRanFuncRevisionId(ranFuncDb);
      ...
      if(fillRicReportStyle(ranFuncDb, ranFuncDefinition->ric_ReportStyle_List, action) != ROK)
      {
...
uint8_t BuildAndSendE2SetupReq()
{
   ...
   do
   {
      ...
      if(BuildRanFunctionAddOrModifyList(&(e2SetupReq->protocolIEs.list.array[arrIdx]->value.choice.RANfunctions_List), CONFIG_ADD)!=ROK)
      {
               ...
               switch(ricServiceAck->protocolIEs.list.array[arrIdx]->id)
               {
                  case ProtocolIE_IDE2_id_RANfunctionsAccepted:
                  {
                     ...
                     if(ranFuncAddedList->list.array)
                     {
                        ...
                        {
                           free(ranFuncAddedList->list.array[arrIdx]);
                        }
...
void procRicServiceUpdateFailure(E2AP_PDU_t *e2apMsg)
{
   ...
   for(arrIdx=0; arrIdx<ricServiceFailure->protocolIEs.list.count; arrIdx++)
   {
      switch(ricServiceFailure->protocolIEs.list.array[arrIdx]->id)
      {
         case ProtocolIE_IDE2_id_TransactionID:
         {
            transId = ricServiceFailure->protocolIEs.list.array[arrIdx]->value.choice.TransactionID;
            if((duCb.e2apDb.e2TransInfo.onGoingTransaction[transId].transactionId == transId) &&\
            (duCb.e2apDb.e2TransInfo.onGoingTransaction[transId].procedureCode == e2apMsg->choice.unsuccessfulOutcome->procedureCode))
              memset(&duCb.e2apDb.e2TransInfo.onGoingTransaction[transId], 0, sizeof(E2TransInfo));
            else
         ...
         case ProtocolIE_IDE2_id_CauseE2:
            {
               ...
               {
                  case CauseE2_PR_ricRequest:  
                     DU_LOG("\nINFO  -->  E2AP : RIC service update failured because of %d ",ricServiceFailure->protocolIEs.list.array[arrIdx]->value.choice.CauseE2.choice.ricRequest);
                     break;
                  case CauseE2_PR_ricService:
                     DU_LOG("\nINFO  -->  E2AP : RIC service update failured because of %d ",ricServiceFailure->protocolIEs.list.array[arrIdx]->value.choice.CauseE2.choice.ricService);
                     break;
                  case CauseE2_PR_e2Node:
                     DU_LOG("\nINFO  -->  E2AP : RIC service update failured because of %d ",ricServiceFailure->protocolIEs.list.array[arrIdx]->value.choice.CauseE2.choice.e2Node);
                     break;
                  case CauseE2_PR_transport:
                     DU_LOG("\nINFO  -->  E2AP : RIC service update failured because of %d ",ricServiceFailure->protocolIEs.list.array[arrIdx]->value.choice.CauseE2.choice.transport);
                     break;
                  case CauseE2_PR_protocol:
                     DU_LOG("\nINFO  -->  E2AP : RIC service update failured because of %d ",ricServiceFailure->protocolIEs.list.array[arrIdx]->value.choice.CauseE2.choice.protocol);
                     break;
                  case CauseE2_PR_misc:
                     DU_LOG("\nINFO  -->  E2AP : RIC service update failured because of %d ",ricServiceFailure->protocolIEs.list.array[arrIdx]->value.choice.CauseE2.choice.misc);
                     break;
               }
               break;
            }
...
uint8_t BuildAndSendRicServiceUpdate()
{
   ...
   do
   {
      ...
      if(BuildRanFunctionAddOrModifyList(&ricServiceUpdate->protocolIEs.list.array[arrIdx]->value.choice.RANfunctions_List, CONFIG_MOD) !=ROK)
      {
      ...
      case E2AP_PDU_PR_initiatingMessage:
         {
            switch(e2apMsg->choice.initiatingMessage->value.present)
            {
               ...
               case ProcedureCodeE2_id_RICserviceQuery:
                  {
