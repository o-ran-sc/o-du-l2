...
uint16_t fillE2SetupReq(E2AP_PDU_t **e2apMsg, E2setupRequest_t **e2SetupReq, uint8_t *idx)
{
   uint8_t elementCnt;
   ...
   if(*e2apMsg != NULLP)
   {
      ...
      (*e2apMsg)->choice.initiatingMessage = NULLP;
...
S16 BuildAndSendE2SetupReq()
{
   ...
   do
   {
      ...
      ret = fillE2SetupReq(&e2apMsg, &e2SetupReq, &idx);
      if(ret != ROK)
      {
   ...
   E2setupRequest_t *e2SetupReq, uint8_t idx)
{
   ...
   if(e2apMsg != NULLP)
   {
      if(e2apMsg->choice.initiatingMessage != NULLP)
      {
         if((e2SetupReq != NULLP) && (e2SetupReq->protocolIEs.list.array != NULLP))
         {
            if(idx == e2SetupReq->protocolIEs.list.count)
            {
