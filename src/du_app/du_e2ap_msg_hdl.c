...
{
   uint8_t eventTriggerIdx, reportStyleIdx, measurementInfoIdx;
         ...
         if(ricReportStyle->list.array)
         {
            for(reportStyleIdx =0;reportStyleIdx<ricReportStyle->list.count; reportStyleIdx++)
            {
               if(ricReportStyle->list.array[reportStyleIdx])
               {
                  ...
                  {
                     for(measurementInfoIdx=0; measurementInfoIdx<ricReportStyle->list.array[reportStyleIdx]->measInfo_Action_List.list.count; \
                           measurementInfoIdx++)
                     {
                        if(ricReportStyle->list.array[reportStyleIdx]->measInfo_Action_List.list.array[measurementInfoIdx])
                        {
                           if(ricReportStyle->list.array[reportStyleIdx]->measInfo_Action_List.list.array[measurementInfoIdx]->measID)
                           {
                              DU_FREE(ricReportStyle->list.array[reportStyleIdx]->measInfo_Action_List.list.array[measurementInfoIdx]->measID, sizeof(long));
                           }
                           if(ricReportStyle->list.array[reportStyleIdx]->measInfo_Action_List.list.array[measurementInfoIdx]->measName.size)
                           {
                              DU_FREE(ricReportStyle->list.array[reportStyleIdx]->measInfo_Action_List.list.array[measurementInfoIdx]->measName.buf,\
                                    ricReportStyle->list.array[reportStyleIdx]->measInfo_Action_List.list.array[measurementInfoIdx]->measName.size);
                           }
                           DU_FREE(ricReportStyle->list.array[reportStyleIdx]->measInfo_Action_List.list.array[measurementInfoIdx],\
...
{
   uint8_t styleIdx, measurementInfoIdx;
   MeasurementInfo_Action_List_t *measurementInfo;
   CmLList  *node;
   ...
   for(styleIdx =0;styleIdx<ricReportStyle->list.count; styleIdx++)
   {
      ...
      CmLListCp measurementInfoList =ranFuncDb->reportStyleList[styleIdx].measurementInfoList;
      ...
      measurementInfo = &ricReportStyle->list.array[styleIdx]->measInfo_Action_List;

      measurementInfo->list.count = measurementInfoList.count; 
      measurementInfo->list.size =  measurementInfoList.count*sizeof(MeasurementInfo_Action_Item_t*);
      DU_ALLOC(measurementInfo->list.array,\
            measurementInfo->list.size);
      if(!measurementInfo->list.array)
      {
         DU_LOG("\nERROR  --> E2AP: Memory allocation failed in function %s at %d",__func__,__LINE__);
         return RFAILED;
      }

      for(measurementInfoIdx=0; measurementInfoIdx<measurementInfo->list.count; measurementInfoIdx++)
      {
         DU_ALLOC(measurementInfo->list.array[measurementInfoIdx],sizeof(MeasurementInfo_Action_Item_t));  
         if(!measurementInfo->list.array[measurementInfoIdx])
         {
            DU_LOG("\nERROR  --> E2AP: Memory allocation failed in function %s at %d",__func__,__LINE__);
            return RFAILED;
         }
         MeasurementInfoForAction *measurementInfoForAction= (MeasurementInfoForAction*)node->node;
         DU_ALLOC(measurementInfo->list.array[measurementInfoIdx]->measID, sizeof(long));
         if(!measurementInfo->list.array[measurementInfoIdx]->measID)
         {
            DU_LOG("\nERROR  --> E2AP: Memory allocation failed in function %s at %d",__func__,__LINE__);
            return RFAILED;
         }
         memcpy(measurementInfo->list.array[measurementInfoIdx]->measID, \
               &measurementInfoForAction->measurementTypeId,sizeof(long));

         measurementInfo->list.array[measurementInfoIdx]->measName.size= strlen(measurementInfoForAction->measurementTypeName);
         DU_ALLOC(measurementInfo->list.array[measurementInfoIdx]->measName.buf,\
               measurementInfo->list.array[measurementInfoIdx]->measName.size);
         if(!measurementInfo->list.array[measurementInfoIdx]->measName.size)
         {
            DU_LOG("\nERROR  --> E2AP: Memory allocation failed in function %s at %d",__func__,__LINE__);
            return RFAILED;
         }
         memcpy(measurementInfo->list.array[measurementInfoIdx]->measName.buf, \
