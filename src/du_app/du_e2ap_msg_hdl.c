 ...
 *
 ******************************************************************/
uint8_t fillRicReportStyle(RanFunction *ranFuncDb, struct E2SM_KPM_RANfunction_Description__ric_ReportStyle_List *ricReportStyle, RanFunctionAction action)
{
...
uint8_t BuildRanFunctionAddOrModifyList(RANfunctions_List_t *ranFunctionsList, RanFunctionAction action)
{
...
uint8_t BuildAndSendE2SetupReq()
{
   ...
   do
   {
      ...
      if(BuildRanFunctionAddOrModifyList(&(e2SetupReq->protocolIEs.list.array[arrIdx]->value.choice.RANfunctions_List), RAN_FUNCTION_ADD)!=ROK)
      {
         ...
         case ProtocolIE_IDE2_id_TimeToWaitE2:
            {
               ...
               if((duChkTmr((PTR)&(duCb.e2apDb), EVENT_RIC_SERVICE_TMR)) == FALSE)
               {
                  duStartTmr((PTR)&(duCb.e2apDb), EVENT_RIC_SERVICE_TMR, timerValue);
               }
               else
               {
                  DU_LOG("\nERROR   -->  E2AP : EVENT_E2_SETUP_TMR timer is already running");
...
uint8_t BuildAndSendRicServiceUpdate()
{
   ...
   do
   {
      ...
      if(BuildRanFunctionAddOrModifyList(&ricServiceUpdate->protocolIEs.list.array[arrIdx]->value.choice.RANfunctions_List, RAN_FUNCTION_MODIFY) !=ROK)
      {
...
{
   CmLList *node;
   uint8_t ranFuncIdx, reportStyleIdx,  measurementInfoLen, measurementInfoIdx=NUM_OF_MEASUREMENT_INFO_SUPPORTED-1;
   MeasurementInfoForAction *measurementInfoForAction;

   for(ranFuncIdx =0; ranFuncIdx<duCb.e2apDb.numOfRanFunction; ranFuncIdx++)
   {
      duCb.e2apDb.ranFunction[ranFuncIdx].revisionCounter = 1;
