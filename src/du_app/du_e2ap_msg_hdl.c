 ...
 * @details
 *
 *    Function : freeE2smKpmRanFunctionDescription
 *
...
void freeE2smKpmRanFunctionDescription(E2SM_KPM_RANfunction_Description_t *ranFunctionDefinition)
{
   uint8_t eventTriggerStyleIdx, eventReportStyleIdx, MeasurementInfoIdx;

   if(ranFunctionDefinition)
   {
      /* Free RAN function Name */     
      DU_FREE(ranFunctionDefinition->ranFunction_Name.ranFunction_ShortName.buf,  ranFunctionDefinition->ranFunction_Name.ranFunction_ShortName.size);
      DU_FREE(ranFunctionDefinition->ranFunction_Name.ranFunction_E2SM_OID.buf, ranFunctionDefinition->ranFunction_Name.ranFunction_E2SM_OID.size);
      DU_FREE(ranFunctionDefinition->ranFunction_Name.ranFunction_Description.buf, ranFunctionDefinition->ranFunction_Name.ranFunction_Description.size);
      ...
      if(ranFunctionDefinition->ric_EventTriggerStyle_List)
      {
         if(ranFunctionDefinition->ric_EventTriggerStyle_List->list.array)
         {
            for(eventTriggerStyleIdx =0;eventTriggerStyleIdx<ranFunctionDefinition->ric_EventTriggerStyle_List->list.count; eventTriggerStyleIdx++)
            {
               ...
               {
                  DU_FREE(ranFunctionDefinition->ric_EventTriggerStyle_List->list.array[eventTriggerStyleIdx]->ric_EventTriggerStyle_Name.buf,\
                        ranFunctionDefinition->ric_EventTriggerStyle_List->list.array[eventTriggerStyleIdx]->ric_EventTriggerStyle_Name.size);
               }
               DU_FREE(ranFunctionDefinition->ric_EventTriggerStyle_List->list.array[eventTriggerStyleIdx], sizeof(RIC_EventTriggerStyle_Item_t ));
            }
            DU_FREE(ranFunctionDefinition->ric_EventTriggerStyle_List->list.array, ranFunctionDefinition->ric_EventTriggerStyle_List->list.size)
         }
         DU_FREE(ranFunctionDefinition->ric_EventTriggerStyle_List, sizeof(struct E2SM_KPM_RANfunction_Description__ric_EventTriggerStyle_List));
      }
         ...
         if(ranFunctionDefinition->ric_ReportStyle_List->list.array)
         {
            for(eventReportStyleIdx =0;eventReportStyleIdx<ranFunctionDefinition->ric_ReportStyle_List->list.count; eventReportStyleIdx++)
            {
               if(ranFunctionDefinition->ric_ReportStyle_List->list.array[eventReportStyleIdx])
               {
                  ...
                  {
                     DU_FREE(ranFunctionDefinition->ric_ReportStyle_List->list.array[eventReportStyleIdx]->ric_ReportStyle_Name.buf,\
                  ...
                  if(ranFunctionDefinition->ric_ReportStyle_List->list.array[eventReportStyleIdx]->measInfo_Action_List.list.array)
                  {
                           ...
                           MeasurementInfoIdx++)
                     {
                        if(ranFunctionDefinition->ric_ReportStyle_List->list.array[eventReportStyleIdx]->measInfo_Action_List.list.array[MeasurementInfoIdx])
                        {
                           ...
                           {
                              DU_FREE(ranFunctionDefinition->ric_ReportStyle_List->list.array[eventReportStyleIdx]->measInfo_Action_List.list.array[MeasurementInfoIdx]->measID, sizeof(long));
                           }
                           ...
                           {
                              DU_FREE(ranFunctionDefinition->ric_ReportStyle_List->list.array[eventReportStyleIdx]->measInfo_Action_List.list.array[MeasurementInfoIdx]->measName.buf,\
                           ...
                           DU_FREE(ranFunctionDefinition->ric_ReportStyle_List->list.array[eventReportStyleIdx]->measInfo_Action_List.list.array[MeasurementInfoIdx],\
                                 sizeof(MeasurementInfo_Action_Item_t)); 
                        }
                     }
                     DU_FREE(ranFunctionDefinition->ric_ReportStyle_List->list.array[eventReportStyleIdx]->measInfo_Action_List.list.array,\
                           ranFunctionDefinition->ric_ReportStyle_List->list.array[eventReportStyleIdx]->measInfo_Action_List.list.size);
                  }
                  DU_FREE(ranFunctionDefinition->ric_ReportStyle_List->list.array[eventReportStyleIdx], sizeof(RIC_ReportStyle_Item_t));
               }
            }
            DU_FREE(ranFunctionDefinition->ric_ReportStyle_List->list.array, ranFunctionDefinition->ric_ReportStyle_List->list.size);
         }
         DU_FREE(ranFunctionDefinition->ric_ReportStyle_List, sizeof(struct E2SM_KPM_RANfunction_Description__ric_ReportStyle_List));
      }
...
uint8_t BuildRanFunctionAddList(RANfunctions_List_t *ranFunctionsList)
{
   ...
   freeE2smKpmRanFunctionDescription(ranFunctionDefinition);
   return ROK;
}
               ...
               if(e2SetupReq->protocolIEs.list.array[arrIdx] != NULLP)
               {
                  switch(e2SetupReq->protocolIEs.list.array[arrIdx]->id)
                  {
                     ...
                     case ProtocolIE_IDE2_id_RANfunctionsAdded:
                     {
                        ...
                        if(ranFunctionsList->list.count)
                        {  
                           for(ranFuncAddListIdx= 0; ranFuncAddListIdx< ranFunctionsList->list.count; ranFuncAddListIdx++)
                           {
                              ...
                              {
                                 ranFuncAddItemIe = (RANfunction_ItemIEs_t *) ranFunctionsList->list.array[ranFuncAddListIdx];
                                 ranFunAddItem = &ranFuncAddItemIe->value.choice.RANfunction_Item;
                                 DU_FREE(ranFunAddItem->ranFunctionOID.buf, ranFunAddItem->ranFunctionOID.size);
                                 DU_FREE(ranFunAddItem->ranFunctionDefinition.buf, ranFunAddItem->ranFunctionDefinition.size);
