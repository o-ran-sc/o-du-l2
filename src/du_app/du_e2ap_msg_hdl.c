 ...
 * @details
 *
 *    Function : reeE2NodeConfigUpdate 
 *
 *    Functionality:
...
uint8_t BuildAndSendE2ResetRequest(E2CauseType failureType, E2Cause failureCause)
{
   ...
   do
   {
      ...
      resetReq->protocolIEs.list.array[ieIdx]->value.choice.TransactionID = assignTranscationId();
      ...
      case E2AP_PDU_PR_successfulOutcome:
         {
            switch(e2apMsg->choice.successfulOutcome->value.present)
            {
               ...
               case SuccessfulOutcomeE2__value_PR_ResetResponseE2:
                  {
                     DU_LOG("\nDEBUG   -->  E2AP : E2 Reset Response message recevied");
                     break;
                  }
               default:
                  {
               ...
               case InitiatingMessageE2__value_PR_RICsubscriptionRequest: 
                  {
                     if(procRicSubsReq(e2apMsg) == ROK)
                     {
                        ...
                        ResetE2Request(E2_NODE_INITIATED, E2_MISCELLANEOUS, E2_MISCELLANEOUS_CAUSE_UNSPECIFIED);
                     }
                     break;
                  }
               default:
