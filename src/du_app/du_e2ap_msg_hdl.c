...
uint8_t BuildAndSendE2ConnectionUpdateAcknowledge(uint16_t transId,  E2ConnectionList connectionInfoList)
{
   ...
   do
   {
      ...
      elementCnt = 1;
      if(connectionInfoList.modifyE2ConnectionCount)
         elementCnt++;
      if(connectionInfoList.failedToModifyE2ConnectionCount)
         elementCnt++;
      ...
      if(connectionInfoList.modifyE2ConnectionCount)
      {
         ...
         if(BuildE2ConnectionUpdateList(&e2ConnectionUpdateAcknowledge->protocolIEs.list.array[ieIdx]->value.choice.E2connectionUpdate_List, \
                  connectionInfoList.modifyE2ConnectionCount, connectionInfoList.modifyE2Node) != ROK)
         {
      ...
      if(connectionInfoList.failedToModifyE2ConnectionCount)
      {
         ...
         e2ConnectionUpdateAcknowledge->protocolIEs.list.array[ieIdx]->value.present = E2connectionUpdateAck_IEs__value_PR_E2connectionUpdate_List;
         if(BuildE2ConnectionUpdateList(&e2ConnectionUpdateAcknowledge->protocolIEs.list.array[ieIdx]->value.choice.E2connectionUpdate_List, \
               connectionInfoList.failedToModifyE2ConnectionCount, connectionInfoList.failedToModifyE2Node) != ROK) 
         {
...
void handle2ConnectionModification(E2connectionUpdate_List_t *connectionUpdateList, E2ConnectionList *connectionInfoList)
{
   ...
   if(connectionUpdateList->list.array)
   {
      for(arrIdx = 0; arrIdx < connectionUpdateList->list.count; arrIdx++)
      {
         ...
         if(infoFound == true)
         {
            count =connectionInfoList->modifyE2ConnectionCount;
            connectionInfoList->modifyE2Node[count].ipV4Addr =  duCb.e2apDb.tnlAssoc[idx].destIpAddress.ipV4Addr;
            connectionInfoList->modifyE2Node[count].usage = duCb.e2apDb.tnlAssoc[idx].usage;
            connectionInfoList->modifyE2ConnectionCount++;
         }
         else
         {
            count = connectionInfoList->failedToModifyE2ConnectionCount;
            connectionInfoList->failedToModifyE2Node[count].ipV4Addr =  ipAddress;
            connectionInfoList->failedToModifyE2Node[count].usage = connectionModifyItem->value.choice.E2connectionUpdate_Item.tnlUsage;
            connectionInfoList->failedToModifyE2ConnectionCount++;
         }
      }
...
void procE2ConnectionUpdate(E2AP_PDU_t  *e2apMsg)
{
   ...
   TNLinformation_t *tnlInformation = NULLP;
   TNLusage_t  *tnlUsage=NULLP;
   E2connectionUpdate_ItemIEs_t *modifyConnection=NULLP;
   E2connectionUpdate_List_t *connectionToBeModifyList = NULLP;
