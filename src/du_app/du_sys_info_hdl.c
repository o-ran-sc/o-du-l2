   ...
   while(1)
	{
       ...
       ret = BuildMib(bcchMsg->message.choice.mib);
		 if(ret != ROK)
		 {
		     break;
		 }
...
int BuildSib1Msg()
{
   ...
   bool checkvar=false;
	...
	do
	{
       ...
       ret =  BuildPlmnList(cellAccessInfo);
       if(ret != ROK)
		 {
		      break;
		 }
       ...
       ret = BuildSiSchedInfoList(&sib1Msg->si_SchedulingInfo->schedulingInfoList);
       if(ret != ROK)
		 {
		     break;
		 }
       ...
       ret =  BuildServCellCfgCommonSib(sib1Msg->servingCellConfigCommon);
       if(ret != ROK)
		 {
		     break;
		 }
