...
void FillSlotConfig()
{
   ...
   for(slot  =0; slot < MAX_NO_OF_SLOT; slot++)
   {
      ...
      {
         duCfgParam.macCellCfg.tddCfg.slotCfg[slot] = DL_SLOT;  
      }
      else if(slot == NUM_DL_SLOTS)
      {
         duCfgParam.macCellCfg.tddCfg.slotCfg[slot] = GUARD_SLOT;
	 for(symbol = 0; symbol < NUM_DL_SYMBOLS ; symbol++)
	 {
	    duCfgParam.macCellCfg.tddCfg.symbolCfg[symbol] = DL_SLOT;
	 }
         duCfgParam.macCellCfg.tddCfg.symbolCfg[symbol] = GUARD_SLOT;
	 symbol++;
	 duCfgParam.macCellCfg.tddCfg.symbolCfg[symbol] = UL_SLOT;
      }
      else
      {
         duCfgParam.macCellCfg.tddCfg.slotCfg[slot] = UL_SLOT;
      }
   }
}
