...
uint8_t readMacCfg(RrmPolicy *rrmPolicy)
{
...
uint8_t readCfg()
{
   ...
   memset(&rrmPolicy, 0, sizeof(RrmPolicy));
   rrmPolicy.rsrcType = RSRC_PRB;
   rrmPolicy.policyMaxRatio = MAX_RATIO;
   rrmPolicy.policyMinRatio = MIN_RATIO;
   rrmPolicy.policyDedicatedRatio = DEDICATED_RATIO;
   
   memset(&rrmPolicy.memberList, 0, sizeof(PolicyMemberList));
