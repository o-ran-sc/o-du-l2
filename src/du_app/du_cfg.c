...
uint8_t readCfg(F1Snsaai snsaai[], ResourceType rsrcType, PolicyMemberList memberList,\
RrmPolicyRatio policyRatio)
{
   ...
   { 
      ...
      for(srvdPlmnIdx=0;srvdPlmnIdx<MAX_PLMN;srvdPlmnIdx++)
      {
         ...
         memset(&duCfgParam.srvdCellLst[srvdCellIdx].duCellInfo.cellInfo.srvdPlmn[srvdPlmnIdx].extPlmn,\
         0, sizeof(Plmn));
         duCfgParam.srvdCellLst[srvdCellIdx].duCellInfo.cellInfo.srvdPlmn[srvdPlmnIdx].extPlmn.mcc[0] =\
         PLMN_MCC0;
         duCfgParam.srvdCellLst[srvdCellIdx].duCellInfo.cellInfo.srvdPlmn[srvdPlmnIdx].extPlmn.mcc[1] =\
         PLMN_MCC1;
         duCfgParam.srvdCellLst[srvdCellIdx].duCellInfo.cellInfo.srvdPlmn[srvdPlmnIdx].extPlmn.mcc[2] =\
         PLMN_MCC2;
         duCfgParam.srvdCellLst[srvdCellIdx].duCellInfo.cellInfo.srvdPlmn[srvdPlmnIdx].extPlmn.mnc[0] =\
         PLMN_MNC0;
         duCfgParam.srvdCellLst[srvdCellIdx].duCellInfo.cellInfo.srvdPlmn[srvdPlmnIdx].extPlmn.mnc[1] = \
         PLMN_MNC1;
      } 
      /* List of Supporting Slices */
      for(srvdPlmnIdx=0;srvdPlmnIdx<MAX_PLMN;srvdPlmnIdx++)
      {
         duCfgParam.srvdCellLst[srvdCellIdx].duCellInfo.cellInfo.srvdPlmn[srvdPlmnIdx].taiSliceSuppLst.pres =\
         true;
         duCfgParam.srvdCellLst[srvdCellIdx].duCellInfo.cellInfo.srvdPlmn[srvdPlmnIdx].taiSliceSuppLst.\
         numSupportedSlices = NUM_OF_SUPPORTED_SLICE;

         memset(&duCfgParam.srvdCellLst[srvdCellIdx].duCellInfo.cellInfo.srvdPlmn[srvdPlmnIdx].taiSliceSuppLst.\
         snssai, 0, sizeof(F1Snsaai));
         for(sliceIdx=0; sliceIdx<NUM_OF_SUPPORTED_SLICE; sliceIdx++)
         {
            DU_ALLOC(duCfgParam.srvdCellLst[srvdCellIdx].duCellInfo.cellInfo.srvdPlmn[srvdPlmnIdx].taiSliceSuppLst.\
            snssai[sliceIdx], sizeof(F1Snsaai));
            /* Storing SNSSAI-SST value*/
            duCfgParam.srvdCellLst[srvdCellIdx].duCellInfo.cellInfo.srvdPlmn[srvdPlmnIdx].taiSliceSuppLst.\
            snssai[sliceIdx]->sst = snsaai[sliceIdx].sst;
            
            /* Storing SNSSAI-SD value*/
            duCfgParam.srvdCellLst[srvdCellIdx].duCellInfo.cellInfo.srvdPlmn[srvdPlmnIdx].taiSliceSuppLst.\
            snssai[sliceIdx]->sd[0]  = snsaai[sliceIdx].sd[0];
            
            duCfgParam.srvdCellLst[srvdCellIdx].duCellInfo.cellInfo.srvdPlmn[srvdPlmnIdx].taiSliceSuppLst.\
            snssai[sliceIdx]->sd[1]  = snsaai[sliceIdx].sd[1];

            duCfgParam.srvdCellLst[srvdCellIdx].duCellInfo.cellInfo.srvdPlmn[srvdPlmnIdx].taiSliceSuppLst.\
            snssai[sliceIdx]->sd[2]  = snsaai[sliceIdx].sd[2];
           
            /* Checking rrmPolicy Slice list available or not */
            if(snsaai[sliceIdx].sst == memberList.snsaai.sst &&\
            snsaai[sliceIdx].sd[0] == memberList.snsaai.sd[0] && snsaai[sliceIdx].sd[1] == memberList.snsaai.sd[1] &&\
            snsaai[sliceIdx].sd[2] == memberList.snsaai.sd[2])
            {
               duCfgParam.srvdCellLst[srvdCellIdx].duCellInfo.cellInfo.srvdPlmn[srvdPlmnIdx].rrmPolicy.\
               present = true;
               duCfgParam.srvdCellLst[srvdCellIdx].duCellInfo.cellInfo.srvdPlmn[srvdPlmnIdx].rrmPolicy.rsrcType\
               = rsrcType; 
               duCfgParam.srvdCellLst[srvdCellIdx].duCellInfo.cellInfo.srvdPlmn[srvdPlmnIdx].rrmPolicy.\
               memberList.snsaai = memberList.snsaai;
               duCfgParam.srvdCellLst[srvdCellIdx].duCellInfo.cellInfo.srvdPlmn[srvdPlmnIdx].rrmPolicy.\
               rrmPolicyRatio = policyRatio;
            }
         }
...
#if 0
      duCfgParam.srvdCellLst[srvdCellIdx].duCellInfo.f1Mode.mode.tdd.nrTxBw.nrScs = SCS_15;
      duCfgParam.srvdCellLst[srvdCellIdx].duCellInfo.f1Mode.mode.tdd.nrTxBw.nrb = NRB_106;
#endif
      ...
      for(brdcstPlmnIdx=0;brdcstPlmnIdx<MAXBPLMNNRMINUS1;brdcstPlmnIdx++)
      { 
...
uint8_t duReadCfg()
{
   ...
   /* Added these below variable for local testing, once we will receive the
    * configuration we can remove these variable */
   F1Snsaai snsaai[2] = {{1,{2,3,4}},{5,{6,7,8}}};
   ResourceType rsrcType = PRB;
   PolicyMemberList membrList = {{0},{5,{6,7,8}}};
   RrmPolicyRatio rrmPolicyRatio = {10,20,30};
   
   memset(&duCfgParam, 0, sizeof(DuCfgParams));
   //Read configs into duCfgParams
   if(readCfg(snsaai, rsrcType, membrList, rrmPolicyRatio) != ROK)
   {
