...
uint8_t cpyRrmPolicyInDuCfgParams(RrmPolicy rrmPolicy[], uint8_t policyNum, uint8_t memberList, CopyOfRecvdSliceCfg *tempSliceCfg)
{
   ...
   if(policyNum)
   {
      tempSliceCfg->totalSliceCfg  = policyNum;
      DU_ALLOC(tempSliceCfg->rrmPolicy, tempSliceCfg->totalSliceCfg * sizeof(RrmPolicy));
      ...
      for(policyIdx = 0; policyIdx<tempSliceCfg->totalSliceCfg; policyIdx++)
      {
         ...
         if(memberList)
         {
            tempSliceCfg->rrmPolicy[policyIdx]->numMemberList = memberList;  
            DU_ALLOC(tempSliceCfg->rrmPolicy[policyIdx]->memberList, memberList * sizeof(PolicyMemberList))
