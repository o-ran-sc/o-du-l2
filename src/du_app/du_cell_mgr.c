...
uint8_t duHandleCellUpInd(Pst *pst, OduCellId *cellId)
{
   ...
   /*Note: For local testing purpose, when O1 is not configuring the RRM policy*/
   RrmPolicy *rrmPolicy;
...
#else
      /*FOr Local tesitng when O1 interface is absent*/
      BuildAndSendSliceConfigReq(&rrmPolicy,1,1);
#endif
