...
* 
******************************************************************************/
uint8_t handlePagingReqToMac(DuCellCb *cellCb)
{
   uint16_t pf = 0;
   ...
   if(handlePageInfoLL(pf, NULLD, &(pagInfoLLFromPF->pagInfoList), TRAVERSE_ALL) != ROK)
   {
      DU_LOG("\nERROR  --> DU APP: Failed in Building Paging RRC PDU");
      return RFAILED;
   }
