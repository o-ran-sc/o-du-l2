...
void printPageList(CmHashListCp *pagingInfoMap)
{
   ...
   do
   {
      ...
      if(ret == ROK)
      {
         DU_LOG("\nList for PF:%d",pagInfoLLFromPF->pf);
...
uint8_t insertPagingRecord(DuCellCb* cellCb, DuPagingMsg *rcvdF1apPagingParam, uint16_t iterations)
{
   ...
   printPageList(&(cellCb->pagingInfoMap));
    ...
    if(iterations == 0)
    {
    rcvdF1apPagingParam->pagingFrame = 256;
    rcvdF1apPagingParam->i_s = 0;
    rcvdF1apPagingParam->pagingFrame += (test % (MAX_PAGING_UE_RECORDS+3));
    if(test % 6 == 0)
    {
      rcvdF1apPagingParam->i_s += (test % 4);
    }
    DU_LOG("\nSANGEETA : test:%d, i_s:%d, pf:%d,ueId:%d",test, rcvdF1apPagingParam->i_s,rcvdF1apPagingParam->pagingFrame, rcvdF1apPagingParam->ueId);
    test++;
    }
    maxIterations = 2;
    #endif
