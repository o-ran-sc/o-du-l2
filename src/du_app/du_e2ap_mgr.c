...
uint8_t e2ProcStatsModificationRsp(MacStatsModificationRsp *statsModificationRsp)
{
   ...
   if(fetchSubsInfoFromSubsId(statsModificationRsp->subscriptionId, &ranFuncDb, &ricSubscriptionNode, &ricSubscriptionInfo) != ROK)
   {
    ...
    for(idx=0; idx<statsModificationRsp->numGrpRejected; idx++)
    {
       ...
       {
          deleteActionSequence(actionNode);
          pendingSubsModRsp->modActionStats.rejectedActionList[pendingSubsModRsp->modActionStats.numOfRejectedActions].id = actionId;
          if(statsModificationRsp->statsGrpRejectedList[idx].cause ==STATS_ID_NOT_FOUND)
          {
             pendingSubsModRsp->modActionStats.rejectedActionList[pendingSubsModRsp->modActionStats.numOfRejectedActions].failureCause.causeType = E2_RIC_REQUEST;
             pendingSubsModRsp->modActionStats.rejectedActionList[pendingSubsModRsp->modActionStats.numOfRejectedActions].failureCause.cause =E2_ACTION_NOT_SUPPORTED;
          }
