...
uint8_t updateE2NodeComponent(InterfaceType interfaceType, uint8_t bufSize, char *bufString)
{
   E2NodeComponent *e2NodeComponentInfo= NULLP;
   CmLList  *node = NULLP;
...
uint8_t deleteE2NodeComponent(InterfaceType interfaceType)
{
   E2NodeComponent *e2NodeComponentInfo= NULLP;
   CmLList  *node = NULLP;
   ...
   DU_ALLOC(e2NodeComponentInfo->deleteConfiguration, sizeof(bool));
...
uint8_t fillE2NodeComponentReqInfo(InterfaceType interfaceType, uint8_t action, uint8_t bufSize, char *bufString)
{
   switch(action)
   {
      ...
      case E2_NODE_COMPONENT_UPDATE: 
         {
            if(updateE2NodeComponent(interfaceType, bufSize, bufString) != ROK)
            {
      ...
      case E2_NODE_COMPONENT_DEL:
         {
            if(deleteE2NodeComponent(interfaceType) != ROK)
            {
