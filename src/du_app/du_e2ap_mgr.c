...
uint8_t fillE2NodeComponentRspInfo(InterfaceType interfaceType, uint8_t action, uint8_t bufSize, char *bufString)
{
   ...
   e2NodeComponentInfo = fetchE2NodeComponentInfo(interfaceType, &node);
...
uint8_t addE2NodeComponent(InterfaceType interfaceType, uint8_t bufSize, char *bufString)
{
   E2NodeComponent *e2NodeComponentInfo= NULLP;
   CmLList  *node = NULLP;
   ...
   memcpy(e2NodeComponentInfo->addConfiguration->componentRequestPart, bufString, e2NodeComponentInfo->addConfiguration->reqBufSize);
...
/*******************************************************************
 *
 * @brief update/delete E2 node component req info
 *
 * @details
 *
 *    Function : modifyE2NodeComponent 
 *
 * Functionality: update/delete E2 node component req info 
 *
 * @parameter
 *       Type of interface 
 *       Component action type
...
uint8_t modifyE2NodeComponent(InterfaceType interfaceType, uint8_t action, uint8_t bufSize, char *bufString)
{
   E2NodeConfig **configInfo=NULLP;
   E2NodeComponent *e2NodeComponentInfo= NULLP;
   CmLList  *node = NULLP;
   ...
   switch(action)
   {
      ...
      case E2_NODE_COMPONENT_DEL:
         {
            DU_ALLOC(e2NodeComponentInfo->deleteConfiguration, sizeof(E2NodeConfig));
            ...
            configInfo = &e2NodeComponentInfo->deleteConfiguration;    
            break;
         }
   }
   
   (*configInfo)->reqBufSize = bufSize;

   DU_ALLOC((*configInfo)->componentRequestPart, bufSize);
   if((*configInfo)->componentRequestPart == NULLP)
   {
      DU_LOG("\nERROR  -->  E2AP : Memory allocation failed in %s at %d",__func__,__LINE__);
      if(action == E2_NODE_COMPONENT_DEL)
      {
         DU_FREE(e2NodeComponentInfo->deleteConfiguration, sizeof(E2NodeConfig));
      }
      else
      {
   ...
   memcpy((*configInfo)->componentRequestPart, bufString, (*configInfo)->reqBufSize);
   return ROK;
...
uint8_t fillE2NodeComponentReqInfo(InterfaceType interfaceType, uint8_t action, uint8_t bufSize, char *bufString)
{
   switch(action)
   {
      case E2_NODE_COMPONENT_ADD:
         {
            if(addE2NodeComponent(interfaceType, bufSize, bufString) != ROK)
            {
      ...
      case E2_NODE_COMPONENT_DEL: 
         {
            if(modifyE2NodeComponent(interfaceType, action, bufSize, bufString) != ROK)
            {

               DU_LOG("\nERROR  -->  E2AP : Failed to %d e2 node component",action);
               return RFAILED;
            }
            break;
         }
