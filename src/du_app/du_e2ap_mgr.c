...
uint8_t rejectAllStatsGroup(RanFunction *ranFuncDb, CmLList *ricSubscriptionNode, MacStatsRsp *statsRsp)
{
   ...
   deleteRicSubscriptionNode((RicSubscription *)ricSubscriptionNode->node);
...
/******************************************************************
 *
 * @brief Delete measured Value 
 *
 * @details
 *
 *    Function : deleteMeasuredValue
 *
 *    Functionality: Delete measured Value 
 *
 ...
 *
 * ****************************************************************/
void deleteMeasuredValue(CmLListCp *measuredValueList,  CmLList *measValNode)
{
...
void deleteMeasuredValueList(CmLListCp *measuredValueList)
{
   ...
   while(measValNode)
   {
      deleteMeasuredValue(measuredValueList, measValNode);
   }
}
 ...
 * @details
 *
 *    Function : deleteRicSubscriptionAction
 *
 ...
 *
 * ****************************************************************/
void deleteRicSubscriptionAction(ActionInfo *action)
{
...
void deleteRicSubscriptionNode(RicSubscription *ricSubscriptionInfo)
{
   ...
   for(actionIdx = 0; actionIdx < MAX_RIC_ACTION; actionIdx++)
   {
      ...
      {
         deleteRicSubscriptionAction(&ricSubscriptionInfo->actionSequence[actionIdx]);
      }
   }
