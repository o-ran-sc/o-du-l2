...
uint8_t updateRrmPolicy(RrmPolicyList rrmPolicy[], uint8_t policyNum, \
		     uint8_t memberList)
{
   DU_LOG("\nINFO   -->  DU_APP : DU APP RRM number of policy %d, \
	  with memberList - %d", policyNum, memberList);
   for(uint8_t i=0; i<policyNum ; i++)
   {
      ...
      for(uint8_t j=0; j<memberList ; j++)
      {
