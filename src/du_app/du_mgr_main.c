...
#endif
#ifdef UE_SIM_TEST
uint8_t fapiClActvTsk(Pst *, Buffer *);
uint8_t fapiClActvInit(Ent, Inst, Region, Reason);
#endif
 ...
 *
 *    Functionality:
 *       - Registers and attaches TAPA tasks for FAPI CL's slot
 *       indication generator
 *
 * @params[in] system task ID
 * @return ROK     - success
...
uint8_t fapiClInit(SSTskId sysTskId)
{
   ...
   DU_LOG("\nINFO   -->  DU_APP : FAPI CL slot indication TAPA task created and registered to %d sys task",
         sysTskId);
   return ROK;
}
#endif
...
uint8_t commonInit()
{
   ...
   SSTskId du_app_stsk, egtp_stsk, sctp_stsk, rlc_ul_stsk, rlc_mac_cl_stsk, lwr_mac_stsk, phy_stub_slot_ind_stsk, fapi_cl_slot_ind_stsk;
...
#ifdef UE_SIM_TEST
   if(fapiClInit(fapi_cl_slot_ind_stsk) != ROK)
   {
