...
S16 duProcRlcUlUeCfgRsp(Pst *pst, SuId suId, CkwCfgCfmInfo *rlcCfgRsp)
{
   uint8_t ret = ROK;
   uint8_t idx;
   ...
   if(rlcCfgRsp)
   {
      for(idx = 0; idx < rlcCfgRsp->numEnt; idx++)
      {
         ...
         {
            DU_LOG("\nDU_APP: RLC UE Create Response : SUCCESS [UE IDX:%d, CELL ID:%d]", rlcCfgRsp->ueId, rlcCfgRsp->cellId);
         }
         else
         {
            DU_LOG("\nDU_APP: RLC UE Create Response : FAILED [UE IDX:%d, CELL ID:%d, REASON :%d]",\
	       rlcCfgRsp->ueId, rlcCfgRsp->cellId, rlcCfgRsp->entCfgCfm[idx].status.reason);
            ret = RFAILED;
         }
      }
   }
