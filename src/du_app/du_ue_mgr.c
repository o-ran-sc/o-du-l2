 ...
 * @details
 *
 *    Function : getMacLcCfg
 *
 *    Functionality: Function to copy the Lc cfg from ueSetupReqDb
 *
 *
 *****************************************************************/
uint8_t getMacLcCfg(LcCfg *ueCfg, LcCfg *ueSetReqDb)
{
   uint8_t ret = ROK;
   
   ueCfg->lcId = ueSetReqDb->lcId;
   ...
   if(ueSetReqDb->drbQos)
   {
      if(!ueCfg->drbQos)
      {
         DU_ALLOC_SHRABL_BUF(ueCfg->drbQos, sizeof(DrbQosInfo));
         if(!ueCfg->drbQos)
         {
            DU_LOG("\nDU APP: Memory Alloc failed at drQos at getMacLcCfg()");
            ret = RFAILED;
         }
      }
      if(!ret)
      {
         memcpy(ueCfg->drbQos, ueSetReqDb->drbQos, sizeof(DrbQosInfo));
      }
   }
   else
   {
   ...
   if(ueSetReqDb->snssai && ret == ROK)
   {
      if(!ueCfg->snssai)
      {
         DU_ALLOC_SHRABL_BUF(ueCfg->snssai, sizeof(Snssai));
         if(!ueCfg->snssai)
         {
            DU_LOG("\nDU APP: Memory Alloc failed at snnsai at getMacLcCfg()");
            ret = RFAILED;
         }
      }
      if(!ret)
      {
         memcpy(ueCfg->snssai, ueSetReqDb->snssai, sizeof(Snssai));
      }
   }
   else
   {
      ...
      if(ueCfg->drbQos)
      {
         DU_FREE_SHRABL_BUF(DU_APP_MEM_REGION, DU_POOL, ueCfg->drbQos, sizeof(DrbQosInfo));
         ueCfg->drbQos = NULLP;
      }
   }
   memcpy(&ueCfg->ulLcCfg, &ueSetReqDb->ulLcCfg, sizeof(UlLcCfg));
   memcpy(&ueCfg->dlLcCfg, &ueSetReqDb->dlLcCfg, sizeof(DlLcCfg));
   return ret;
   ...
   MacUeCfg *macUeCfg, DuUeCfg *duUeCfg, F1UeSetupReq *f1UeSetReqDb)
{
   ...
   if(f1UeSetReqDb)
   {
      ...
      ret = BuildMacCellCfg(macUeCfg, f1UeSetReqDb->cellGrpCfg);
      if(!ret)
      {
   ...
   {  
	 ...
	 duUeCfg->numMacLcs++;
      ...
      else
      {
	    ...
	    ret = getMacLcCfg(&macUeCfg->lcCfgToModList[macUeCfg->numLcsToMod], &f1UeSetReqDb->macLcCfgToAddMod[lcIdx]);
	    if(!ret)
	    {
               ret = getMacLcCfg(&duUeCfg->macLcCfg[lcIdx], &f1UeSetReqDb->macLcCfgToAddMod[lcIdx]);
         ...
         else
         {
            ...
            ret = getMacLcCfg(&macUeCfg->lcCfgToAddList[macUeCfg->numLcsToAdd], \
               &f1UeSetReqDb->macLcCfgToAddMod[lcIdx]);
	    if(!ret)
	    {
              ret = getMacLcCfg(&duUeCfg->macLcCfg[duUeCfg->numMacLcs], \
                 &f1UeSetReqDb->macLcCfgToAddMod[lcIdx]);
               macUeCfg->numLcsToAdd++;
	       duUeCfg->numMacLcs++;
	    }
   ...
   RlcUeCfg *rlcUeCfg, DuUeCfg *duUeCfg, F1UeSetupReq *f1UeSetReqDb)
{
   ...
   for(lcIdx = 0; lcIdx < numLcs; lcIdx++)
   {
      if(!ueDbFound)
      {
         BuildRlcLcCfg(SRB_ID_1, SRB_ID_1, RB_TYPE_SRB, RLC_AM, NULL, &rlcUeCfg->rlcBearerCfgToAdd[lcIdx]);
