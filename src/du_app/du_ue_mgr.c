...
uint8_t duUpdateDuUeCbCfg(uint8_t cellId, uint8_t ueIdx, DuUeCb *ueCb, UeContextSetupDb *f1UeDb)
{
   uint8_t ret = ROK, crnti = 0;
...
uint8_t duProcUeContextSetupRsp(uint8_t ueIdx, uint8_t cellId)
{
   ...
   {
      DU_LOG("\nDU_APP: Updating DuUeCb\n");
      ret = duUpdateDuUeCbCfg(cellId, ueIdx, &duCb.actvCellLst[cellIdx]->ueCb[ueIdx-1],\
         duCb.actvCellLst[cellIdx]->ueCb[ueIdx-1].f1UeDb);
      if(!ret)
      {
...
uint8_t DuProcMacUeCfgRsp(Pst *pst, MacUeCfgRsp *cfgRsp)
{
   ...
   if(cfgRsp)
   {
         ...
         if(pst->event == EVENT_MAC_UE_CREATE_RSP)
	 {
            ...
            duCb.actvCellLst[cellIdx]->ueCb[ueIdx].macUeCfg.macUeCfgState = UE_CREATE_COMPLETED;
	 }
         else if(pst->event == EVENT_MAC_UE_RECONFIG_RSP)
         {
            DU_LOG("\nDU APP : MAC UE Reconfig Response : SUCCESS [UE IDX : %d]", cfgRsp->ueIdx);
            duCb.actvCellLst[cellIdx]->ueCb[ueIdx].macUeCfg.macUeCfgState = UE_RECFG_COMPLETED;
            duProcUeContextSetupRsp(cfgRsp->ueIdx, cfgRsp->cellId);
         }
      }
      else
      {
...
uint8_t DuProcRlcUlUeCfgRsp(Pst *pst, RlcUeCfgRsp *cfgRsp)
{
   ...
   if(cfgRsp)
   {
         ...
         if(pst->event == EVENT_RLC_UL_UE_CREATE_RSP)
	 {
	    ...
	    duCb.actvCellLst[cellIdx]->ueCb[ueIdx].rlcUeCfg.rlcUeCfgState = UE_CREATE_COMPLETED;
	 }
	 else if(pst->event == EVENT_RLC_UL_UE_RECONFIG_RSP)
	 {
            DU_LOG("\nDU_APP: RLC UE Reconfig Response : SUCCESS [UE IDX:%d]", cfgRsp->ueIdx);
	    duCb.actvCellLst[cellIdx]->ueCb[ueIdx].rlcUeCfg.rlcUeCfgState = UE_RECFG_COMPLETED;
	    duProcUeContextSetupRsp(cfgRsp->ueIdx, cfgRsp->cellId);
	 }
      }
      else
