...
/******************************************************************
 *
 * @brief Fills DuMacUeCfg structure
 *
 * @details
 *
...
uint8_t updateDuMacUeCfg(uint16_t cellId, uint8_t gnbDuUef1apId, uint16_t crnti, DuUeCfg *ueCfgDb, DuMacUeCfg *macUeCfg)
{
   ...
   else
   {
      ...
      memcpy(&ueCfgDb->copyOfmacUeCfg, macUeCfg, sizeof(DuMacUeCfg));
   }
   return ret;
}
...
uint8_t duCreateUeCb(UeCcchCtxt *ueCcchCtxt, uint32_t gnbCuUeF1apId)
{
   ...
   for(cellIdx = 0; cellIdx < MAX_NUM_CELL; cellIdx++)
   {
      if(duCb.actvCellLst[cellIdx] && (ueCcchCtxt->cellId == duCb.actvCellLst[cellIdx]->cellId))
      {
         ...
         memset(&duCb.actvCellLst[cellIdx]->ueCb[ueIdx].macUeCfg, 0, sizeof(DuMacUeCfg));
...
uint8_t duBuildAndSendUeContextSetupReq(uint16_t cellId, DuUeCb *ueCb)
{
   ...
   {
      /* Filling MAC UE Config */
      memset(&ueCb->macUeCfg, 0, sizeof(DuMacUeCfg));
