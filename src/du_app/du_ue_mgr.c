...
uint8_t duBuildAndSendUeCreateReqToRlc(uint16_t cellId, uint8_t ueIdx, RlcUeCfg *duRlcUeCfg)
{
   ...
   FILL_PST_TO_DUAPP(pst, EVENT_RLC_UE_CREATE_REQ);
 ...
 * 
 *  @params[in]  Pointer to CkwCfgCfm
...
void fillRlcUlUeCfgRsp(RlcUeCfgRsp *rlcCfgRsp, CkwCfgCfmInfo *ckwCfgRsp)
{
   uint8_t idx;
...
S16 duProcRlcUlUeCfgRsp(Pst *pst, SuId suId, CkwCfgCfmInfo *ckwCfgRsp)
{
   uint8_t ret = ROK;
   uint8_t idx;
   RlcUeCfgRsp rlcCfgRsp;
   memset(&rlcCfgRsp, 0, sizeof(RlcUeCfgRsp));

   if(ckwCfgRsp)
   {
      /* Mapping Old api to New Api */
      fillRlcUlUeCfgRsp(&rlcCfgRsp, ckwCfgRsp);
      if(rlcCfgRsp.result == RLC_DU_APP_RSP_OK)
      {
         DU_LOG("\nDU_APP: RLC UE Create Response : SUCCESS [UE IDX:%d]", rlcCfgRsp.ueIdx);
      }
      else
      {
         DU_LOG("\nDU_APP: RLC UE Create Response : FAILED [UE IDX:%d, REASON :%d]",\
	    rlcCfgRsp.ueIdx, rlcCfgRsp.reason);
	 ret = RFAILED;
      }
   }
   else
