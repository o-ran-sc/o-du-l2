   ...
   UeContextSetupDb *ueCfgDb, MacUeCfg *macUeCfg)
{
   ...
   else
   {
      ...
      if(ueCfg)
      {
         ...
         ueCfg->ueIdx        = ueIdx;
         ueCfg->crnti        = crnti;
         ret = procUeReCfgCellInfo(ueCfg, ueCfgDb->cellGrpCfg);
         if(!ret)
         {
            ret = getMaxAggrBitRate(ueCfg->maxAggrBitRate, ueCfgDb->maxAggrBitRate);
         }
         /* Filling LC Context */
         for(dbIdx = 0; (dbIdx < ueCfgDb->duUeCfg.numMacLcs && ret == ROK); dbIdx++)
         {
            for(lcIdx = 0; lcIdx < macUeCfg->numLcs; lcIdx++)
            {
               if(ueCfgDb->duUeCfg.macLcCfg[dbIdx].lcId == macUeCfg->lcCfgList[lcIdx].lcId)
               {
	          if((ueCfgDb->duUeCfg.macLcCfg[dbIdx].configType == CONFIG_UNKNOWN) ||
		     (ueCfgDb->duUeCfg.macLcCfg[dbIdx].configType == CONFIG_MOD_TYPE))
		  {
                     ret = getLcCfgToAddMod(&ueCfg->lcCfgList[dbIdx], &ueCfgDb->duUeCfg.macLcCfg[dbIdx]);
		     if(!ret)
		     {
		        ueCfg->lcCfgList[dbIdx].configType = CONFIG_MOD_TYPE;
		        ueCfgDb->duUeCfg.macLcCfg[dbIdx].configType = CONFIG_MOD_TYPE;
	             }
		  }
	       }
	       else
	       {
	          /* ADD/DEL CONFIG */
               ...
               if(!ret)
               {
	          if(!ueCfg->lcCfgList[dbIdx].ulLcCfgPres)
	          {
                     fillDefUlLcCfg(&ueCfg->lcCfgList[dbIdx].ulLcCfg);
                  }
                  ueCfg->numLcs++;
	       }
               else
   ...
   DuUeCfg *ueCfgDb, RlcUeCfg *rlcUeCfg)
{
   ...
   else
   {
      ...
      if(ueCfg)
      {
         ...
         {
            /* Fill Rlc Ue Cfg List */
            for(lcIdx = 0; lcIdx < rlcUeCfg->numLcs; lcIdx++)
            { 
               if(ueCfgDb->rlcLcCfg[dbIdx].lcId == rlcUeCfg->rlcLcCfg[lcIdx].lcId)
               {
	          if((ueCfgDb->rlcLcCfg[dbIdx].configType == CONFIG_UNKNOWN)||
		     (ueCfgDb->rlcLcCfg[dbIdx].configType == CONFIG_MOD_TYPE))
	          {
	             /* MOD */ 
                     memcpy(&ueCfg->rlcLcCfg[dbIdx], &ueCfgDb->rlcLcCfg[dbIdx], sizeof(RlcBearerCfg));
                     ueCfg->rlcLcCfg[dbIdx].configType = CONFIG_MOD_TYPE;
		     ueCfgDb->rlcLcCfg[dbIdx].configType = CONFIG_MOD_TYPE; /* update Db as well */
	          }
	       }
	       else
	       {
	          /* ADD/ DEL Config Type */
               ...
               if(!ueCfg->rlcLcCfg[dbIdx].rlcModeCfgPres)
               {
                  buildRlcModeDefaultCfg(ueCfg->rlcLcCfg[dbIdx].rlcMode, &ueCfg->rlcLcCfg[dbIdx]);
               }
               ueCfg->numLcs++;
	    }
         }
         ret = sendUeReCfgReqToRlc(ueCfg);
      }
      else
      {
         DU_LOG("\nDU_APP: Memory alloc failed at fillRlcUeCfg()");
	 ret = RFAILED;
      }
   }
   return ret;
 ...
 *
 * ****************************************************************/
uint8_t duUpdateMacLcCfg(MacUeCfg *macUeCfg, UeContextSetupDb *f1UeDb) 
{
   //TODO: fillMacUeCfg and delete DB
   return ROK;
}
...
uint8_t duUpdateRlcLcCfg(RlcUeCfg *rlcUeCfg, UeContextSetupDb *f1UeDb)
{
   ...
   for(dbIdx = 0; dbIdx < f1UeDb->duUeCfg.numRlcLcs; dbIdx++)
   {
      ...
      for(lcIdx = 0; lcIdx < numLcs; lcIdx++)
      {
	    ...
	    else if(f1UeDb->duUeCfg.rlcLcCfg[dbIdx].configType == CONFIG_MOD_TYPE)
	    {
               memset(&rlcUeCfg->rlcLcCfg[lcIdx], 0, sizeof(RlcBearerCfg));
	       //TODO: LC Cfg List to array to be updated
               //rlcUeCfg->rlcLcCfg[lcIdx].numLcs--;
	    }
         }
       }
       if(!lcIdFound)
...
uint8_t duUpdateDuUeCbCfg(uint8_t cellIdx, uint8_t ueIdx, DuUeCb *ueCb, UeContextSetupDb *f1UeDb)
{
   ...
   /*Filling MAC Ue Cfg */
   ueCb->macUeCfg.cellId = cellIdx++;
   ueCb->macUeCfg.ueIdx = ueIdx;
   ueCb->macUeCfg.crnti = ueCb->crnti;
   //TODO: Update MAC Config
   
   ret = duUpdateMacLcCfg(&ueCb->macUeCfg, f1UeDb);
   if(!ret)
...
uint8_t duProcUeSetupRequest(UeContextSetupDb *ueSetupDb)
{
   ...
   if(ueSetupDb)
   {
      ...
      if(ueCb)
      {
         ...
         ret = duBuildAndSendUeReCfgReqToRlc(cellId, ueCb->crnti, &ueSetupDb->duUeCfg, &ueCb->rlcUeCfg);
	 if(!ret)
	 {
         ...
         ret = duBuildAndSendUeReCfgReqToMac(cellId, ueCb->crnti, ueSetupDb, &ueCb->macUeCfg);
         if(ret)
         {
	 ...
	 else
	 {
	    ...
	    {
               getCellGrpCfg(ueCb->cellGrpCfg, ueSetupDb->cellGrpCfg);
	    }
#endif
	 }
      }
