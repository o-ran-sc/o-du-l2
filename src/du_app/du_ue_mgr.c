...
uint8_t sendUeResetReqToMac(uint16_t cellId, uint8_t ueId)
{
   Pst pst;
   uint8_t ret=ROK;
   MacUeReset *ueReset = NULLP;

   DU_ALLOC_SHRABL_BUF(ueReset, sizeof(MacUeReset));
   if(ueReset)
   {
      ...
      if(ret == RFAILED)
      {
         ...
         DU_FREE_SHRABL_BUF(DU_APP_MEM_REGION, DU_POOL, ueReset, sizeof(MacUeReset));
      }
   }
   else
   {
...
uint8_t duBuildAndSendUeResetReq(uint16_t cellId, uint16_t crnti)
{
   ...
   DU_LOG("\nDEBUG  -->  DU_APP: Processing UE Reset Request ");
   GET_CELL_IDX(cellId, cellIdx);
   GET_UE_ID(crnti, ueId);
   ...
   if(duCb.actvCellLst[cellIdx] != NULLP)
   {
      ...
      {
         DU_LOG("\nERROR  -->  DU APP : DuProcMacUeResetRsp():Failed to build UE  reset req for RLC ");
         return RFAILED;
      }
   }
   else
   {
      DU_LOG("\nERROR  -->  DU APP : duBuildAndSendUeResetReq(): Cell Id is not found");
      return RFAILED;
   }
...
uint8_t DuProcMacUeResetRsp(Pst *pst, MacUeResetRsp *resetRsp)
{
   ...
   if(resetRsp)
   {
      if(resetRsp->result == RESET_SUCCESSFUL)
      {
