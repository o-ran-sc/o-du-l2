...
/******************************************************************
 *
 * @brief Fills Lc Cfg of Dl Rrc Msg Info
 *
 * @details
 *
 *    Function : fillDlRrcMsgInfo 
 *
 *    Functionality: Fills Lc Cfg of Dl Rrc Msg Info
 *
 * @params[in] RlcBearerCfg *lcCfg
 * @return ROK     - success
 *         RFAILED - failure
 *
 * ****************************************************************/
void fillDlRrcMsgInfo(RlcDlRrcMsgInfo *dlRrcMsgInfo, RlcBearerCfg *lcCfg)
{
   dlRrcMsgInfo->rbType = lcCfg->rbType;
   dlRrcMsgInfo->rbId   = lcCfg->rbId;
   dlRrcMsgInfo->lcType = lcCfg->lcType;
   dlRrcMsgInfo->lcId   = lcCfg->lcId;
}
   ...
   uint8_t lcId, bool execDup, bool deliveryStaReq, uint16_t rrcMsgLen, uint8_t *rrcMsg)
{
   ...
   for(lcIdx = 0; lcIdx <= MAX_NUM_LC; lcIdx++)
   {
      ...
      {
         fillDlRrcMsgInfo(dlRrcMsgInfo, &ueCfg.rlcBearerCfgToAdd[lcIdx]);
         break;
      }
      if(ueCfg.rlcBearerCfgToMod[lcIdx].lcId == lcId)
      {
         fillDlRrcMsgInfo(dlRrcMsgInfo, &ueCfg.rlcBearerCfgToMod[lcIdx]);
         break;
      }
   }
   ...
   MacUeCfg *macUeCfg, DuUeCfg *duUeCfg, F1UeSetupReq *f1UeSetReqDb)
{
   ...
   {  
      if(!ueDbFound)
      {
         fillMacSrbCfg(SRB1_LCID, &macUeCfg->lcCfgToAddList[lcIdx], NULL);
   ...
   RlcUeCfg *rlcUeCfg, DuUeCfg *duUeCfg, F1UeSetupReq *f1UeSetReqDb)
{
   ...
   for(lcIdx = 0; lcIdx < numLcs; lcIdx++)
   {
      if(!ueDbFound)
      {
         procRlcLcCfgToAddMod(SRB1_LCID, SRB1_LCID, RB_TYPE_SRB, RLC_AM, NULL, &rlcUeCfg->rlcBearerCfgToAdd[lcIdx]);
