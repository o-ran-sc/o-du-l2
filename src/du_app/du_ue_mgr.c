...
uint8_t procDlRrcMsgTrans(F1AP_PDU_t *f1apMsg)
{
   ...
   for(idx=0; idx<duCb.numUe; idx++)
	...
	if(srbId == SRB_ID_1) //RRC connection setup
	{
      ...
      {
         duCreateUeCb(&duCb.ueCcchCtxt[idx], &ueIdx);
 ...
 *
 *****************************************************************/
uint8_t fillUeIdx(uint16_t *ueIdx)
{
   uint8_t ueBitPos = 0;
   uint8_t ueBitMask = 1; 
   
   do{
   	 if((duCb.ueBitMap & ueBitMask) == 0)
   	 {
   	   *ueIdx = ueBitPos;
   	   duCb.ueBitMap |= ueBitMask; 
   	   break;
       }
   	 else
   	 {
 ...
 *
 * ****************************************************************/
void duCreateUeCb(UeCcchCtxt *ueCcchCtxt, uint16_t *ueIdx)
{
	uint8_t cellIdx = 0;
   uint8_t idx = 0;
	DuCellCb duCellCb;
   ...
   for(cellIdx = 0; cellIdx < DU_MAX_CELLS; cellIdx++)
	{
	   if(ueCcchCtxt->cellId == duCb.actvCellLst[cellIdx]->cellId)
		{
			*ueIdx = fillUeIdx(&duCellCb.ueCb[idx].ueIdx);
         ...
         /* Filling MacCellGroup Config */ 
		   memset(&duCellCb.ueCb[*ueIdx].macCellGrpCfg, 0, sizeof(MacCellGrpCfg)); 
         fillMacCellGrpInfo(&duCellCb.ueCb[*ueIdx].macCellGrpCfg);
         /* Filling PhyCellGroup Config */ 
		   memset(&duCellCb.ueCb[*ueIdx].phyCellGrpCfg, 0, sizeof(PhyCellGrpCfg)); 
         fillPhyCellGrpInfo(&duCellCb.ueCb[*ueIdx].phyCellGrpCfg);
         /* Filling SpCellGroup Config */ 
		   memset(&duCellCb.ueCb[*ueIdx].spCellCfg, 0, sizeof(SpCellCfg)); 
         fillSpCellGrpInfo(&duCellCb.ueCb[*ueIdx].spCellCfg);
         
			duCellCb.ueCb[*ueIdx].maxAggrBitRate = NULLP;
         /* Filling LC Context */
         duCellCb.ueCb[*ueIdx].numLcs = 1;
         if(duCellCb.ueCb[*ueIdx].numLcs < MAX_NUM_LOGICAL_CHANNELS)
         {
            for(idx = 0; idx < duCellCb.ueCb[*ueIdx].numLcs; idx++)
            {   
               fillLcCfgList(&duCellCb.ueCb[*ueIdx].lcCfgList[idx]);
            }
         }
		}
	}
...
uint8_t duBuildAndSendUeCreateReqToMac(DuUeCb *duUeCb)
{
   ...
   DU_ALLOC_SHRABL_BUF(duUeCfg, sizeof(DuUeCb));
	memset(duUeCfg, 0, sizeof(DuUeCb));
	if(duUeCfg)
	{
      memcpy(duUeCfg, duUeCb, sizeof(DuUeCb));
      /* Processing one Ue at a time to MAC */
      ret = packDuMacUeCreateReq(&pst, duUeCfg);
		if(ret)
		{
         DU_LOG("\nDU_APP : Failure in sending Ue Create Req to MAC");
		   DU_FREE_SHRABL_BUF(DU_APP_MEM_REGION, DU_POOL, duUeCfg, sizeof(DuUeCb));
			ret = RFAILED;
         return ret;
		}
	}
