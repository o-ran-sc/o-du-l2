...
uint8_t rlcUeDeleteComp = 0;
uint8_t macUeDeleteComp = 0;
/*******************************************************************
 *
...
uint8_t  deleteUeCfg(uint16_t cellIdx, uint8_t ueIdx)
{
   ...
   if((duCb.actvCellLst[cellIdx]->ueCb[ueIdx-1].macUeCfg.macUeCfgState == UE_CFG_INACTIVE) &&\
   (duCb.actvCellLst[cellIdx]->ueCb[ueIdx-1].rlcUeCfg.rlcUeCfgState == UE_CFG_INACTIVE))
   {
      ueCb = &duCb.actvCellLst[cellIdx]->ueCb[ueIdx-1];
      deleteMacUeCfg(&ueCb[ueIdx-1].macUeCfg);
      deleteF1UeContextSetupDb(ueCb[ueIdx-1].f1UeDb);
      deleteRlcUeCfg(ueCb[ueIdx-1].rlcUeCfg.rlcLcCfg, ueCb[ueIdx-1].rlcUeCfg.numLcs);
      duCb.actvCellLst[cellIdx]->numActvUes--;
...
uint8_t DuProcMacUeDeleteRsp(Pst *pst, MacUeDeleteRsp *deleteRsp)
{
   ...
   if(deleteRsp)
   {
      if(deleteRsp->result == SUCCESS)
      {
         ...
         {
            duCb.actvCellLst[cellIdx]->ueCb[deleteRsp->ueIdx -1].macUeCfg.macUeCfgState = UE_CFG_INACTIVE;
...
void deleteF1UeContextSetupDb(F1UeContextSetupDb *f1UeDb)
{
   ...
   if(f1UeDb)
   {
      ...
      for(lcIdx=0 ;lcIdx < f1UeDb->duUeCfg.numMacLcs; lcIdx++)
      {
         DU_FREE_SHRABL_BUF(DU_APP_MEM_REGION, DU_POOL, f1UeDb->duUeCfg.macLcCfg[lcIdx].drbQos,\
               sizeof(DrbQosInfo));
         DU_FREE_SHRABL_BUF(DU_APP_MEM_REGION, DU_POOL, f1UeDb->duUeCfg.macLcCfg[lcIdx].snssai,\
               sizeof(Snssai));
      }

      for(tnlInfoIdx=0 ;tnlInfoIdx < f1UeDb->duUeCfg.numDrb; tnlInfoIdx++)
      {
         DU_FREE(f1UeDb->duUeCfg.upTnlInfo[tnlInfoIdx].tnlCfg1, sizeof(GtpTnlCfg));
         DU_FREE(f1UeDb->duUeCfg.upTnlInfo[tnlInfoIdx].tnlCfg2, sizeof(GtpTnlCfg));
      }
...
void deleteDuCellCb(DuCellCb *cellCb)
{
   ...
   for(ueIdx=0; ueIdx<cellCb->numActvUes; ueIdx++)
   {
      deleteMacUeCfg(&cellCb->ueCb[ueIdx].macUeCfg);
      deleteF1UeContextSetupDb(cellCb->ueCb[ueIdx].f1UeDb);
      deleteRlcUeCfg(cellCb->ueCb[ueIdx].rlcUeCfg.rlcLcCfg, cellCb->ueCb[ueIdx].rlcUeCfg.numLcs);
   }
...
uint8_t duProcCellDeleteReq(uint16_t cellId)
{
   ...
   if(duCb.actvCellLst[cellIdx] != NULLP)
   {
      ...
      {
         DU_LOG("\nERROR  -->  DU APP : duProcCellDeleteReq(): Delete all active UE");
         return RFAILED;
      }
      else
      {
