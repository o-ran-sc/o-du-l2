...
*
* ****************************************************************/
uint8_t duBuildAndSendDlCcchInd(uint8_t *dlCcchMsg, uint16_t crnti,\
  uint16_t cellId, DlCcchMsgType msgType, uint16_t size)
{
	uint8_t ret                  = ROK;
	uint16_t idx2;
   ...
   if(!dlCcchIndInfo)
   {
	...
	dlCcchIndInfo->dlCcchMsgLen = size;
...
uint8_t procDlRrcMsgTrans(F1AP_PDU_t *f1apMsg)
{
	...
	uint16_t               idx2, size, crnti, cellId;
			...
			case ProtocolIE_ID_id_RRCContainer:
				{
				   if(dlRrcMsg->protocolIEs.list.array[idx]->value.choice.RRCContainer.size > 0)
					{
				      size = dlRrcMsg->protocolIEs.list.array[idx]->value.choice.RRCContainer.size;
				      DU_ALLOC(dlCcchMsg, size);
   ...
   for(idx=0; idx<duCb.numUe; idx++)
	...
	if(srbId == SRB1) //RRC connection setup
	{
		ret = duBuildAndSendDlCcchInd(dlCcchMsg, crnti, cellId, RRC_SETUP, size);
	}
	return ret;
}
