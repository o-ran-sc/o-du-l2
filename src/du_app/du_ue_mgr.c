 ...
 *
 *****************************************************************/
void fillInitDlBwp(InitalDlBwp *initDlBwp)
{
   uint8_t idx = 0;
...
void fillSpCellGrpInfo(SpCellCfg *spCell)
{
   if(spCell)
   {
      ...
      DU_ALLOC_SHRABL_BUF(spCell->servCellCfg.pdschServCellCfg,\
       sizeof(PdschServCellCfg));
      if(spCell->servCellCfg.pdschServCellCfg)
      {
...
uint8_t duBuildAndSendUeCreateReq()
{
   ...
   /* sending to MAC */
   packDuMacUeCreateReq(&pst, duCb.ueCb);
