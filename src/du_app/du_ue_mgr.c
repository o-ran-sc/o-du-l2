		   ...
		   if(duCb.actvCellLst[cellId-1]->numActvUes < DU_MAX_UE)
			{
               ...
               ret = duCreateUeCb(&duCb.ueCcchCtxt[idx], ueIdx);
                  ...
                  DU_LOG("\nDU_APP: Failed at duCreateUeCb for cellId [%d]", duCb.ueCcchCtxt[idx].cellId);
				   ...
				   {
			         /* TODO:  Send Ue create Req To MAC 
                  DU_LOG("\nDU_APP: Sending Ue create Req to MAC");
			         duBuildAndSendUeCreateReqToMac(cellId, ueIdx);*/
				...
				{
               DU_LOG("\nDU_APP: Failed while assigning ueIdx");
					ret = RFAILED;
			   }
 ...
 *
 *****************************************************************/
void setUeBitMap(uint8_t ueIdx, uint8_t *ueBitMap)
{
   uint8_t ueBitMask = 1; 
   
   do{
   	 if((ueIdx & ueBitMask) == 0)
   	 {
   	   *ueBitMap = ueBitMask; 
   	   break;
       }
   ...
   for(cellIdx = 0; cellIdx < DU_MAX_CELLS; cellIdx++)
	{
	   if(ueCcchCtxt->cellId == duCb.actvCellLst[cellIdx]->cellId)
		{
		   setUeBitMap(ueIdx, &duCb.actvCellLst[cellIdx]->ueBitMap);
         ...
         duCb.actvCellLst[cellIdx]->ueCb[ueIdx].ueState       = UE_ACTIVE;
		   ...
		   duCb.actvCellLst[cellIdx]->ueCb[ueIdx].gnbCuUeF1apId = ueCcchCtxt->gnbCuUeF1apId;
