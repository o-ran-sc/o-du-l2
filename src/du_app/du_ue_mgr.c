...
uint8_t  deleteUeCfg(uint16_t cellIdx, uint8_t ueIdx)
{
   ...
   if((duCb.actvCellLst[cellIdx]->ueCb[ueIdx-1].macUeCfg.macUeCfgState == UE_CFG_DELETE) &&\
   (duCb.actvCellLst[cellIdx]->ueCb[ueIdx-1].rlcUeCfg.rlcUeCfgState == UE_CFG_DELETE))
   {
      ueCb = &duCb.actvCellLst[cellIdx]->ueCb[ueIdx-1];
...
uint8_t DuProcMacUeDeleteRsp(Pst *pst, MacUeDeleteRsp *deleteRsp)
{
   ...
   if(deleteRsp)
   {
      if(deleteRsp->result == SUCCESS)
      {
         ...
         {
            duCb.actvCellLst[cellIdx]->ueCb[deleteRsp->ueIdx -1].macUeCfg.macUeCfgState = UE_CFG_DELETE;
