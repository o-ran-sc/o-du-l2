...
uint8_t duHdlEgtpDlData(EgtpMsg  *egtpMsg)
{
   ...
   DU_LOG("\nINFO   -->  DU_APP : Processing DL data");
#ifdef EGTP_TEST
   Pst pst;
      ...
      DlCcchMsgType msgType, uint16_t dlCcchMsgSize, uint8_t *dlCcchMsg)
{
   ...
   DU_LOG("\nINFO   -->  DU APP : Building and Sending DL CCCH Ind to MAC");
...
uint8_t duBuildAndSendDlRrcMsgToRlc(uint16_t cellId, RlcUeCfg ueCfg, F1DlRrcMsg *f1DlRrcMsg)
{
   ...
   DU_LOG("\nINFO   -->  DU_APP: Sending Dl RRC Msg to RLC \n");
...
uint8_t duProcDlRrcMsg(F1DlRrcMsg *dlRrcMsg)
{
   ...
   if(ueCcchCtxtFound)
   {
	 ...
	 {
	    DU_LOG("\nINFO   -->  DU_APP: Max Active UEs has reached at procDlRrcMsgTrans()");
	    ret = RFAILED;
	 }
      }
   }
...
uint8_t sendUeReCfgReqToMac(MacUeCfg *macUeCfg)
{
   ...
   if(macUeCfg)
   {
      ...
      DU_LOG("\nINFO   -->  DU_APP: Sending Ue Reconfig Request to MAC");
...
uint8_t getDrbLcId(uint32_t *drbBitMap)
{
   ...
   DU_LOG("\nINFO   -->  DU_APP: Max LC Reached in getDrbLcId()");
   return RFAILED;
}
...
uint8_t sendUeReCfgReqToRlc(RlcUeCfg *rlcUeCfg)
{
   ...
   if(rlcUeCfg)
   {
      ...
      DU_LOG("\nINFO   -->  DU_APP: Sending Ue Reconfig Request to RLC UL");
...
uint8_t duCreateUeCb(UeCcchCtxt *ueCcchCtxt, uint32_t gnbCuUeF1apId)
{
   ...
   for(cellIdx = 0; cellIdx < MAX_NUM_CELL; cellIdx++)
   {
	 ...
	 DU_LOG("\nINFO   -->  DU_APP: Filling UeCb for ueIdx [%d]", ueIdx);
   ...
   uint16_t crnti, MacUeCfg *duMacUeCfg)
{
   ...
   if(macUeCfg)
   {
      ...
      DU_LOG("\nINFO   -->  DU_APP: Sending UE create request to MAC");
   ...
   RlcUeCfg *duRlcUeCfg)
{
   ...
   if(rlcUeCfg)
   {
      ...
      /* Processing one Ue at a time to RLC */
      DU_LOG("\nINFO   -->  DU_APP: Sending UE create request to RLC UL");
...
uint8_t duBuildAndSendUeContextSetupReq(uint16_t cellId, uint16_t crnti, DuUeCfg *duUeCfg)
{
   ...
   DU_LOG("\nINFO   -->  DU_APP: Processing Ue Context Setup Request for cellId [%d]", cellId);
