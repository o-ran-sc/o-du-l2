...
S16 BuildULNRInfo(NRFreqInfo_t *ulnrfreq)
{
   ...
   DU_ALLOC(ulnrfreq->freqBandListNr.list.array,sizeof(FreqBandNrItem_t *));
   ...
   {
      DU_FREE(ulnrfreq->freqBandListNr.list.array,sizeof(FreqBandNrItem_t *));
      RETVALUE(RFAILED);
   }
...
S16 BuildDLNRInfo(NRFreqInfo_t *dlnrfreq)
{
   ...
   DU_ALLOC(dlnrfreq->freqBandListNr.list.array,sizeof(FreqBandNrItem_t *));
   ...
   {
      DU_FREE(dlnrfreq->freqBandListNr.list.array,sizeof(FreqBandNrItem_t *));
      RETVALUE(RFAILED);
   }
...
S16 BuildExtensions(ProtocolExtensionContainer_4624P3_t **ieExtend)
{
   ...
   {
      DU_FREE((*ieExtend),sizeof(ProtocolExtensionContainer_4624P3_t));
      RETVALUE(RFAILED);
   }
...
{
   S16 BuildNrcgiret=0;
   S16 BuildFiveGSTacret=0,BuildServedPlmnret=0,BuildNrModeret=0;
   U8  idx;
   U8  plmnidx;
   U8  plmnId;
   U8  plmnCnt=1;
                  ...
                  if(BuildNrModeret == ROK)
                  {
                       ...
                       measurementTimingConfiguration.buf != NULLP)
                     {
                         if(srvCellItem->gNB_DU_System_Information !=NULLP)
                         {
                            ...
                            DU_FREE(srvCellItem->gNB_DU_System_Information,
                               sizeof(GNB_DU_System_Information_t));
                         }
                     ...
                     DU_FREE(srvCellItem->served_Cell_Information.nR_Mode_Info.choice.fDD->\
                     dL_NRFreqInfo.freqBandListNr.list.array,sizeof(FreqBandNrItem_t *));
...
{
   S16 BuildServedCellListreturn=0,BuildRrcVerreturn=0; 
   U8   idx=0,idx2=0,i=0;
   U8   ieId=0;
   U8   plmnidx=0;
   U8   servId=0;
                  ...
                  if(f1SetupReq->protocolIEs.list.array[idx]->value.choice.GNB_DU_Name.buf != NULLP)
                  {
                     if(BuildServedCellListreturn == ROK)
                     {
                       if(BuildRrcVerreturn == ROK)
                       {
                          ...
                          DU_FREE(rrcVer->iE_Extensions->list.array[0],\
                           sizeof(RRC_Version_ExtIEs_t));
                          DU_FREE(rrcVer->iE_Extensions->list.array,\
                           sizeof(RRC_Version_ExtIEs_t *));
                          DU_FREE(rrcVer->iE_Extensions,sizeof(ProtocolExtensionContainer_4624P81_t));
                       ...
                       choice.GNB_DU_Name.buf, strlen((char *)duCfgParam.duName));
                    }
