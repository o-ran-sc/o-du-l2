...
/*******************************************************************
 *
 * @brief Build and Send F1reset request from du to cu
 *
 * @details
 *
...
uint8_t BuildAndSendF1ResetReq()
{
   ...
   F1AP_PDU_t       *f1apMsg = NULLP;
   asn_enc_rval_t   encRetVal;
   DU_LOG("\nF1AP : Building F1 Reset request received from du to cu\n");
   do
   {
      DU_ALLOC(f1apMsg, sizeof(F1AP_PDU_t));
      if(f1apMsg == NULLP)
	 ...
	 for(idx=0; idx< encBufSize; idx++)
	 {
      ...
      if(SendF1APMsg(DU_APP_MEM_REGION, DU_POOL) != ROK)
      {
	 DU_LOG("\nF1AP : Sending F1 Reset request from du to cu failed");
	 break;
      }
...
uint8_t BuildAndSendF1ResetAck()
{
   ...
   asn_enc_rval_t         encRetVal;
   DU_LOG("\nF1AP : Building F1 Reset Acknowledgment when reset request recived from cu to du\n");
...
{
   //TODO will develop this later

   /*Reset_t       *f1ResetMsg = NULLP;
   uint8_t       idx = 0, ueIdx = 0, ret = ROK;
   ResetType_t   *ResetTypeValue = NULLP;

   DU_LOG("\nProcessing F1 reset request recived from cu to du");
   f1ResetMsg = &f1apMsg->choice.initiatingMessage->value.choice.Reset;
   ...
   switch(f1apMsg->present)
   {
      ...
      case F1AP_PDU_PR_initiatingMessage:
	 {
	    switch(f1apMsg->choice.initiatingMessage->value.present)
	    {
	       ...
	       {
	           DU_LOG("\nF1AP : F1 reset request received from cu to du");
