...
uint8_t BuildBsrConfig(DuUeCb *ueCb, struct BSR_Config *bsrConfig)
{
   ...
   else
   {
      ...
      *(bsrConfig->logicalChannelSR_DelayTimer) = ueCb->macUeCfg.macCellGrpCfg.bsrTmrCfg.srDelayTimer;
   }
 ...
 *
 * ****************************************************************/
uint8_t BuildDuToCuRrcContainer(DuUeCb *ueCb, OCTET_STRING_t *duToCuRrcContainer)
{
   uint8_t  ret = ROK;
      ...
      uint16_t rrcContSize, uint8_t *rrcContainer)
{
   ...
   while(true)
   {
	 ...
	 break;
      ...
      ret = BuildDuToCuRrcContainer(NULLP, &initULRRCMsg->protocolIEs.list.array[ieIdx]->value.choice.DUtoCURRCContainer);
      if(ret != ROK)
      {
	 break;
      }
...
uint8_t BuildAndSendUeContextModRsp(DuUeCb *ueCb)
{
   ...
   {
     DU_LOG("\nResponding with configurations from DU");
     //return ROK;
   }
   ...
   while(1)
   {
      ...
      if(ueCb->f1UeDb->actionType == UE_CTXT_CFG_QUERY)
      {
         ...
         BuildDuToCuRrcContainer(ueCb, &ueContextModifyRes->protocolIEs.list.array[ieIdx]->value.choice.DUtoCURRCInformation.cellGroupConfig);
      ...
      else if(ueCb->f1UeDb->actionType == UE_CTXT_MOD)
      {
         ...
         if(ueCb->f1UeDb && ueCb->f1UeDb->actionType == UE_CTXT_MOD)
         {
