...
{
   uint8_t  ieIdx=0, ret=ROK, ueIdx=0, ueId = 0;
   uint16_t cellIdx =0, cellId = 0, crnti = 0;
   bool ueIdxFound = false, ueReleaseForHandover = false;
      ...
      for(ieIdx=0; ieIdx < ueContextReleaseCommand->protocolIEs.list.count; ieIdx++)
      {
         if(ueContextReleaseCommand->protocolIEs.list.array[ieIdx])
         {
            switch(ueContextReleaseCommand->protocolIEs.list.array[ieIdx]->id)
            {
               ...
               case ProtocolIE_ID_id_Cause:
                  {
                     if(ueContextReleaseCommand->protocolIEs.list.array[ieIdx]->value.choice.Cause.choice.radioNetwork != CauseRadioNetwork_normal_release)
                     {
                        ueReleaseForHandover = true;
                     }
   ...
   if(ret != RFAILED)
   {
      duProcUeContextReleaseCommand(cellId, crnti, duUeCb, ueReleaseForHandover);
   }
