...
* @details
*
*    Function : FillDrbSetupModItemList
*
...
uint8_t FillDrbSetupModItemList(DRBs_SetupMod_Item_t *drbItem)
{
   uint8_t ret = ROK;
...
uint8_t FillDrbItemList(struct DRBs_SetupMod_ItemIEs *drbItemIe)
{
   ...
   if(FillDrbSetupModItemList(&(drbItemIe->value.choice.DRBs_SetupMod_Item)) != ROK)
   {
      DU_LOG("\nERROR  -->  F1AP : FillDrbSetupModItemList failed");
      return RFAILED;
   }
   return ROK;
}
...
/*****************************************************************i
 *
 * @brief extract DrbCfg For Ue Modification 
 *
 * @details
 *
 *    Function : extractDrbCfgForUeModification 
 *
 *    Functionality:
 *         - extract DrbCfg For Ue Modification 
 *
 * @params[in] DRBs_ToBeSetupMod_Item_t *drbItem, LcCfg *macLcToAdd, UpTnlCfg *upTnlInfo
 * @return ROK     - success
 *         RFAILED - failure
 *
 * ****************************************************************/

uint8_t extractDrbCfgForUeModification(DRBs_ToBeSetupMod_Item_t *drbItem, LcCfg *macLcToAdd, UpTnlCfg *upTnlInfo)
{
   DRB_Information_t *drbInfo = NULLP;
...
/*****************************************************************i
 *
 * @brief extract Mac RbCfg For UeModification 
 *
 * @details
 *
 *    Function : extractMacRbCfgForUeModification 
 *
 *    Functionality:
 *         - extract Mac RbCfg For UeModification 
 *
 * @params[in] 
 * @return ROK     - success
 *         RFAILED - failure
...
uint8_t extractMacRbCfgForUeModification(uint8_t lcId, DRBs_ToBeSetupMod_Item_t *drbCfg,\
  LogicalChannelConfig_t *ulLcCfg, LcCfg *lcCfg, UpTnlCfg *upTnlInfo)
{
   if(drbCfg != NULLP)
   {
      if(extractDrbCfgForUeModification(drbCfg, lcCfg, upTnlInfo) != ROK)
      {
         DU_LOG("ERROR  -->  F1AP : Failed to build Drb Qos at extractMacRbCfg()");
         return RFAILED;
      }
...
/*****************************************************************i
 *
 * @brief processing Mac LcCfg For Ue Modification
 *
 * @details
 *
 *    Function : procMacLcCfgForUeModification 
 *
 *    Functionality:
 *         - processing Mac LcCfg For Ue Modification
 *
 * @params[in] 
 * @return ROK     - success
 *         RFAILED - failure
...
uint8_t procMacLcCfgForUeModification(uint8_t lcId, uint8_t rbType, uint8_t configType,\
   DRBs_ToBeSetupMod_Item_t *drbItem, LogicalChannelConfig_t *ulLcCfg, LcCfg *lcCfg, UpTnlCfg *upTnlInfo)
{
   uint8_t ret = ROK;
   ...
   if(rbType == RB_TYPE_SRB)
   {
      ret = extractMacRbCfgForUeModification(lcId, NULL, ulLcCfg, lcCfg, NULL);
   }
   else if(rbType == RB_TYPE_DRB)
   {
      ret = extractMacRbCfgForUeModification(lcId, drbItem, ulLcCfg, lcCfg, upTnlInfo);
   }
   return ret;
}
...
/*****************************************************************i
 *
 * @brief processing Drb List To SetupMod of UE modification 
 *
 * @details
 *
 ...
 *
 *    Functionality:
 *         -processing Drb List To SetupMod
 *
 * @params[in]
 * @return ROK     - success
 *         RFAILED - failure
...
uint8_t procDrbListToSetupMod(uint8_t lcId, DRBs_ToBeSetupMod_Item_t *drbItem,\
   LcCfg *macLcToAdd, RlcBearerCfg *rlcLcToAdd, UpTnlCfg *upTnlInfo)
{
   uint8_t ret = ROK;
   ...
   /* Filling MAC INFO */
   ret = procMacLcCfgForUeModification(lcId, RB_TYPE_DRB, CONFIG_ADD, drbItem, NULL, macLcToAdd, upTnlInfo);
   if(ret == RFAILED)
   {
      DU_LOG("\nERROR  --> F1AP : Failed at RLC LC Cfg in procDrbListToSetup()");
      return ret;
...
/******************************************************************
 *
 * @brief extracting the DrbListToSetupMod
 *
 * @details
 *
 *    Function : extractDrbListToSetupMod
 *
 *    Functionality: extracting the DrbListToSetupMod
 *
 * @params[in] uint8_t lcId, DRBs_ToBeSetupMod_List_t *drbCfg, DuUeCfg *ueCfgDb
 * @return ROK     - success
 *         RFAILED - failure
 *
 * ****************************************************************/

uint8_t extractDrbListToSetupMod(uint8_t lcId, DRBs_ToBeSetupMod_List_t *drbCfg, DuUeCfg *ueCfgDb)
{
   uint8_t ret, drbIdx;
   struct DRBs_ToBeSetupMod_ItemIEs *drbItem = NULLP;
   ...
   if(drbCfg)
   {
      for(drbIdx = 0; drbIdx < drbCfg->list.count; drbIdx++)
      {
         drbItem = drbCfg->list.array[drbIdx];
         if((ueCfgDb->numMacLcs -1)> MAX_NUM_LC)
         {
            DU_LOG("\nERROR  -->  F1AP :  MAX LC Reached in MAC at extractDrbListToSetup()");
            ret = RFAILED;
            break;
         }
         if((ueCfgDb->numRlcLcs-1) > MAX_NUM_LC)
         {
            DU_LOG("\nERROR  -->  F1AP :  MAX LC Reached in RLC at extractDrbListToSetup()");
            ret = RFAILED;
            break;
         }
         memset(&ueCfgDb->macLcCfg[ueCfgDb->numMacLcs-1], 0, sizeof(LcCfg));
         memset(&ueCfgDb->rlcLcCfg[ueCfgDb->numRlcLcs-1], 0, sizeof(RlcBearerCfg));

         ret = procDrbListToSetupMod(lcId, &(drbItem->value.choice.DRBs_ToBeSetupMod_Item) ,\
               &ueCfgDb->macLcCfg[ueCfgDb->numMacLcs-1], &ueCfgDb->rlcLcCfg[ueCfgDb->numRlcLcs-1],\
	       &ueCfgDb->upTnlInfo[ueCfgDb->numDrb-1]);
         if(ret == RFAILED)
         {
            DU_LOG("\nERROR  -->  F1AP :  Failed at extractDrbListToSetup()");
            break;
...
uint8_t procF1UeContextModificationReq(F1AP_PDU_t *f1apMsg)
{
                     ...
                     (duCb.actvCellLst[cellIdx]->ueCb[ueIdx].gnbCuUeF1apId == gnbCuUeF1apId))
                     {
                        ...
                        if(lcId != RFAILED)
                        {
                           ...
                           if(duUeCb->f1UeDb)
                           {
                              ...
                              if(extractDrbListToSetupMod(lcId, &ueContextModifyReq->protocolIEs.list.array[ieIdx]->value.\
                              choice.DRBs_ToBeSetupMod_List,&duUeCb->f1UeDb->duUeCfg))
                              {
                                 DU_LOG("\nERROR  -->  DU APP : Failed at extractDrbListToSetup()");
                                 ret = RFAILED;
                              }
   ...
   BuildAndSendUeContextModResp(0,1);
   return ret; 
}
/*****************************************************************i
 *
