...
uint8_t BuildAndSendF1SetupReq()
{
   ...
   do
   {
      ...
      addOrModifyE2NodeComponent(COMPONENT_ADD_LIST, true, encBufSize, encBuf);
      /* Sending msg */
      if(sendF1APMsg() != ROK)
      {
...
uint8_t procF1SetupRsp(F1AP_PDU_t *f1apMsg, MsgLen recvBufLen, char *recvBuf)
{
   ...
   addOrModifyE2NodeComponent(COMPONENT_ADD_LIST, false, recvBufLen, recvBuf);
