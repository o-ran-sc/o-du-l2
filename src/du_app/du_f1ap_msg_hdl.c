...
uint8_t BuildInitialDlBWPret=ROK;
uint8_t BuildUlCfgret=ROK;
uint8_t BuildInitialUlBWPret=ROK;
uint8_t BuildPuschTimeDomAllocListret=ROK;
uint8_t BuildDMRSULPuschMapTypeAret=ROK;
uint8_t BuildDMRSDLPdschMapTypeAret=ROK;
uint8_t BuildPdschTimeDomAllocListret=ROK;
...
uint8_t BuildBWPDlDedPdschCfg(struct PDSCH_Config *pdschCfg)
{
   ...
   BuildDMRSDLPdschMapTypeAret=ROK;
   ...
   BuildPdschTimeDomAllocListret = ROK;
...
uint8_t BuildBWPUlDedPuschCfg(PUSCH_Config_t *puschCfg)
{
   ...
   *(puschCfg->dataScramblingIdentityPUSCH) = SCRAMBLING_ID;
   ...
   BuildDMRSULPuschMapTypeAret=ROK;
   ...
   BuildPuschTimeDomAllocListret=ROK;
						      ...
						      if(srvCellCfg->defaultDownlinkBWP_Id)
							   {
							      if(srvCellCfg->uplinkConfig)
								   {
								      ulCfg=srvCellCfg->uplinkConfig;
									   if(ulCfg->initialUplinkBWP)
									   {
									      ulBwp=ulCfg->initialUplinkBWP;
											if(BuildInitialUlBWPret == ROK)
										   {
										      if(ulCfg->firstActiveUplinkBWP_Id)
											   {
											      if(ulCfg->pusch_ServingCellConfig)
													      ...
													      if(srvCellCfg->pdsch_ServingCellConfig)
														   {
														      pdschCfg= srvCellCfg->pdsch_ServingCellConfig;
															   if(pdschCfg->choice.setup)
															   {
															      DU_FREE(pdschCfg->choice.setup->nrofHARQ_ProcessesForPDSCH,
															      sizeof(long));
															      DU_FREE(pdschCfg->choice.setup, sizeof( struct
											         ...
											         if(puschCfg_l->choice.setup)
														{
														   ...
														   {
														      if(puschCfg_l->choice.setup->ext1->maxMIMO_Layers)
														      {
															       DU_FREE(puschCfg_l->choice.setup->ext1->processingType2Enabled,sizeof(BOOLEAN_t));
																    DU_FREE(puschCfg_l->choice.setup->ext1->maxMIMO_Layers,
														   	    sizeof(long));
															   }
														      DU_FREE(puschCfg_l->choice.setup->ext1, sizeof(struct
											   			   PUSCH_ServingCellConfig__ext1));
														    }
													       DU_FREE(puschCfg_l->choice.setup, sizeof(struct
														    PUSCH_ServingCellConfig));
													    }
													    DU_FREE(ulCfg->pusch_ServingCellConfig, \
												   	 sizeof(struct UplinkConfig__pusch_ServingCellConfig));
												    }
													 ...
													 if(puschCfg->dataScramblingIdentityPUSCH)
													 {
												 	     if(puschCfg->dmrs_UplinkForPUSCH_MappingTypeA)
														  {
															   dmrsUlCfg=puschCfg->dmrs_UplinkForPUSCH_MappingTypeA;
															 	if(BuildDMRSULPuschMapTypeAret == ROK)
																{
																    if(puschCfg->pusch_TimeDomainAllocationList)
																	 {
																	    timeDomAllocList_t=puschCfg->pusch_TimeDomainAllocationList;
																		 if(BuildPuschTimeDomAllocListret == ROK)
																		 {
																		    if(puschCfg->transformPrecoder)
																			 {
																		 ...
																		 if(timeDomAllocList_t->choice.setup)
																		 {
																		     if(timeDomAllocList_t->choice.setup->list.array)
																		 	  {
																				    if(timeDomAllocList_t->choice.setup->list.array[idx2])
																					 {
																					 ...
																					 {
																					     if(timeDomAllocList_t->choice.setup->list.array[idx1])
																						  {
													     ...
													     if(timeDomAllocList->choice.setup->list.array)
													 	  {
															    ...
															    {
															        if(timeDomAllocList->choice.setup->list.array[idx1])
																	  {
							         ...
							         if(pdcchCfg->controlResourceSetToAddModList)
									   {
										    ...
										    if(controlRSetList->list.array)
										    {
													      ...
													      if(controlRSet->pdcch_DMRS_ScramblingID)
														   {
														      if(pdcchCfg->searchSpacesToAddModList)
															   {
															      searchSpcList=pdcchCfg->searchSpacesToAddModList;
																   if(searchSpcList->list.array)
																   { 
																				   	     ...
																				   	     if(searchSpc->nrofCandidates)
																					 	     {
																					           ...
																					           DU_FREE(searchSpc->nrofCandidates, sizeof(struct
																						        SearchSpace__nrofCandidates));
																					        }
																       ...
																       {
																          if(searchSpcList->list.array[idx1])
																          {
																             DU_FREE(searchSpcList->list.array[idx1], sizeof(struct
																	          SearchSpace));
																	       }
																       }
											      ...
											      for(idx1 = 0; idx1 <controlRSetList->list.count; idx1++)
												   {
													   if(controlRSetList->list.array[idx1])
														{
														   DU_FREE(controlRSetList->list.array[idx1], sizeof(struct
															ControlResourceSet));
														}
													}
