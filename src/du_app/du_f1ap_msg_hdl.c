...
void FreeF1SetupReq(F1AP_PDU_t *f1apMsg)
{
   ...
   if(f1apMsg != NULLP)
   {
	 ...
	 {
	    for(idx1 = 0; idx1 < f1SetupReq->protocolIEs.list.count; idx1++)
	    {
	       if(f1SetupReq->protocolIEs.list.array[idx1] != NULLP)
	       {
		  switch(f1SetupReq->protocolIEs.list.array[idx1]->id)
		  {
...
uint8_t extractCellsToBeActivated(Cells_to_be_Activated_List_t cellsToActivate)
{
   ...
   for(idx=0; idx<cellsToActivate.list.count; idx++)
   {
	 ...
	 pci = *cell.nRPCI;
      ...
      ret = procCellsToBeActivated(nci, pci);
   }
   return ret;
}
...
{
   uint8_t ret, ieIdx, elementIdx;
   uint8_t elementCnt, servCellModCnt;
   ...
   while(true)
   {
	 ...
	 if(duCfgUpdate->protocolIEs.list.array[ieIdx] == NULLP)
	 {
      ...
      duCfgUpdate->protocolIEs.list.array[ieIdx]->value.choice.TransactionID = \
									      TRANS_ID;
      ...
      cellsToModify = &duCfgUpdate->protocolIEs.list.array[ieIdx]->value.choice.\
		      Served_Cells_To_Modify_List;
      servCellModCnt = 1;
      cellsToModify->list.count = servCellModCnt;
      cellsToModify->list.size = \
				 servCellModCnt*sizeof(Served_Cells_To_Modify_ItemIEs_t *);
      DU_ALLOC(cellsToModify->list.array,cellsToModify->list.size);
      if(cellsToModify->list.array == NULLP)
      {
	 break;
      }
      for(elementIdx=0; elementIdx<servCellModCnt ;elementIdx++)
      {
	 DU_ALLOC(cellsToModify->list.array[elementIdx],sizeof(\
		  Served_Cells_To_Modify_ItemIEs_t));
	 if(cellsToModify->list.array[elementIdx] == NULLP)
	 {
	    break;
	 }
      }
	 ...
	 modifyItem->served_Cell_Information.nRCGI.nRCellIdentity.buf[tmp] = 0;
      ...
      /*servedPLMNs*/
      servCellModCnt = 1;
      modifyItem->served_Cell_Information.servedPLMNs.list.count = servCellModCnt;
      modifyItem->served_Cell_Information.servedPLMNs.list.size = \
								  servCellModCnt*sizeof(ServedPLMNs_Item_t *);
	 ...
	 break;
      ...
      for(elementIdx=0;elementIdx<servCellModCnt;elementIdx++)
      {
	 DU_ALLOC(modifyItem->served_Cell_Information.servedPLMNs.list.array[elementIdx],\
	       sizeof(ServedPLMNs_Item_t));
	 if(modifyItem->served_Cell_Information.servedPLMNs.list.array[elementIdx]== NULLP)
	 {
	    break;
	 }
      }
	 ...
	 iE_Extensions->list.count = servCellModCnt;
      modifyItem->served_Cell_Information.servedPLMNs.list.array[0]->\
	 iE_Extensions->list.size = servCellModCnt *sizeof(ServedPLMNs_ItemExtIEs_t *);
      DU_ALLOC(modifyItem->served_Cell_Information.servedPLMNs.list.\
	 ...
	 break;
      ...
      {
	 DU_ALLOC(modifyItem->served_Cell_Information.servedPLMNs.list.\
	       array[0]->iE_Extensions->list.array[elementIdx],\
	       sizeof(ServedPLMNs_ItemExtIEs_t));
	 if(modifyItem->served_Cell_Information.servedPLMNs.list.\
	       array[0]->iE_Extensions->list.array[elementIdx] == NULLP)
	 {
	    break;
	 }
      }
	 ...
	 break;
      ...
      duCfgUpdate->protocolIEs.list.array[ieIdx]->value.choice.GNB_DU_ID.buf[0] = \
										 duCfgParam.duId;
	 ...
	 for(elementIdx =0; elementIdx < encBufSize; elementIdx++)
	 {
	    printf("%x",encBuf[elementIdx]);
	 }
      }
      /* Sending msg */
      if(SendF1APMsg(DU_APP_MEM_REGION, DU_POOL) != ROK)
...
uint8_t procGNBDUCfgUpdAck(F1AP_PDU_t *f1apMsg)
{
   ...
   F1GnbDuCfgUpdAck gnbDuAckDb;
   memset(&gnbDuAckDb, 0, sizeof(F1GnbDuCfgUpdAck));
   ...
   for(elementIdx=0; elementIdx < gnbDuAck->protocolIEs.list.count; elementIdx++)
   {
      switch(gnbDuAck->protocolIEs.list.array[elementIdx]->id)
      {
         case ProtocolIE_ID_id_TransactionID:
	    gnbDuAckDb.transId = gnbDuAck->protocolIEs.list.array[elementIdx]->\
	       value.choice.TransactionID;
	    break;
         case ProtocolIE_ID_id_Cells_to_be_Activated_List:
	    break;
         default :
            DU_LOG("\nF1AP: Invalid IE Received: %ld, at procGNBDUCfgUpdAck()", \
	    gnbDuAck->protocolIEs.list.array[elementIdx]->id);
	    break;
      }
   }
   duProcGnbDuCfgUpdAckMsg(&gnbDuAckDb);
   return ROK;
}
/*******************************************************************
 *
...
uint8_t procDlRrcMsgTrans(F1AP_PDU_t *f1apMsg)
{
   ...
   ret = duProcRrcMsgTrans(srbId, dlCcchMsg, dlCcchMsgSize, \
	 gnbDuUeF1apId, gnbCuUeF1apId);
   return ret;
}
...
uint8_t procUeContextSetupReq(F1AP_PDU_t *f1apMsg)
{
   ...
   ret = duProcUeSetupRequest(cellIdx, deliveryStaReq, rrcMsgLen, rrcMsg,\
	 gnbDuUeF1apId, gnbCuUeF1apId);
   return ret;
