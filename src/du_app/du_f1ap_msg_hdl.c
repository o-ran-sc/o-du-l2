...
/*****************************************************************i
*
* @brief processing the ue context modification request  
*
* @details
*
...
*
*    Functionality:
*         - processing the ue context modification request 
*
* @params[in] F1AP_PDU_t *f1apMsg
* @return ROK     - success
*         RFAILED - failure
...
uint8_t procF1UeContextModificationReq(F1AP_PDU_t *f1apMsg)
{
   ...
   uint32_t gnbCuUeF1apId, gnbDuUeF1apId, bitRateSize;
   DuUeCb   *duUeCb = NULLP;
   UEContextModificationRequest_t *ueSetReq = NULLP;

   ret = ROK;

   ueSetReq = &f1apMsg->choice.initiatingMessage->value.choice.UEContextModificationRequest;
   for(ieIdx=0; (ieIdx < ueSetReq->protocolIEs.list.count && ret == ROK); ieIdx++)
   {
      switch(ueSetReq->protocolIEs.list.array[ieIdx]->id)
      {
	 case ProtocolIE_ID_id_gNB_CU_UE_F1AP_ID:
	    {
	       gnbCuUeF1apId =
		  ueSetReq->protocolIEs.list.array[ieIdx]->value.choice.GNB_CU_UE_F1AP_ID;
	       break;
	    }
	 case ProtocolIE_ID_id_gNB_DU_UE_F1AP_ID:
	    {
	       gnbDuUeF1apId =
		  ueSetReq->protocolIEs.list.array[ieIdx]->value.choice.GNB_DU_UE_F1AP_ID;
	       break;
	    }
...
*
* ****************************************************************/
uint8_t BuildAndSendUeContextModifyRes()
{
   uint8_t    ieIdx = 0;
   uint8_t    elementCnt = 0;
   uint8_t    ret = RFAILED;
   ...
   while(1)
   {
	 ...
	 if(ueContextModifyRes->protocolIEs.list.array[ieIdx] == NULLP)
	 {
      ...
      ueContextModifyRes->protocolIEs.list.array[ieIdx]->value.choice.GNB_CU_UE_F1AP_ID =CU_ID;
      ...
      ueContextModifyRes->protocolIEs.list.array[ieIdx]->value.choice.GNB_DU_UE_F1AP_ID =DU_ID;
	    ...
	    switch(f1apMsg->choice.initiatingMessage->value.present)
	    {
	       ...
	       case InitiatingMessage__value_PR_UEContextSetupRequest:
		  {
	       ...
	       case InitiatingMessage__value_PR_UEContextModificationRequest:
		     ...
		     BuildAndSendUeContextModifyRes();
		     break;
		  }
	       default:
		  {
