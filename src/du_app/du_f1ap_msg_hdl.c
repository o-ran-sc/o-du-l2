 ...
 * @details
 *
 *    Function : fillCellsToBeActivated
 *
 *    Functionality:
...
uint8_t fillCellsToBeActivated(Cells_to_be_Activated_List_t cellsToActivate)
{
   uint8_t  ret = ROK;
...
uint8_t procF1SetupRsp(F1AP_PDU_t *f1apMsg)
{
   ...
   for(idx=0; idx<f1SetRspMsg->protocolIEs.list.count; idx++)
   {
	 ...
	 case ProtocolIE_ID_id_Cells_to_be_Activated_List:
	    {
	       fillCellsToBeActivated(f1SetRspMsg->protocolIEs.list.array[idx]->\
