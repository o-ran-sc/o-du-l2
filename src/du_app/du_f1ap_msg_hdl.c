...
uint8_t addOrModifyE2NodeComponent(uint8_t action, bool reqPart, uint8_t bufSize, char *bufString)
{
   ...
   else
   {
      if(duCb.e2apDb.e2NodeComponentList.count)
      {
         ...
         if(!nodeFound)
         {
            DU_LOG("\nERROR  -->  F1AP : Unable to find the node");
            return RFAILED;
         }
      } 
   }
...
uint8_t BuildAndSendF1SetupReq()
{
   ...
   do
   {
      ...
      if(addOrModifyE2NodeComponent(E2_NODE_COMPONENT_ADD, true, encBufSize, encBuf) !=ROK)
      {
...
uint8_t BuildAndSendDUConfigUpdate(ServCellAction servCellAction)
{
   ...
   while(true)
   {
      ...
      if(addOrModifyE2NodeComponent(E2_NODE_COMPONENT_UPDATE, true, encBufSize, encBuf)!=ROK)
      {
...
uint8_t procF1SetupRsp(F1AP_PDU_t *f1apMsg, MsgLen recvBufLen, char *recvBuf)
{
   ...
   if(addOrModifyE2NodeComponent(E2_NODE_COMPONENT_ADD, false, recvBufLen, recvBuf) !=ROK)
   {
      DU_LOG("\nERROR  -->  F1AP : Failed to add the e2 node in the list");
      return RFAILED;
   }
...
uint8_t procF1GNBDUCfgUpdAck(F1AP_PDU_t *f1apMsg, MsgLen recvBufLen, char *recvBuf)
{
   ...
   if(addOrModifyE2NodeComponent(E2_NODE_COMPONENT_UPDATE, false, recvBufLen, recvBuf) !=ROK)
   {
      DU_LOG("\nERROR  -->  F1AP : Failed to update the e2 node in the list");
      return RFAILED;
   }
