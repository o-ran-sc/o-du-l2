...
uint8_t FillPagingInfoInCellCb(DuCellCb* cellCb, uint8_t ueId, uint8_t sTmsi, uint8_t pagingDrx, uint8_t pagingPrioity)
{
   uint16_t pagingOffset = 0, T=0, N=0, ns =0, pagingFrame = 0, x = 0;
   ...
   if(cellCb)
   {
      ...
      pagingFrame = (T / N) * ((ueId) % N);
      pagingFrame = (pagingFrame + pagingOffset) % T;
      
      currentSfn = cellCb->currSlotInfo.sfn;
      /* SFN for the PF is determined by */
      if(currentSfn > pagingFrame)
      {
         x  = ((currentSfn - pagingFrame) / T) + 1;
      }
      else
      {
         x  = ((pagingFrame - currentSfn) / T) + 1;
      }

      tSfn = pagingFrame + T * x;
      if(currentSfn <= pagingFrame)
      {
      ...
      cellCb->tmpPagingInfoOfUe.pagingFrame =  (sfn % 1024);
...
uint8_t procPagingMsg(F1AP_PDU_t *f1apMsg) 
{
   ...
   uint8_t pagingDrx = 0, pagingPriority = 0;
   uint8_t ueIdentity, ueId = 0;
   uint8_t sTmsi = 0;
   Paging_t   *paging;
   PagingCell_list_t  *pagingCelllist;
   PagingCell_ItemIEs_t *pagingCellItemIes;
                           ...
                           for(cellIdx = 0; cellIdx < pagingCelllist->list.count; cellIdx++)
                           {
                              if(pagingCelllist->list.array[cellIdx])
                              {
                                 ...
                                 if(duCb.actvCellLst[cellIdxNum])
                                 {
                                    ...
                                    if(FillPagingInfoInCellCb(duCb.actvCellLst[cellIdxNum], ueId, sTmsi, pagingDrx, pagingPriority)!= ROK)
                                    {
