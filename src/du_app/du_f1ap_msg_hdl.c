   ...
   LcCfg *macLcToAdd, RlcBearerCfg *rlcLcToAdd)
{
   ...
   if(ret == RFAILED)
   { 
      DU_LOG("\nDEBUG  --> F1AP : Failed at RLC LC Cfg in procDrbListToSetup()");
      return ret;
   }
...
uint8_t extractDrbListToSetup(uint8_t lcId, DRBs_ToBeSetup_List_t *drbCfg, DuUeCfg *ueCfgDb)
{
   ...
   if(drbCfg)
   {
	 ...
	 if(ueCfgDb->numMacLcs > MAX_NUM_LC)
	 { 
            DU_LOG("\nDEBUG  -->  F1AP :  MAX LC Reached in MAC at extractDrbListToSetup()");
	    ret = RFAILED;
	    break;
	 }
	 if(ueCfgDb->numRlcLcs > MAX_NUM_LC)
	 {
            DU_LOG("\nDEBUG  -->  F1AP :  MAX LC Reached in RLC at extractDrbListToSetup()");
	    ret = RFAILED;
	    break;
	 }
	 memset(&ueCfgDb->macLcCfg[ueCfgDb->numMacLcs], 0, sizeof(LcCfg));
	 ...
	 if(ret == RFAILED)
	 {
            DU_LOG("\nDEBUG  --> F1AP : Failed at extractDrbListToSetup()");
	    break;
	 }
      }
   }
