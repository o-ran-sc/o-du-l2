...
uint8_t fillF1apPduInfo(uint8_t transId, F1AP_PDU_t *f1apMsg)
{
    F1apPduInfo *f1apPdu = NULLP, *temp = NULLP;
    DU_ALLOC(f1apPdu, sizeof(F1apPduInfo));
    if(f1apPdu == NULLP)
    {
       DU_LOG("\nERROR  --> F1AP : fillF1apPduInfo(): Memory allocation failed");
       return RFAILED;      
    }
    f1apPdu->transId = transId;
    f1apPdu->f1apMsg = f1apMsg;
    
    if(f1apPduInfoList == NULLP)
    {
       f1apPdu->next = NULLP;
       f1apPduInfoList = f1apPdu;
    }
    else
    {
       temp = f1apPduInfoList;
...
uint8_t buildServCellToDeleteList(Served_Cells_To_Delete_List_t *cellsToDelete)
{
   ...
   {
      DU_LOG("\nERROR  -->  F1AP: buildServCellToDeleteList(): falied to fill deleteItemIe");
      return RFAILED;
   }
   return ROK;
}
...
uint8_t BuildAndSendDUConfigUpdate()
{
   ...
   if(fillF1apPduInfo(TRANS_ID,f1apDuCfg) != ROK)
   {
      DU_LOG("\nERROR  -->  F1AP : failed to fill F1AP pdu information");
      return RFAILED;
   }
...
F1AP_PDU_t *searchF1apPduInfo(uint8_t transId)
{
   bool pduInfoPresent = false;
   F1apPduInfo *pdu=NULLP;

   pdu = f1apPduInfoList;
   if(pdu == NULLP)
   {
      DU_LOG("\nERROR  -->  F1AP: searchF1apPduInfo(): f1apPdu is empty");
      return NULL;
   }
   ...
   if(pduInfoPresent != true)
   {
      DU_LOG("\nERROR  -->  F1AP: searchF1apPduInfo(): TransId is not available");
      return NULL;
   }
   return pdu->f1apMsg;
}
...
void deleteF1apPduInfo(uint8_t transId)
{
   F1apPduInfo *temp = f1apPduInfoList, *prev=NULL;

   if (temp != NULL && temp->transId == transId)
   {
      FreeDUConfigUpdate((F1AP_PDU_t *)temp->f1apMsg);
      f1apPduInfoList = temp->next;
      DU_FREE(temp, sizeof(F1apPduInfo));;
   }
...
* @details
*
*    Function : BuildGnbDuCfgUpdateAckOutput 
*
*    Functionality: 
*        Building output of gnb-du config update ack 
*
* @params[in] transId, pointer to  GNBDUConfigurationUpdateAcknowledge_t
* @return void
*
* ****************************************************************/

uint8_t BuildGnbDuCfgUpdateAckOutput(uint8_t transId, GNBDUConfigurationUpdateAcknowledge_t *gnbDuAck)
{
   uint8_t ueId =0 , ueIdx =0, ieIdx=0;
   uint8_t cellId =0, cellIdx =0, crnti=0;
   F1AP_PDU_t *f1apMsgPdu = NULLP;
   bool cellToModifyPres = false, cellToDeletePres = false;
   GNBDUConfigurationUpdate_t *gnbDuConfigUpdate = NULLP;

   f1apMsgPdu = searchF1apPduInfo(transId);
      ...
      if(f1apMsgPdu->choice.initiatingMessage)
      {
         ...
         for(ieIdx=0; ieIdx < gnbDuConfigUpdate->protocolIEs.list.count; ieIdx++)
         {
            switch(gnbDuConfigUpdate->protocolIEs.list.array[ieIdx]->id)
            {
               ...
               case ProtocolIE_ID_id_Served_Cells_To_Modify_List:
               {
                  cellToModifyPres = true;
                  break;
               }

               case ProtocolIE_ID_id_Served_Cells_To_Delete_List:
               {
                  cellToDeletePres = true;
                  break;
               }
   ...
   deleteF1apPduInfo(transId);
   if(cellToModifyPres == true)
   {
      duProcGnbDuCfgUpdAckMsg();
   }
   
   if(cellToDeletePres == true)
   {
      for(cellIdx = 0; cellIdx < duCb.numActvCells; cellIdx++)
      {
         ...
         {
            DU_LOG("ERROR  --> DU_APP : BuildGnbDuCfgUpdateAckOutput(): Cell does not exists");
            return RFAILED;
         }
...
uint8_t procF1GNBDUCfgUpdAck(F1AP_PDU_t *f1apMsg)
{
   ...
   BuildGnbDuCfgUpdateAckOutput(transId, gnbDuAck);
