...
uint8_t BuildAndSendF1SetupReq()
{
   ...
   do
   {
	 ...
	 break;
      ...
      {
	 DU_LOG("\nINFO   -->  F1AP : Created APER encoded buffer for F1SetupRequest\n");
   ...
   while(true)
   {
	...
	GNBDUConfigurationUpdateIEs__value_PR_GNB_DU_ID;
	 ...
	 break;
      ...
      {
	 DU_LOG("\nINFO   -->  F1AP : Created APER encoded buffer for DUConfigUpdate\n");
	 for(ieIdx =0; ieIdx < encBufSize; ieIdx++)
	 {
	    printf("%x",encBuf[ieIdx]);
	 }
      ...
      if(SendF1APMsg(DU_APP_MEM_REGION, DU_POOL) != ROK)
      {
	 DU_LOG("\nERROR  -->  F1AP : Sending GND-DU Config Update failed");
	 break;
      }
      ...
      uint16_t msgLen, uint8_t *rrcMsg)
{
   ...
   while(true)
   {
	 ...
	 break;
      ...
      {
	 DU_LOG("\n INFO   -->  F1AP : Created APER encoded buffer for ULRRCMessageTransfer\n");
...
uint8_t BuildDuToCuRrcContainer(DUtoCURRCContainer_t *duToCuRrcContainer)
{
   ...
   while(true)
   {
	 ...
	 break;
      ...
      {
	 DU_LOG("\n INFO   -->  F1AP : Created APER encoded buffer for DuToCuRrcContainer\n");
	 for(int i=0; i< encBufSize; i++)
	 {
	    printf("%x",encBuf[i]);
	 }
      ...
      uint16_t rrcContSize, uint8_t *rrcContainer)
{
   ...
   while(true)
   {
	 ...
	 DU_LOG("\n INFO   -->  F1AP : Created APER encoded buffer for Initial UL RRC\
	       Message transfer\n");
	 for(int i=0; i< encBufSize; i++)
	 {
	    printf("%x",encBuf[i]);
...
uint8_t extractSrbListToSetup(SRBs_ToBeSetup_List_t *srbCfg, DuUeCfg *ueCfgDb)
{
   ...
   if(srbCfg)
   {
	 ...
	 { 
            DU_LOG("\nINFO   -->  F1AP:  MAX LC Reached in MAC");
	    ret = RFAILED;
	    break;
	 }
         if(ueCfgDb->numRlcLcs > MAX_NUM_LC)
	 {
            DU_LOG("\nINFO   -->  F1AP:  MAX LC Reached in RLC");
	    ret = RFAILED;
	    break;
	 }
   ...
   for(ieIdx=0; (ieIdx < ueSetReq->protocolIEs.list.count && ret == ROK); ieIdx++)
   {
	 ...
	 case ProtocolIE_ID_id_ServCellIndex:
         ...
         case ProtocolIE_ID_id_CUtoDURRCInformation:
	    {
         ...
         case ProtocolIE_ID_id_SCell_ToBeSetup_List:
	    {
	       DU_LOG("\nINFO   -->  DU_APP: Received SCell to be added");
	       DU_LOG("\nINFO   -->  DU_APP: Not processing the SCell_ToBeSetup_List");
	    break;
	    }
         case ProtocolIE_ID_id_SRBs_ToBeSetup_List:
	    {
...
uint8_t EncodeUeCntxtDuToCuInfo(CellGroupConfig_t *duToCuCellGrp, CellGroupConfigRrc_t *duCellGrpCfg)
{
   ...
   {
      DU_LOG("\n INFO   -->  F1AP : Created APER encoded buffer for UeCntxtDuToCuInfo\n");
      for(int i=0; i< encBufSize; i++)
      {
	 printf("%x",encBuf[i]);
      }
...
uint8_t BuildAndSendUeContextSetupRsp(uint8_t ueIdx, uint8_t cellId)
{
   ...
   while(true)
   {
	 ...
	 break;
      ...
      {
	 DU_LOG("\n INFO   -->  F1AP : Created APER encoded buffer for UE Context Setup Request\n");
	 for(int i=0; i< encBufSize; i++)
	 {
	    printf("%x",encBuf[i]);
	 }
...
uint8_t BuildAndSendF1ResetReq()
{
   ...
   do
   {
	 ...
	 break;
      ...
      {
	 DU_LOG("\nINFO   -->  F1AP : Created APER encoded buffer for F1Reset\n");
...
uint8_t BuildAndSendF1ResetAck()
{
   ...
   do{
	 ...
	 break;
      ...
      {
	 DU_LOG("\nINFO   -->  F1AP : Created APER encoded buffer for F1ResetAck\n");
	 for(int i=0; i< encBufSize; i++)
	 {
	    printf("%x",encBuf[i]);
	 }
...
uint8_t procF1ResetReq(F1AP_PDU_t *f1apMsg)
{
   ...
   for(ieIdx=0; ieIdx<f1ResetMsg->protocolIEs.list.count; ieIdx++)
   {
	 ...
	 case ProtocolIE_ID_id_ResetType:
	    {
	       DU_LOG("\nINFO   -->  Received F1 Reset request");
	       break;
	    }
   ...
   uint32_t gnbDuUeF1apId, RrcDeliveryReport *rrcDelivery)
{
   ...
   do{
	 ...
	 break;
      ...
      {
	 DU_LOG("\nINFO   -->  F1AP : Created APER encoded buffer for RRC Delivery Msg \n");
	 for(idx=0; idx< encBufSize; idx++)
	 {
	    printf("%x",encBuf[idx]);
	 }
   ...
   for(ieIdx=0; ieIdx < gnbDuAck->protocolIEs.list.count; ieIdx++)
   {
      switch(gnbDuAck->protocolIEs.list.array[ieIdx]->id)
      {
	    ...
	    transId = gnbDuAck->protocolIEs.list.array[ieIdx]->\
	       value.choice.TransactionID;
            DU_LOG("\nINFO   -->  F1AP : GNB-DU config update acknowledgment received for transId %d", transId);
	    break;
         case ProtocolIE_ID_id_Cells_to_be_Activated_List:
	    break;
         default :
...
void F1APMsgHdlr(Buffer *mBuf)
{
   ...
   {
      DU_LOG("\nINFO   -->  F1AP : Failed while copying %d", copyCnt);
      return;
   }

   printf("\nINFO   -->  F1AP : Received flat buffer to be decoded : ");
      ...
      case F1AP_PDU_PR_successfulOutcome:
	 {
	    switch(f1apMsg->choice.successfulOutcome->value.present)
	    {
	       ...
	       case SuccessfulOutcome__value_PR_GNBDUConfigurationUpdateAcknowledge:
		  {
	       ...
	       default:
		  {
		     DU_LOG("\nERROR  -->  F1AP : Invalid type of successful outcome [%d]", f1apMsg->choice.successfulOutcome->value.present);
		     return;
		  }
      ...
      case F1AP_PDU_PR_initiatingMessage:
	 {
	    switch(f1apMsg->choice.initiatingMessage->value.present)
	    {
	       ...
	       case InitiatingMessage__value_PR_UEContextSetupRequest:
		  {
	       ...
	       default:
		  {
		     DU_LOG("\nERROR  -->  F1AP : Invalid type of initiating message[%d]",
			   f1apMsg->choice.initiatingMessage->value.present);
		     return;
		  }
