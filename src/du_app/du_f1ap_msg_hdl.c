 ...
 * @details
 *
 *    Function : getStartSymbolAndLen
 *
 *    Functionality: Function to fill the start Symbol and Symbol Len from the 
 * sliv Config Idx Table
 *
 ...
 *
 * ****************************************************************/
void fillStartSymbolAndLen(uint8_t *startSymbolIdx, uint8_t *symbolLen, uint8_t sliv)
{
   uint8_t slivIdx;

   for(slivIdx = 0; slivIdx < MAX_SLIV_CONFIG_IDX; slivIdx++)
   {
      if(sliv == slivCfgIdxTable[slivIdx][2])
      {
         *startSymbolIdx  = slivCfgIdxTable[slivIdx][0];
         *symbolLen       = slivCfgIdxTable[slivIdx][1];
         break;
      }
   }
}
   ...
   if(cuPdschCfg->pdsch_TimeDomainAllocationList)
   {
         ...
         if(timeDomAlloc->choice.setup)
	 {
	    macPdschCfg->numTimeDomRsrcAlloc  = timeDomAlloc->choice.setup->list.count;
	       ...
	       fillStartSymbolAndLen(&macPdschCfg->timeDomRsrcAllociList[timeDomIdx].startSymbol,\
