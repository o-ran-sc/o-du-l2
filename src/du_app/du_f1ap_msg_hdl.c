   ...
   if(ueSetReq->protocolIEs.list.array != NULLP)
   {
         ...
         if(ueSetReq->protocolIEs.list.array[idx])
	 {
	    switch(ueSetReq->protocolIEs.list.array[idx]->id)
	    {
		  ...
		  {
		     free(ueSetReq->protocolIEs.list.array[idx]->value.choice.BitRate.buf);
		  }
		  break;
	       }
	       default:
...
uint8_t procF1ResetReq(F1AP_PDU_t *f1apMsg)
{
   ...
   //Added free part for the memory allocated by aper_deocder
   if(f1ResetMsg->protocolIEs.list.array)
    {
       for(idx=0 ;idx < f1ResetMsg->protocolIEs.list.count ; idx++)
       {
...
uint8_t procF1SetupRsp(F1AP_PDU_t *f1apMsg)
{
	 ...
	 case ProtocolIE_ID_id_Cells_to_be_Activated_List:
	    {
	       ...
	       cellToActivate = &f1SetRspMsg->protocolIEs.list.array[idx]->value.choice.Cells_to_be_Activated_List;
	       if(cellToActivate->list.array)
	       {
                  for(int ieIdx=0; ieIdx<cellToActivate->list.count ; ieIdx++)
		  {
		     if(cellToActivate->list.array[ieIdx])
		     {
		        if(cellToActivate->list.array[0]->value.choice.Cells_to_be_Activated_List_Item.nRCGI.pLMN_Identity.buf)
		        {
			   ...
			   nRCGI.pLMN_Identity.buf);
		        ...
		        free(cellToActivate->list.array[ieIdx]);
		     }
		  }
	 ...
	 case ProtocolIE_ID_id_gNB_CU_Name:
	    {
	       ...
	       free(f1SetRspMsg->protocolIEs.list.array[idx]->value.choice.GNB_CU_Name.buf);
	       break;
	    }
	 case ProtocolIE_ID_id_GNB_CU_RRC_Version:
	    {
	       rrcVer = &f1SetRspMsg->protocolIEs.list.array[idx]->\
   ...
   //added free part for the memory allocated by aper deocder
