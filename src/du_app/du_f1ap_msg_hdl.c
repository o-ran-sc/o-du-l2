...
{
   uint8_t idx=0;
   RRCDeliveryReport_t *RrcDeliveryMsg= NULLP;

   if(f1apMsg)
   {
      if(f1apMsg->choice.initiatingMessage)
      {
	 RrcDeliveryMsg = &f1apMsg->choice.initiatingMessage->value.choice.RRCDeliveryReport;
	 if(RrcDeliveryMsg->protocolIEs.list.array)
	 {
	    for(idx =0 ;idx <RrcDeliveryMsg->protocolIEs.list.count ;
		  idx++)
	    {
	       if(RrcDeliveryMsg->protocolIEs.list.array[idx])
	       {
		  DU_FREE(RrcDeliveryMsg->protocolIEs.list.array[idx],
			sizeof(RRCDeliveryReportIEs_t));
	       }   
	    }
	    DU_FREE(RrcDeliveryMsg->protocolIEs.list.array,
		  RrcDeliveryMsg->protocolIEs.list.size);
	 }
	 DU_FREE(f1apMsg->choice.initiatingMessage,sizeof(InitiatingMessage_t));
      }
      DU_FREE(f1apMsg,
...
*
* ****************************************************************/
uint8_t BuildAndSendRrcDeliveryReport(uint32_t gnbCuUeF1apId, uint32_t gnbDuUeF1apId, RrcDeliveryReportInfo
*rrcDelivery)
{
   uint8_t             ret = RFAILED;
   uint8_t             idx    = 0;
   ...
   do{
	 ...
	 break;
      ...
      RrcDeliveryMsg = &f1apMsg->choice.initiatingMessage->value.choice.RRCDeliveryReport;
      elementCnt = 4;
      RrcDeliveryMsg->protocolIEs.list.count = elementCnt;
      RrcDeliveryMsg->protocolIEs.list.size = elementCnt * sizeof(RRCDeliveryReportIEs_t *);

      /* Initialize the F1Setup members */
      DU_ALLOC(RrcDeliveryMsg->protocolIEs.list.array, RrcDeliveryMsg->protocolIEs.list.size);
      if(RrcDeliveryMsg->protocolIEs.list.array == NULLP)
      {
	 ...
	 break;
      ...
      for(idx =0 ;idx <elementCnt; idx++)
      {
	 DU_ALLOC(RrcDeliveryMsg->protocolIEs.list.array[idx], sizeof(RRCDeliveryReportIEs_t));
	 if(RrcDeliveryMsg->protocolIEs.list.array[idx] == NULLP)
	 {
	    break;
	 }
      }
      ...
      /*GNB CU UE F1AP ID*/
      RrcDeliveryMsg->protocolIEs.list.array[idx1]->id = ProtocolIE_ID_id_gNB_CU_UE_F1AP_ID;
      RrcDeliveryMsg->protocolIEs.list.array[idx1]->criticality = Criticality_reject;
      RrcDeliveryMsg->protocolIEs.list.array[idx1]->value.present = RRCDeliveryReportIEs__value_PR_GNB_CU_UE_F1AP_ID;
      RrcDeliveryMsg->protocolIEs.list.array[idx1]->value.choice.GNB_CU_UE_F1AP_ID = gnbCuUeF1apId;

      /*GNB DU UE F1AP ID*/
      idx1++;
      RrcDeliveryMsg->protocolIEs.list.array[idx1]->id = ProtocolIE_ID_id_gNB_DU_UE_F1AP_ID;
      RrcDeliveryMsg->protocolIEs.list.array[idx1]->criticality = Criticality_reject;
      RrcDeliveryMsg->protocolIEs.list.array[idx1]->value.present = RRCDeliveryReportIEs__value_PR_GNB_DU_UE_F1AP_ID;
      RrcDeliveryMsg->protocolIEs.list.array[idx1]->value.choice.GNB_DU_UE_F1AP_ID = gnbDuUeF1apId;

      /*RRC delivery status*/
      idx1++;
      RrcDeliveryMsg->protocolIEs.list.array[idx1]->id = ProtocolIE_ID_id_RRCDeliveryStatus;
      RrcDeliveryMsg->protocolIEs.list.array[idx1]->criticality = Criticality_ignore;
      RrcDeliveryMsg->protocolIEs.list.array[idx1]->value.present = RRCDeliveryReportIEs__value_PR_RRCDeliveryStatus;
      RrcDeliveryMsg->protocolIEs.list.array[idx1]->value.choice.RRCDeliveryStatus.delivery_status =
      rrcDelivery->rrcDeliveryStatus.deliveryStatus;
      RrcDeliveryMsg->protocolIEs.list.array[idx1]->value.choice.RRCDeliveryStatus.triggering_message =
      rrcDelivery->rrcDeliveryStatus.triggeringMessage;

      /* SRB ID */ 
      idx1++;
      RrcDeliveryMsg->protocolIEs.list.array[idx1]->id = ProtocolIE_ID_id_SRBID;
      RrcDeliveryMsg->protocolIEs.list.array[idx1]->criticality = Criticality_ignore;
      RrcDeliveryMsg->protocolIEs.list.array[idx1]->value.present = RRCDeliveryReportIEs__value_PR_SRBID;
      RrcDeliveryMsg->protocolIEs.list.array[idx1]->value.choice.SRBID =  rrcDelivery->srbId;
