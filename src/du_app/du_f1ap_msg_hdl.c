...
/******************************************************************
 Deallocating the memory of function BuildRrcVer
*******************************************************************/
void FREEBuildRrcVer(RRC_Version_t *rrcVer)
{
   if(rrcVer->latest_RRC_Version.buf != NULLP)
	{
	   if(rrcVer->iE_Extensions != NULLP)
...
/*******************************************************************
 Deallocating memory for the function
  ******************************************************************/
void FREEBuildServedCellList( GNB_DU_Served_Cells_List_t *duServedCell)
{
   U8   plmnCnt=1;
   U8  servId=0;
	U8 sliceId=0;
	...
	U8   extensionCnt=1;
	U8 plmnidx=0;
   GNB_DU_Served_Cells_Item_t *srvCellItem= &duServedCell->list.array[3]->value.choice.GNB_DU_Served_Cells_Item;
   if(duServedCell->list.array!=NULLP)
   {
									     ...
									     if(srvCellItem->served_Cell_Information.servedPLMNs.list.array[servId]->iE_Extensions->list.array!=NULLP)
										  {
										     if(srvCellItem->served_Cell_Information.servedPLMNs.list.array[servId]->iE_Extensions->list.array[0]!=NULLP)
                                              ...
                                              if(srvCellItem->served_Cell_Information.nR_Mode_Info.choice.fDD!=NULLP)
																 ...
																 freqBandListNr.list.array!=NULLP)
																 {
																    if(srvCellItem->served_Cell_Information.nR_Mode_Info.choice.fDD->uL_NRFreqInfo.\
																	 freqBandListNr.list.array[0]!=NULLP)
																	 {
																			    ...
																			    if(srvCellItem->served_Cell_Information.measurementTimingConfiguration.buf!=NULLP)
																				 {
																				   if(!srvCellItem->gNB_DU_System_Information)
																					{
																					   if(duCfgParam.srvdCellLst[0].duSysInfo.mibMsg!=NULLP)
																						{
																						   if(duCfgParam.srvdCellLst[0].duSysInfo.sib1Msg!=NULLP)
																							{
																				   ...
																				   DU_FREE(srvCellItem->served_Cell_Information.measurementTimingConfiguration.buf,sizeof(U8));
																				 }
																	     ...
																	     DU_FREE(srvCellItem->served_Cell_Information.nR_Mode_Info.choice.fDD->uL_NRFreqInfo.freqBandListNr.list.\
																		  array[0],sizeof(FreqBandNrItem_t));
																	 }
																    DU_FREE(srvCellItem->served_Cell_Information.nR_Mode_Info.choice.fDD->uL_NRFreqInfo.freqBandListNr.list.array,\
														    ...
														    list.array[servId]->iE_Extensions->list.array[ieId]->extensionValue.choice.SliceSupportList.list.\
														    array[sliceId]->sNSSAI.sD->size);																					                                                                 }
													    DU_FREE(srvCellItem->served_Cell_Information.servedPLMNs.list.array[servId]->\
														 iE_Extensions->list.array[ieId]->extensionValue.choice.SliceSupportList.\
														 list.array[sliceId]->sNSSAI.sD,sizeof(OCTET_STRING_t));
													 }
												    DU_FREE(srvCellItem->served_Cell_Information.servedPLMNs.list.array[servId]->\
													 iE_Extensions->list.array[ieId]->extensionValue.choice.SliceSupportList.list.array[sliceId]->sNSSAI.sST.buf,sizeof(U8));
												 }
         ...
         if(f1SetupReq->protocolIEs.list.array != NULLP)
         {
            if(idx == elementCnt-1)
            {
               if(f1SetupReq->protocolIEs.list.array[idx]->value.choice.GNB_DU_ID.buf !=  NULLP)
               {
                   ...
                   {
						     FREEBuildRrcVer(rrcVer);
							  FREEBuildServedCellList(duServedCell);
							  DU_FREE(f1SetupReq->protocolIEs.list.array[--idx2]->value.choice.GNB_DU_Name.buf, strlen((char *)duCfgParam.duName));
                   }
                   DU_FREE(f1SetupReq->protocolIEs.list.array[--idx2]->value.choice.GNB_DU_ID.buf, sizeof(U8));
                }
