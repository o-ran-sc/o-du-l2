...
void extractUpTnlInfo(uint8_t drbId, uint8_t configType,\
   ULUPTNLInformation_ToBeSetup_List_t *tnlInfo, UpTnlCfg *upTnlInfo)
{
   ...
   upTnlInfo->numTunnels = tnlInfo->list.count;
   cmInetAddr((int8_t *)DU_IP_V4_ADDR, &ipv4_du);

   for(tnlIdx = 0; tnlIdx < tnlInfo->list.count; tnlIdx++)
   {
      if(tnlInfo->list.array[tnlIdx])
      {
         if(tnlInfo->list.array[tnlIdx]->uLUPTNLInformation.present == UPTransportLayerInformation_PR_gTPTunnel)
	 {
	    if(tnlInfo->list.array[tnlIdx]->uLUPTNLInformation.choice.gTPTunnel)
	    {
               ...
               bitStringToInt(&gtpTunnel->transportLayerAddress, &upTnlInfo->tnlCfg[tnlIdx].ulTnlAddress);
               upTnlInfo->tnlCfg[tnlIdx].dlTnlAddress = ipv4_du;
	       memcpy(&upTnlInfo->tnlCfg[tnlIdx].teId, gtpTunnel->gTP_TEID.buf, gtpTunnel->gTP_TEID.size);
	    }
	 }
      }
   }
...
uint8_t extractDrbCfg(DRBs_ToBeSetup_Item_t *drbItem, LcCfg *macLcToAdd, UpTnlCfg *upTnlInfo)
{
   ...
   extractUpTnlInfo(drbItem->dRBID, CONFIG_ADD, &drbItem->uLUPTNLInformation_ToBeSetup_List, upTnlInfo);
...
#if 1
uint8_t fillGtpTunnelforDl(GTPTunnel_t *gtpDl)
{
   ...
   gtpDl->transportLayerAddress.buf[0] = 192;
   gtpDl->transportLayerAddress.buf[1] = 168;
   gtpDl->transportLayerAddress.buf[2] = 130;
   gtpDl->transportLayerAddress.buf[3] = 81;
   gtpDl->transportLayerAddress.bits_unused = 0;
   ...
   gtpDl->gTP_TEID.buf[0] = 0;
   gtpDl->gTP_TEID.buf[1] = 0;
   gtpDl->gTP_TEID.buf[2] = 0;
   gtpDl->gTP_TEID.buf[3] = 1;
...
/*******************************************************************
 *
 * @brief Builds and sends the Drb Setup Item List
 *
 * @details
 *
 *    Function : encodeDrbSetupList
 *
 *    Functionality: Builds and sends the Drb Setup Item List
 *
 * @params[in] 
 *
 * @return ROK     - success
...
uint8_t encodeDlTnlSetupList(DLUPTNLInformation_ToBeSetup_List_t *dlTnlInfo, UpTnlCfg *tnlCfg)
{
   uint8_t ret = ROK, arrIdx = 0, eleCount = 0;

   eleCount = tnlCfg->numTunnels;
   ...
   {
      DU_LOG(" ERROR  -->  F1AP : Memory allocation for DL Tnl Setup List in encodeDlTnlSetupList()");
      ret = RFAILED;
   }
   ...
   for(arrIdx=0; arrIdx < eleCount; arrIdx++)
   {
      ...
      {
         DU_LOG(" ERROR  -->  F1AP : Memory allocation for arrIdx [%d] failed in encodeDlTnlSetupList()", arrIdx);
         ret = RFAILED;
         break;
      }
   }
...
uint8_t encodeDrbSetupList(DRBs_Setup_List_t *drbSetupList, DuUeCfg *ueCfg)
{
   uint8_t ret = ROK, arrIdx = 0, eleCount = 0;
   DRBs_Setup_Item_t *drbItem = NULLP;
   ...
   {
      DU_LOG(" ERROR  -->  F1AP : Memory allocation for DRB Setup List in encodeDrbSetupList()");
      ret = RFAILED;
   }
   ...
   for(arrIdx=0; arrIdx < eleCount; arrIdx++)
   {
      ...
      {
         DU_LOG(" ERROR  -->  F1AP : Memory allocation for arrIdx [%d] failed in encodeDrbSetupList()", arrIdx);
         ret = RFAILED;
         break;
      }
   }
   ...
   {
      drbItem = (DRBs_Setup_Item_t *)drbSetupList->list.array[arrIdx];
      drbItem->dRBID = ueCfg->upTnlInfo[arrIdx].drbId;
      encodeDlTnlSetupList(&drbItem->dLUPTNLInformation_ToBeSetup_List,\
...
uint8_t BuildAndSendUeContextSetupRsp(uint8_t ueIdx, uint8_t cellId)
{
   ...
   while(true)
   {
         ...
         if(ueCb->f1UeDb->duUeCfg.cellGrpCfg)
	 {
      ...
      ret = encodeDrbSetupList(&ueSetRsp->protocolIEs.list.array[idx]->value.choice.DRBs_Setup_List,\
               &ueCb->f1UeDb->duUeCfg);
      if(ret == RFAILED)
      {
         DU_LOG("\nERROR  -->  F1AP : Failed to encodeDrbSetupList in BuildAndSendUeContextSetupRsp()");
         freeF1UeDb(ueCb->f1UeDb);
         ueCb->f1UeDb = NULLP;
         break;
      }
