...
void buildAmInfo(AmBearerCfg *amCfgToSet, struct RLC_Config__am *rlcAmCfg)
{
    if(rlcAmCfg)
    {
       /* UL AM */
...
void buildUmBiInfo(UmBiDirBearerCfg *umBiCfgToSet, struct RLC_Config__um_Bi_Directional *rlcBiCfg)
{
    if(rlcBiCfg)
    {
       /* UL UM BI DIR Cfg */
...
void buildUmUlInfo(UmUniDirUlBearerCfg *umUlCfgToSet, struct RLC_Config__um_Uni_Directional_DL *umUlCfg)
{
   if(umUlCfg)
   {
 ...
 *
 * ****************************************************************/
void buildUmDlInfo(UmUniDirDlBearerCfg *umDlCfgToSet, struct RLC_Config__um_Uni_Directional_UL *umDlCfg)
{
   if(umDlCfg)
   {
 ...
 *
 * ****************************************************************/
void buildRlcModeCfg(uint8_t rlcMode, RlcBearerCfg *rlcDbCfg, RLC_Config_t *lcCfg)
{
   if(lcCfg)
   {
      switch(rlcMode)
...
uint8_t buildUlLcCfg(UlLcCfg *f1UlLcCfg, LogicalChannelConfig_t *ulLcCfg)
{
   if(ulLcCfg)
   {
...
uint8_t procRlcLcCfg(uint8_t rbId, uint8_t lcId, uint8_t rbType, uint8_t rlcMode,\
   uint8_t configType, RLC_Config_t *f1RlcCfg, RlcBearerCfg *lcCfg)
{
   ...
   {
      lcCfg->rlcModeCfgPres = true;
      buildRlcModeCfg(lcCfg->rlcMode, lcCfg, f1RlcCfg);
   }
   else
      lcCfg->rlcModeCfgPres = false;
   return ret;
}
 ...
 *
 * @params[in] idx, ueCfgDb
 * @return void
 *
 * ****************************************************************/
void freeDrbList(uint8_t drbs, DuUeCfg *ueCfgDb)
{
   uint8_t idx;
   for(idx = 0; idx <drbs; idx++)
   {
      freeDrbQosAndSnssaiInfo(&ueCfgDb->macLcCfg[idx]);
...
uint8_t fillMacRbCfg(uint8_t lcId, DRBs_ToBeSetup_Item_t *drbCfg, LogicalChannelConfig_t *ulLcCfg, LcCfg *lcCfg)
{
   uint8_t ret = ROK;
   ...
   DRBs_ToBeSetup_Item_t *drbItem, LogicalChannelConfig_t *ulLcCfg, LcCfg *lcCfg)
{
   ...
   if(rbType == RB_TYPE_SRB)
   {
      ret = fillMacRbCfg(lcId, NULL, ulLcCfg, lcCfg);
   }
   else if(rbType == RB_TYPE_DRB)
   {
      ret = fillMacRbCfg(lcId, drbItem, ulLcCfg, lcCfg);
   }
   return ret;
}
   ...
   DuUeCfg *ueCfgDb)
{
  ...
  for(idx = 0; idx < lcCfg->list.count; idx++)
  {
     ...
     ret = procRlcLcCfg(rbId, lcId, rbType, rlcMode, CONFIG_UNKNOWN, f1RlcCfg, &(ueCfgDb->rlcLcCfg[idx]));
     if(ret)
     {
...
uint8_t fillSrbList(SRBs_ToBeSetup_Item_t * srbItem, LcCfg *macLcToAdd, RlcBearerCfg *rlcLcToAdd)
{
   ...
   /* Filling RLC INFO */
   ret = procRlcLcCfg(srbItem->sRBID, srbItem->sRBID,\
      RB_TYPE_SRB, RLC_AM, CONFIG_ADD_TYPE, NULL, rlcLcToAdd);
   if(ret)
   { 
   ...
   /* Filling MAC INFO */
   ret = procMacLcCfg(srbItem->sRBID, RB_TYPE_SRB, CONFIG_ADD_TYPE, NULL, NULL, macLcToAdd);
   if(ret)
   { 
...
uint8_t extractSrbListToSetup(SRBs_ToBeSetup_List_t *srbCfg, DuUeCfg *ueCfgDb)
{
   ...
   if(srbCfg)
   {
	 ...
	 if(isMaxLcReached(ueCfgDb->numMacLcs) < 0)
	 { 
            DU_LOG("\nF1AP:  MAX LC Reached in MAC");
	    return RFAILED;
	 }
	 if(isMaxLcReached(ueCfgDb->numRlcLcs) < 0)
	 {
	 ...
	 memset(&ueCfgDb->rlcLcCfg[ueCfgDb->numRlcLcs], 0, sizeof(RlcBearerCfg));
         ret = fillSrbList(srbItem, &ueCfgDb->macLcCfg[ueCfgDb->numMacLcs],\
...
uint8_t fillDrbList(DRBs_ToBeSetup_Item_t *drbItem, LcCfg *macLcToAdd, RlcBearerCfg *rlcLcToAdd)
{
   ...
   /* Filling RLC INFO */
   ret = procRlcLcCfg(drbItem->dRBID, lcId, RB_TYPE_DRB, drbItem->rLCMode, CONFIG_ADD_TYPE, NULL, rlcLcToAdd);
   if(ret)
   { 
   ...
   ret = procMacLcCfg(lcId, RB_TYPE_DRB, CONFIG_ADD_TYPE, drbItem, NULL, macLcToAdd);
   if(ret)
   { 
      DU_LOG("\nF1AP:Failed at RLC LC Cfg in fillDrbList()");
      return ret;
...
uint8_t extractDrbListToSetup(DRBs_ToBeSetup_List_t *drbCfg, DuUeCfg *ueCfgDb)
{
   uint8_t ret, drbIdx;
   ...
   if(drbCfg)
   {
	 ...
	 memset(&ueCfgDb->macLcCfg[ueCfgDb->numMacLcs], 0, sizeof(LcCfg));
	 memset(&ueCfgDb->rlcLcCfg[ueCfgDb->numRlcLcs], 0, sizeof(RlcBearerCfg));
         ret = fillDrbList(drbItem, &ueCfgDb->macLcCfg[ueCfgDb->numMacLcs],\
	    &ueCfgDb->rlcLcCfg[ueCfgDb->numRlcLcs]);
	 if(ret)
	 {
...
{
   uint8_t    ret, ieIdx, ueIdx, cellIdx;
   uint32_t gnbCuUeF1apId, gnbDuUeF1apId;
   UeContextSetupDb *ueSetupDb = NULLP;
   ...
   for(ieIdx=0; ieIdx < ueSetReq->protocolIEs.list.count; ieIdx++)
   {
	 ...
	 case ProtocolIE_ID_id_ServCellIndex:
	    {
		  ...
		  {
                     ueSetupDb = &duCb.actvCellLst[cellIdx]->ueCb[ueIdx].f1UeDb;
		     ueSetupDb->cellIdx = cellIdx;
		     ueSetupDb->gnbDuUeF1apId = gnbDuUeF1apId;
		     ueSetupDb->gnbCuUeF1apId = gnbCuUeF1apId;
		  }
         ...
         case ProtocolIE_ID_id_CUtoDURRCInformation:
	    {
	       ...
	       {
                  ueSetupDb->cellGrpCfg = extractCellGrpInfo(ueSetReq->protocolIEs.list.array[ieIdx]->\
		     value.choice.CUtoDURRCInformation.iE_Extensions, &ueSetupDb->duUeCfg);
		  if(!ueSetupDb->cellGrpCfg)
		  {
	 ...
	 case ProtocolIE_ID_id_RRCContainer:
	    {
	       ueSetupDb->rrcMsgLen = ueSetReq->protocolIEs.list.array[ieIdx]->value.choice.RRCContainer.size;
	       DU_ALLOC_SHRABL_BUF(ueSetupDb->rrcMsg, ueSetupDb->rrcMsgLen);
	       if(!ueSetupDb->rrcMsg)
	       {
		  DU_LOG("\nDU APP : Memory allocation failed for RRC Msg in procUeCtxtSetupReq");
		  return RFAILED;
	       }
	       memcpy(ueSetupDb->rrcMsg, ueSetReq->protocolIEs.list.array[ieIdx]->value.choice.RRCContainer.buf,\
		     ueSetupDb->rrcMsgLen);
	       break;
	    }
	 case ProtocolIE_ID_id_RRCDeliveryStatusRequest:
	    {
	       ueSetupDb->deliveryStaReq = true;
	       break;
	    }
	 default:
	    {
   ...
   ret = duProcUeSetupRequest(ueSetupDb);
   return ret;
...
uint8_t BuildAndSendUeContextSetupRsp(uint8_t ueIdx, uint8_t cellIdx)
{
   ...
   while(true)
   {
	 ...
	 DU_FREE(ueCb->f1UeDb.cellGrpCfg, sizeof(CellGroupConfigRrc_t));
	 ueCb->f1UeDb.cellGrpCfg = NULLP;
	 if(ret)
	    break;
      }
