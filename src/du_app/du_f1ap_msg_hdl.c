...
*
* ****************************************************************/
uint8_t BuildTagConfig(struct TAG_Config *tag_Config)
{
	struct TAG_Config__tag_ToAddModList *tagList;
   uint8_t                     idx, elementCnt;

   DU_ALLOC(tag_Config->tag_ToAddModList, sizeof(struct TAG_Config__tag_ToAddModList));
	if(!tag_Config->tag_ToAddModList)
	{
	...
	tagList = tag_Config->tag_ToAddModList;
...
uint8_t BuildMacCellGrpCfg(MAC_CellGroupConfig_t *macCellGrpCfg)
{
	...
	DU_ALLOC(macCellGrpCfg->schedulingRequestConfig,
