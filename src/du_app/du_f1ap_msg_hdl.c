 ...
 *
 * ****************************************************************/
void freeMacLcCfg(LcCfg *macLcCfg)
{
  freeDrbQosAndSnssaiInfo(macLcCfg);
  memset(macLcCfg, 0, sizeof(LcCfg));
}
...
void freeDuUeCfg(DuUeCfg *ueCfg)
{
   ...
   {
      memset(&ueCfg->rlcLcCfg[lcIdx], 0, sizeof(RlcBearerCfg));
   }
...
void freeF1UeDb(UeContextSetupDb *f1UeDb)
{
   ...
   if(f1UeDb->maxAggrBitRate)
   {
      DU_FREE_SHRABL_BUF(DU_APP_MEM_REGION, DU_POOL, f1UeDb->maxAggrBitRate, sizeof(MaxAggrBitRate));
   }
...
void extractAmInfo(AmBearerCfg *amCfgToSet, struct RLC_Config__am *rlcAmCfg)
{
    if(rlcAmCfg)
    {
       /* UL AM */
...
void extractUmBiInfo(UmBiDirBearerCfg *umBiCfgToSet, struct RLC_Config__um_Bi_Directional *rlcBiCfg)
{
    if(rlcBiCfg)
    {
       /* UL UM BI DIR Cfg */
...
void extractUmUlInfo(UmUniDirUlBearerCfg *umUlCfgToSet, struct RLC_Config__um_Uni_Directional_DL *umUlCfg)
{
   if(umUlCfg)
   {
 ...
 *
 * ****************************************************************/
void extractUmDlInfo(UmUniDirDlBearerCfg *umDlCfgToSet, struct RLC_Config__um_Uni_Directional_UL *umDlCfg)
{
   if(umDlCfg)
   {
   ...
   if(lcCfg)
   {
      switch(rlcMode)
      {
         case RLC_AM :
            {
               if(lcCfg->choice.am)
                  extractAmInfo(&rlcDbCfg->u.amCfg, lcCfg->choice.am);
               break;
            }
         case RLC_UM_BI_DIRECTIONAL :
            {
               if(lcCfg->choice.um_Bi_Directional)
                     extractUmBiInfo(&rlcDbCfg->u.umBiDirCfg, lcCfg->choice.um_Bi_Directional);
               break;
            }
         case RLC_UM_UNI_DIRECTIONAL_UL :
            {
               if(lcCfg->choice.um_Uni_Directional_UL)
                     extractUmUlInfo(&rlcDbCfg->u.umUniDirUlCfg, lcCfg->choice.um_Uni_Directional_DL);
               break;
            }
         case RLC_UM_UNI_DIRECTIONAL_DL :
            {
               if(lcCfg->choice.um_Uni_Directional_DL)
                     extractUmDlInfo(&rlcDbCfg->u.umUniDirDlCfg, lcCfg->choice.um_Uni_Directional_UL);
               break;
            }
         default:
...
uint8_t extractUlLcCfg(UlLcCfg *f1UlLcCfg, LogicalChannelConfig_t *ulLcCfg)
{
   if(ulLcCfg)
   {
...
uint8_t extractMacRbCfg(uint8_t lcId, DRBs_ToBeSetup_Item_t *drbCfg, LogicalChannelConfig_t *ulLcCfg, LcCfg *lcCfg)
{
   ...
   if(drbCfg)
   {
      ...
      if(ret)
      {
         DU_LOG("F1AP: Failed at building Drb Qos in extractDrbCfg()");
	 return ret;
      }
   }
   else
   ...
   if(ulLcCfg)
   {
      ...
      ret = extractUlLcCfg(&lcCfg->ulLcCfg, ulLcCfg);
   }
   else
...
uint8_t procSrbListToSetup(SRBs_ToBeSetup_Item_t * srbItem, LcCfg *macLcToAdd, RlcBearerCfg *rlcLcToAdd)
{
   ...
   if(ret)
   { 
      DU_LOG("\nF1AP:Failed at RLC LC Cfg in procSrbListToSetup()");
      return ret;
   }
...
uint8_t procDrbListToSetup(DRBs_ToBeSetup_Item_t *drbItem, LcCfg *macLcToAdd, RlcBearerCfg *rlcLcToAdd)
{
   uint8_t ret, lcId;
   ret= ROK;
...
uint8_t extractDrbListToSetup(DRBs_ToBeSetup_List_t *drbCfg, DuUeCfg *ueCfgDb)
{
   ...
   if(drbCfg)
   {
	 ...
	 memset(&ueCfgDb->macLcCfg[ueCfgDb->numMacLcs], 0, sizeof(LcCfg));
	 memset(&ueCfgDb->rlcLcCfg[ueCfgDb->numRlcLcs], 0, sizeof(RlcBearerCfg));
         ret = procDrbListToSetup(drbItem, &ueCfgDb->macLcCfg[ueCfgDb->numMacLcs],\
	    &ueCfgDb->rlcLcCfg[ueCfgDb->numRlcLcs]);
	 ueCfgDb->numRlcLcs++;
	 ueCfgDb->numMacLcs++;
	 if(ret)
...
uint8_t procF1UeContextSetupReq(F1AP_PDU_t *f1apMsg)
{
   ...
   ret = ROK;
   DU_ALLOC(ueSetupDb, sizeof(UeContextSetupDb));
   if(!ueSetupDb)
   {
      DU_LOG("\n Memory Alloc Failed at procF1UeContextSetupReq()");
      return RFAILED;
   }
   
   memset(ueSetupDb, 0, sizeof(UeContextSetupDb));
	 ...
	 case ProtocolIE_ID_id_ServCellIndex:
	    {
		  ...
		  {
                     duCb.actvCellLst[cellIdx]->ueCb[ueIdx].f1UeDb = ueSetupDb;
