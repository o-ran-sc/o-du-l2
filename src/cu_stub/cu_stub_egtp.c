...
S16 cuEgtpSrvOpenReq(Pst *pst)
{
   ...
   for(destIdx=0; destIdx < egtpCb.egtpCfg.numDu; destIdx++)
   {
      ...
      if(ret != ROK)
      {
         DU_LOG("\nPRIYA ERROR  -->  EGTP : Failed to bind socket");
         return RFAILED;
      }
      ...
      if(ret != ROK)
      {  
         DU_LOG("\nBORLA ERROR  -->  EGTP : Failed to bind socket");
         return RFAILED;
      }
...
uint16_t cuEgtpDatReq(uint8_t duCount, uint8_t teId)
{
   ...
   for(duIdx = 0; duIdx<duCount; duIdx++)
   {
      ...
      ret = BuildEgtpMsg(duIdx, &egtpMsg);
      if(ret != ROK)
      {
         DU_LOG("\nERROR  -->  EGTP : Failed to build EGTP Msg");
         return RFAILED;
      }
      cuEgtpSendMsg(duIdx, egtpMsg.msg);
...
S16 BuildAppMsg(uint32_t duIdx,EgtpMsg  *egtpMsg)
{
   ...
   ipv4Hdr.hdrVer = 0x45;
   ipv4Hdr.proto = 1;
   ipv4Hdr.srcAddr = CM_INET_NTOH_UINT32(egtpCb.egtpCfg.egtpAssoc[duIdx].localIp.ipV4Addr);
   ipv4Hdr.destAddr = CM_INET_NTOH_UINT32(egtpCb.egtpCfg.egtpAssoc[duIdx].destIp.ipV4Addr);
...
S16 BuildEgtpMsg(uint32_t duIdx, EgtpMsg *egtpMsg)
{
   ...
   cmHashListFind(&(egtpCb.dstCb[duIdx].teIdLst), (uint8_t *)&(egtpMsg->msgHdr.teId), sizeof(uint32_t), 0, (PTR *)&teidCb);
   if(teidCb == NULLP)
   {
...
S16 cuEgtpSendMsg(uint32_t duIdx, Buffer *mBuf)
{
   ...
   dstAddr.address = egtpCb.dstCb[duIdx].dstIp;
 
   ret = cmInetSendMsg(&(egtpCb.dstCb[duIdx].sendTptSrvr.sockFd), &dstAddr, &info, mBuf, &txLen, CM_INET_NO_FLAG);
