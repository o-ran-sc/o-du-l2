...
{
  DU_LOG("\n\nEGTP : Initializing");
  cmMemset ((uint8_t *)&egtpCb, 0, sizeof(EgtpGlobalCb));
...
S16 cuEgtpCfgReq()
{
   ...
   cmMemcpy((uint8_t *)&egtpCb.egtpCfg, (uint8_t *)&cuCfgParams.egtpParams, (PTR)sizeof(EgtpParams));

   egtpCb.recvTptSrvr.addr.address = CM_INET_NTOH_uint32_t(egtpCb.egtpCfg.localIp.ipV4Addr);
   egtpCb.recvTptSrvr.addr.port = EGTP_DFLT_PORT;

   egtpCb.dstCb.dstIp = CM_INET_NTOH_uint32_t(egtpCb.egtpCfg.destIp.ipV4Addr);
   egtpCb.dstCb.dstPort = egtpCb.egtpCfg.destPort;
   egtpCb.dstCb.sendTptSrvr.addr.address = CM_INET_NTOH_uint32_t(egtpCb.egtpCfg.localIp.ipV4Addr);
   egtpCb.dstCb.sendTptSrvr.addr.port = egtpCb.egtpCfg.localPort;
   egtpCb.dstCb.numTunn = 0;

   ret = cmHashListInit(&(egtpCb.dstCb.teIdLst), 1024, sizeof(EgtpTeIdCb), FALSE, CM_HASH_KEYTYPE_uint32_tMOD, CU_APP_MEM_REG, CU_POOL);
...
S16 cuEgtpTnlAdd(EgtpTnlEvt tnlEvt)
{
   ...
   cmMemset((uint8_t *)teidCb, 0, sizeof(EgtpTeIdCb));
   ...
   cmMemset((uint8_t*)&preDefHdr, 0, sizeof(EgtpMsgHdr));
...
S16 BuildAppMsg(EgtpMsg  *egtpMsg)
{
   ...
   ipv4Hdr.hdrVer = 0x45;
   ipv4Hdr.proto = 1;
   ipv4Hdr.srcAddr = CM_INET_NTOH_uint32_t(egtpCb.egtpCfg.localIp.ipV4Addr);
   ipv4Hdr.destAddr = CM_INET_NTOH_uint32_t(egtpCb.egtpCfg.destIp.ipV4Addr);
