...
*
* ****************************************************************/
void buildSliceList(SliceSupportList_t *sliceSupportList)
{
   uint8_t sdIdx=0, sliceListIdx = 0;
      ...
      if(sliceSupportList->list.array)
      {
         for(sliceListIdx=0; sliceListIdx<sliceSupportList->list.count; sliceListIdx++)
         {
            ...
            if(sliceSupportList->list.array[sliceListIdx])
            {
               ...
               {
                  cuCfgParams.snssaiList[sliceListIdx]->sst = sliceSupportList->list.array[sliceListIdx]->\
                  sNSSAI.sST.buf[0];
               }
               for(sdIdx =0; sdIdx<SD_SIZE; sdIdx++)
               {
                  if(sliceSupportList->list.array[sliceListIdx]->sNSSAI.sD->buf[sdIdx])
                  {
                     cuCfgParams.snssaiList[sliceListIdx]->sd[sdIdx] = sliceSupportList->list.array[sliceListIdx]->\
                                                                      sNSSAI.sD->buf[sdIdx];
                  }
               }
            }
...
{
   uint8_t ieIdx = 0, plmnidx=0;
   uint8_t srvPlmnIdx=0, ieExtensionsLstIdx=0;
   bool sliceInfoUpdating = FALSE;
   F1SetupRequest_t *f1SetupReq = NULLP;
   ServedPLMNs_List_t *srvPlmn = NULLP;
   GNB_DU_Served_Cells_Item_t *srvCellItem = NULLP; 
   ProtocolExtensionContainer_4624P3_t **ieExtend = NULLP;
                     ...
                     if(duServedCell->list.array[plmnidx])
                     {
                        switch(duServedCell->list.array[plmnidx]->id)
                        {
                           case ProtocolIE_ID_id_GNB_DU_Served_Cells_Item:
                           {
                              ...
                              srvPlmn = &srvCellItem->served_Cell_Information.servedPLMNs; 
                                  ...
                                  for(srvPlmnIdx=0; srvPlmnIdx<srvPlmn->list.count; srvPlmnIdx++)
                                  {
                                     if(srvPlmn->list.array[srvPlmnIdx])
                                     {
                                        ...
                                        if(*ieExtend)
                                        {
                                           ...
                                           {
                                              for(ieExtensionsLstIdx = 0;ieExtensionsLstIdx<(*ieExtend)->list.count;
                                                    ieExtensionsLstIdx++)
                                              {
                                                 if((*ieExtend)->list.array[ieExtensionsLstIdx])
                                                 {
                                                     switch((*ieExtend)->list.array[ieExtensionsLstIdx]->id )
                                                     {
                                                        ...
                                                        {
                                                           buildSliceList(&(*ieExtend)->list.array[ieExtensionsLstIdx]->\
                                                           extensionValue.choice.SliceSupportList);
                                                           sliceInfoUpdating = true;
                                                           break;   
                                                        }
                                                     }
                                                 }
