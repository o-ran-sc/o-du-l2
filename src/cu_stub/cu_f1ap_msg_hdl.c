...
S16 BuildAndSendF1SetupRsp()
{
   ...
   if(cuName->buf == NULLP)
   {
      ...
      {
	 CU_FREE(f1SetupRsp->protocolIEs.list.array[idx1],\
	       sizeof(F1SetupResponseIEs_t));
      }
      CU_FREE(f1SetupRsp->protocolIEs.list.array,\
	    elementCnt * sizeof(F1SetupResponseIEs_t *));
   ...
   for(idx1=0; idx1<cellCnt; idx1++)
   {
      CU_ALLOC(cellToActivate->list.array[idx1],sizeof(struct Cells_to_be_Activated_List_ItemIEs ));
      if(cellToActivate->list.array[idx1] == NULLP)
      {
	 ...
	 Cells_to_be_Activated_List_Item.nRCGI.pLMN_Identity.buf == NULLP)
   {
      ...
      for(idx1=0; idx1<cellCnt; idx1++)
      {
	 CU_FREE(cellToActivate->list.array[idx1],\
	       sizeof(struct Cells_to_be_Activated_List_ItemIEs ));
      }
   ...
   {  
      CU_FREE(cuName->buf, sizeof(cuName->size));
      for(idx1=0; idx1<elementCnt; idx++)
      {
	 CU_FREE(f1SetupRsp->protocolIEs.list.array[idx1], \
	       sizeof(F1SetupResponseIEs_t));
      } 
...
/*******************************************************************
*
* @brief build ansld ans send f1reset msg from cu to du 
*
* @details
*
*    Function : BuildAndSendF1ResetMsg
*
*    Functionality: build and send f1reset msg from cu to du
*
* @return ROK     - success
*         RFAILED - failure
*
...
uint8_t BuildAndSendF1ResetReq()
{
    ...
    asn_enc_rval_t   encRetVal;
    DU_LOG("\nF1AP : Building F1 Reset request from cu to du\n");
    do
    {
       CU_ALLOC(f1apMsg, sizeof(F1AP_PDU_t));
       if(f1apMsg == NULLP)
	  ...
	  break;
       ...
       {
          DU_LOG("\nF1AP : Memory allocation failed for f1ResetMsg");
	  break;
       }
       for(idx=0; idx<elementCnt; idx++)
       {
           ...
           {
	      DU_LOG("\nF1AP : Memory allocation failed for f1ResetMsg array");
              break;
           }
       }
       ...
       if(SendF1APMsg(CU_APP_MEM_REG, CU_POOL) != ROK)
       {
          DU_LOG("\nF1AP : Sending F1 Reset request from cu to du failed");
          break;
       }
...
/****************************************************************
 * @brief Build And Send F1ResetAck from du to cu
 *
 * @details
 *
...
uint8_t BuildAndSendF1ResetAck()
{
   ...
   asn_enc_rval_t         encRetVal;
   DU_LOG("\nF1AP : Building F1 Reset Acknowledgment when reset request revied from du to cu\n");
   ...
   do{
	   ...
	   break;
        ...
        {
	   DU_LOG("\nF1AP : Created APER encoded buffer for F1ResetAck from du to cu\n");
	   for(int i=0; i< encBufSize; i++)
	   {
	      printf("%x",encBuf[i]);
	   }
        ...
        if(SendF1APMsg(CU_APP_MEM_REG, CU_POOL) != ROK)
        {
	   DU_LOG("\nF1AP : Sending F1 Reset Response from du to cu failed");
	   break;
        }
...
void F1APMsgHdlr(Buffer *mBuf)
{
   ...
   switch(f1apMsg->present)
   {
      case F1AP_PDU_PR_initiatingMessage:
      {
	    ...
	    {
	        DU_LOG("\nF1AP : F1 reset request received from du to cu");
