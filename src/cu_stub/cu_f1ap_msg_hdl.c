         ...
         if(dl_CCCH_Msg.message.choice.c1->choice.rrcSetup != NULLP)
         {
                ...
                criticalExtensions.choice.rrcSetup != NULLP)
            {
               ...
               if(!ret)
                  ...
                  {
                     memcpy(rrcContainer->buf, encBuf, rrcContainer->size);
                  }
					}
            }
         }
...
S16 BuildAndSendDLRRCMessageTransfer()
{
   ...
   /* ExecuteDuplication */
   idx++;
   dlRRCMsg->protocolIEs.list.array[idx]->id          = ProtocolIE_ID_id_ExecuteDuplication;
   dlRRCMsg->protocolIEs.list.array[idx]->criticality  = Criticality_ignore;
   dlRRCMsg->protocolIEs.list.array[idx]->value.present = \
                        DLRRCMessageTransferIEs__value_PR_ExecuteDuplication;
   dlRRCMsg->protocolIEs.list.array[idx]->value.choice.ExecuteDuplication = 0;
   ...
   /*TODO: Remove the below hardcoding, after the encodig fix for BuildDuToCuContainer */
   char rrcBuf[8]= {0x0020, 0x0040, 0x0004, 0x0008, 0x004a, 0x00e2};
   uint8_t rrcBufLen = 8;
   ...
   CU_ALLOC(dlRRCMsg->protocolIEs.list.array[idx]->value.choice.RRCContainer.buf,\
            rrcBufLen);
   if(dlRRCMsg->protocolIEs.list.array[idx]->value.choice.RRCContainer.buf != NULLP)
   {
      memcpy(dlRRCMsg->protocolIEs.list.array[idx]->value.choice.RRCContainer.buf,\
       rrcBuf, dlRRCMsg->protocolIEs.list.array[idx]->value.choice.RRCContainer.size);
   }
      ...
      switch(initULRRCMsg->protocolIEs.list.array[idx]->id)
      {
         ...
         case ProtocolIE_ID_id_DUtoCURRCContainer:
                  ...
                  .DUtoCURRCContainer.buf != NULLP)
               {
                  ...
                  CU_ALLOC(f1apMsgDb.duToCuContainer.buf, f1apMsgDb.duToCuContainer.size);
