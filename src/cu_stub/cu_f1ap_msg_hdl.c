 ...
 *
 * ****************************************************************/
uint8_t BuildAndSendPagingMsg(uint64_t gsTmsi)
{
   bool       memAllocFailed = false;
   uint8_t    ieIdx = 0, elementCnt = 0, cellIdx=0, ret = RFAILED;
   uint64_t   val = 0;
   uint16_t   ueId = 0, duId = 1, duIdx = 0;
   ...
   while(true)
   {
      ...
      paging->protocolIEs.list.array[ieIdx]->value.choice.UEIdentityIndexValue.choice.indexLength10.bits_unused = unusedBitsInUeId;

      /*As per 3gpp Spec 38.304 Sec 7.1: UE_ID: 5G-S-TMSI mod 1024*/
      ueId = gsTmsi % 1024;
      val = ueId << unusedBitsInUeId;
      fillBitString(&paging->protocolIEs.list.array[ieIdx]->value.choice.UEIdentityIndexValue.choice.indexLength10, unusedBitsInUeId, totalByteInUeId, &val);
      ...
      paging->protocolIEs.list.array[ieIdx]->value.choice.PagingIdentity.choice.cNUEPagingIdentity->choice.fiveG_S_TMSI.bits_unused = unusedBitsInTmsi;
      val = gsTmsi << unusedBitsInTmsi;
      ...
      , unusedBitsInTmsi, totalByteInTmsi, &val);
