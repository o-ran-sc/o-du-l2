 ...
 *
 * ****************************************************************/
uint8_t fillRbCfgSrb(uint8_t srbId, SRB_ToAddModList_t *bearerCfg)
{
   uint8_t elementCnt;
   uint8_t idx;
   uint8_t ied;
...
uint8_t fillRRCSetupIE(RRCSetup_IEs_t *rrcSetupIE)
{
   ...
   if(rrcSetupIE)
   {
      ...
      {
	 ret = fillRbCfgSrb(SRB1, rrcSetupIE->radioBearerConfig.srb_ToAddModList);
      }		
      if(!ret)
      {
...
uint8_t fillDlDCcchRrcMsg(RRCContainer_t *rrcContainer)
{
   uint8_t ret = ROK;
   uint16_t idx2;
...
uint8_t fillCnAssoc(struct DRB_ToAddMod__cnAssociation *cnAssoc)
{
   ...
   if(cnAssoc->present == DRB_ToAddMod__cnAssociation_PR_sdap_Config)
   {
      ...
      if(cnAssoc->choice.sdap_Config)
      {
         ...
         {
            DU_LOG("\n Memory allocated mappedQoS_FlowsToAdd");
            ret = fillQosFlowsToAdd(cnAssoc->choice.sdap_Config->mappedQoS_FlowsToAdd);
         }
         else
         {
            DU_LOG("\nF1AP: Mem alloc failed at mappedQoS_FlowsToAdd in fillCnAssoc()");
 ...
 *
 * ****************************************************************/
uint8_t fillRbCfgDrb(uint8_t drbId, DRB_ToAddModList_t *drbCfg)
{
...
uint8_t fillRrcReconfigIE(RRCReconfiguration_IEs_t *rrcReconfigMsg)
{
   ...
   if(rrcReconfigMsg->radioBearerConfig)
   {
      ...
      {
         ret = fillRbCfgSrb(SRB2, rrcReconfigMsg->radioBearerConfig->srb_ToAddModList);
	 ...
	 {
            ret = fillRbCfgDrb(DRB1, rrcReconfigMsg->radioBearerConfig->drb_ToAddModList);
	    if(ret)
	    {
               DU_LOG("\n F1AP: Failed at fillRbCfgDrb())");
	 ...
	 if(dl_DCCH_Msg.message.choice.c1->choice.rrcReconfiguration != NULLP)
	 {
		  ...
		  criticalExtensions.choice.rrcReconfiguration != NULLP)
	    {
		  ...
		  else
		  {
	       ...
	       {
	          DU_LOG("\nF1AP: Failed at fillRrcReconfigIE()");
	       }
	    }
	    else
	    {
...
uint8_t	BuildDLRRCContainer(uint8_t rrcMsgType, RRCContainer_t *rrcContainer)
{
   ...
   { 
      ret = fillDlDCcchRrcMsg(rrcContainer);
      if(ret)
         DU_LOG("\n F1AP: Failed at fillDlDCcchRrcMsg()");
   }
   else if(rrcMsgType == RRC_RECONFIG)
   {
      ret = fillDlDCchRrcMsg(rrcContainer);
      if(ret)
         DU_LOG("\n F1AP: Failed at fillDlDCchRrcMsg()");
   }
   return ret;
}
