 ...
 *
 * ****************************************************************/
uint8_t BuildPagingCellList(PagingCell_list_t  *pagingCelllist,  CuCellCb *cellCb)
{
   uint8_t cellIdx =0;
   PagingCell_ItemIEs_t *pagingCellItemIes; 
   PagingCell_Item_t *pagingCellItem;

   pagingCelllist->list.count = 1;
   pagingCelllist->list.size = sizeof(PagingCell_ItemIEs_t*);
   ...
   BuildNrcgi(&pagingCellItem->nRCGI, cellCb->nrCellId);
 ...
 *
 * ****************************************************************/
uint8_t BuildAndSendPagingMsg(uint32_t duId, uint8_t ueId)
{
   bool       memAllocFailed = false;
   uint8_t    ieIdx = 0,elementCnt = 0, bits_unused=6, bufLen=0;
   uint8_t    val = 0, cellIdx=0;
   uint8_t    unusedByteInUeId = 6, unusedByteInTmsi = 0;
   uint8_t    totalBitInUeId = 2, totalBitInTmsi = 6;
   F1AP_PDU_t *f1apMsg = NULLP;
   ...
   while(true)
   {
      ...
      paging->protocolIEs.list.array[ieIdx]->value.choice.UEIdentityIndexValue.choice.indexLength10.size = totalBitInUeId*sizeof(uint8_t);
      ...
      paging->protocolIEs.list.array[ieIdx]->value.choice.UEIdentityIndexValue.choice.indexLength10.bits_unused = unusedByteInUeId;
      val = ueId << unusedByteInUeId;
      fillBitString(&paging->protocolIEs.list.array[ieIdx]->value.choice.UEIdentityIndexValue.choice.indexLength10, unusedByteInUeId, totalBitInUeId, val); 
      ...
      paging->protocolIEs.list.array[ieIdx]->value.choice.PagingIdentity.choice.cNUEPagingIdentity->choice.fiveG_S_TMSI.size = totalBitInTmsi*sizeof(uint8_t);
      ...
      paging->protocolIEs.list.array[ieIdx]->value.choice.PagingIdentity.choice.cNUEPagingIdentity->choice.fiveG_S_TMSI.bits_unused = unusedByteInTmsi;
      val = ueId << unusedByteInTmsi;
      ...
      , unusedByteInTmsi, totalBitInTmsi, val);
      ...
      paging->protocolIEs.list.array[ieIdx]->value.choice.PagingPriority = PagingPriority_priolevel1;
      ...
      duDb = getDuDb(duId);
      if(BuildPagingCellList(&paging->protocolIEs.list.array[ieIdx]->value.choice.PagingCell_list, &duDb->cellCb) != ROK)
      {
