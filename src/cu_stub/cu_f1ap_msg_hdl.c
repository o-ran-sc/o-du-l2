...
uint8_t BuildDRBSetup(CuUeCb *ueCb, DRBs_ToBeSetup_List_t *drbSet)
{
   ...
   for(idx=0; idx<drbCnt; idx++)
   {
      ...
      if(ueCb->state == HANDOVER_IN_PROGRESS)
      {
         ...
         CU_ALLOC(extIe, sizeof(DRBs_ToBeSetup_ItemExtIEs_t));
         if(!extIe)
         {
         DU_LOG("\nERROR  -->  F1AP : Failed to allocated memory to extension IE ");
         return RFAILED;
         }
         extIe->id = ProtocolIE_ID_id_DLPDCPSNLength;
 ...
 *
 * ****************************************************************/
uint8_t fillMeasTimingCfg(MeasurementTimingConfiguration_IEs_t *measTimingCfg)
{
   uint8_t elementCnt = 0;
...
uint8_t fillMeasTimingConfigBuf(MeasConfig_t *measTimingConfigBuf)
{
   uint8_t          ret = RFAILED;
   asn_enc_rval_t   encRetVal;
   struct MeasurementTimingConfiguration measTimingConfig;

   while(true)
   {
      measTimingConfig.criticalExtensions.present = MeasurementTimingConfiguration__criticalExtensions_PR_c1;
      CU_ALLOC(measTimingConfig.criticalExtensions.choice.c1, sizeof(struct MeasurementTimingConfiguration__criticalExtensions__c1));
      ...
      measTimingConfig.criticalExtensions.choice.c1.present = MeasurementTimingConfiguration__criticalExtensions__c1_PR_measTimingConf;

      CU_ALLOC(measTimingConfig.criticalExtensions.choice.c1.choice.measTimingConf, sizeof(struct MeasurementTimingConfiguration_IEs));
