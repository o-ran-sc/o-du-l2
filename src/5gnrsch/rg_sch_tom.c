...
{ 
  ...
  cmMemset((uint8_t *)pucchReqInfo->hqInfo.hqRes,0xff,sizeof(uint16_t)*TFU_MAX_HQ_RES);
#ifdef LTEMAC_SPS
...
#ifdef TFU_ALLOC_EVENT_NO_INIT
     cmMemset((uint8_t *)&pucchRecpInfo->t.pucchRecpReq, 0, sizeof(TfuUePucchRecpReq));
      pucchRecpInfo->type = TFU_RECP_REQ_PUCCH;
#endif      
      /*Fill SR params*/
...
#endif
{
   ...
   if(pucchInfo == NULLP)
   {
      ...
      cmMemset((uint8_t *)pucchInfo->pucchRecpInfo->t.pucchRecpReq.hqInfo.hqRes,0xff,sizeof(uint16_t)*TFU_MAX_HQ_RES);
#ifdef TFU_ALLOC_EVENT_NO_INIT
      cmMemset((uint8_t *)&pucchInfo->pucchRecpInfo->t.pucchRecpReq, 0, sizeof(TfuUePucchRecpReq));
#endif
