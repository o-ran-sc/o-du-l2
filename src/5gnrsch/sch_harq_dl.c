...
uint8_t schDlGetAvlHqProcess(SchCellCb *cellCb, SchUeCb *ueCb, SchDlHqProcCb **hqP)
{
   ...
   *hqP = tmp;
   ...
   HQ_LOG(0,tmp->procId,"\nANKIT schDlGetAvlHqProcess proc[%2d]\n", tmp->procId);
   return ROK;
}
/*
...
{
   HQ_LOG(0,hqP->procId,"\nANKIT schDlReleaseHqProcess proc[%2d]\n", hqP->procId);
...
{
   HQ_LOG(0, hqP->procId,"\nANKIT schDlReleaseHqPTb tbIdx[%d] togNdi[%d]\n",tbIdx, togNdi);  
   if (TRUE == togNdi)
   {
...
void schMsg4FeedbackUpdate(SchDlHqProcCb *hqP, uint8_t fdbk)
{
   ...
   {
      HQ_LOG(0, hqP->procId,"\nANKIT schMsg4FeedbackUpdate ACK\n");
   ...
   {
      HQ_LOG(0, hqP->procId,"\nANKIT schMsg4FeedbackUpdate NACK proc[%2d] txCntr[%2d] maxTx[%2d]\n",\
         hqP->procId, hqP->tbInfo[0].txCntr , hqP->hqEnt->cell->cellCfg.schHqCfg.maxMsg4HqTx);
      if( hqP->tbInfo[0].txCntr >= hqP->hqEnt->cell->cellCfg.schHqCfg.maxMsg4HqTx)
      {
         HQ_LOG(0, hqP->procId,"\nANKIT schMsg4FeedbackUpdate max tx reached\n");
         schDlReleaseHqProcess(hqP);
   ...
   hqP->hqEnt->ue->retxMsg4HqProc = hqP->hqEnt->ue->msg4Proc;
}
...
void schDlHqFeedbackUpdate(SchDlHqProcCb *hqP, uint8_t fdbk1, uint8_t fdbk2)
{
   ...
   for (tbIdx = 0; tbIdx <2; tbIdx++)
   {
      ...
      if (TRUE == hqP->tbInfo[tbIdx].isEnabled)
      {
         ...
         {
            HQ_LOG(0,hqP->procId,"\nANKIT schDlHqFeedbackUpdate proc[%2d] txcntr[%2d] maxTx[%2d]\n",\
               hqP->procId, hqP->tbInfo[tbIdx].txCntr , hqP->maxHqTxPerHqP);
