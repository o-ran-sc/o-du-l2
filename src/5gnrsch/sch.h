...
typedef struct schUlHqProcCb
{
   ...
   uint8_t           strtSymbl;
   uint8_t           numSymbl;
//SCH SPC : LC PRB estimate stores LCs to be scheduled
   //SchLcPrbEstimate  ulLcPrbEst; /*UL PRB Alloc Estimate among different LC*/
   void              *schSpcUlHqProcCb;  /*Scheduler specific Haqr Proc CB*/
...
struct schDlHqProcCb
{
   ...
   uint8_t           k1;
//SCH SPC : Since this contains list of LCs to be scheduled and requested resources   
   //SchLcPrbEstimate  dlLcPrbEst; /*DL PRB Alloc Estimate among different LC*/
   void              *schSpcDlHqProcCb;  /*Scheduler specific Haqr Proc CB*/
...
typedef struct schUeCb
{
   ...
   SchHqDlMap   **hqDlmap;
   SchHqUlMap   **hqUlmap;
//SCH SPC : HQ to be retransmitted are put in this list to be scheduled in future slot
   void       *schSpcUeCb;
#ifdef NR_DRX
   bool           ueDrxInfoPres;
   SchDrxUeCb     drxUeCb;
...
typedef struct schAllApis
{
   ...
   void (* SchCellCfgReq) (SchCellCb *cellCb);
   void (* SchCellDeleteReq) (SchCellCb *cellCb);
   void (* SchAddUeConfigReq) (SchUeCb  *ueCb);
   void (* SchModUeConfigReq) (SchUeCb  *ueCb);
   void (* SchUeDeleteReq) (SchUeCb  *ueCb);
   void (* SchCrcInd)(SchCellCb *cellCb, uint8_t ueId);
   void (* SchRachInd)(SchCellCb *cellCb, uint8_t ueId);
   void (* SchDlRlcBoInfo) (SchCellCb *cellCb, uint8_t ueId);
   void (* SchSrUciInd) (SchCellCb *cellCb, uint8_t ueId);
   void (* SchBsr) (SchCellCb *cellCb, uint8_t ueId);
   ...
   uint8_t (* SchAddUeToSchedule) (SchCellCb *cellCb, uint8_t ueId);
   void (* SchRemoveUeFrmScheduleLst)(SchCellCb *cell, CmLList *node);
   void (* SchInitDlHqProcCb)(SchDlHqProcCb *hqP);
   void (* SchInitUlHqProcCb)(SchUlHqProcCb *hqP);
   void (* SchFreeDlHqProcCb)(SchDlHqProcCb *hqP);
   void (* SchFreeUlHqProcCb)(SchUlHqProcCb *hqP);
   void (* SchDlScheduling)(SchCellCb *cell, SlotTimingInfo *slotInd, Inst schInst);
}SchAllApis;
/**
 * @brief
