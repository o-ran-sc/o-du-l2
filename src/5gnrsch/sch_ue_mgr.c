...
void fillSchUlLcCtxt(SchUlLcCtxt *ueCbLcCfg, SchLcCfg *lcCfg)
{
   ...
   if(lcCfg->snssai)
   {
     if(ueCbLcCfg->snssai == NULLP)/*In CONFIG_MOD case, no need to allocate SNSSAI memory again*/
     {
        SCH_ALLOC(ueCbLcCfg->snssai, sizeof(SchSnssai));
     }
     memcpy(ueCbLcCfg->snssai, lcCfg->snssai,sizeof(SchSnssai));
...
void updateSchUlCb(uint8_t delIdx, SchUlCb *ulInfo)
{
   ...
   if(ulInfo->ulLcCtxt[ulInfo->numUlLc].snssai != NULLP) /*last index of ulLcCtxt(before deletion) should be void*/
   {
...
void updateSchDlCb(uint8_t delIdx, SchDlCb *dlInfo)
{
   ...
   /*Leakage of Last Index*/
   if(dlInfo->dlLcCtxt[dlInfo->numDlLc].snssai != NULLP) /*last index of ulLcCtxt(before deletion) should be void*/
   {
