...
uint8_t schFillBoGrantDlSchedInfo(SchCellCb *cell, DlSchedInfo *dlSchedInfo, DlMsgAlloc *dlMsgAlloc)
{
   ...
   uint16_t crnti = 0, reqPRB = 0, reqTBS = 0, mcsIdx = 0;
   ...
   while(cell->boIndBitMap)
   {
      ...
      for(lcIdx = 0; lcIdx < MAX_NUM_LC; lcIdx++)
      {
         ...
         {
            //Bug; LCIDX is different from LCID.
            ...
            /*Calculate reqPRB for this LCID*/
            reqTBS = schCalcTbSize(dlMsgAlloc->lcSchInfo[dlMsgAlloc->numLc].schBytes);
            reqPRB = schCalcNumPrb(reqTBS, mcsIdx, pdschSymbols);
            ...
            if(updateLcListReqPRB(lcLL, ueCb->dlInfo.dlLcCtxt[lcIdx].lcId, reqPRB,\
      ...
      maxFreePRB = bestFreeBlockSearch(cell, dlSchedInfo->schSlotValue.dlMsgTime, *startPrb);
      if(maxFreePRB == 0)
      {
      ...
      if(maxFreePRB <  ueCb.dlLcPrbEst.dedLcInfo->rsvdDedicatedPRB)
      { 
         ueCb.dlLcPrbEst.sharedNumPrb = maxFreePRB;
         DU_LOG("\nWARNING  --> SCH : RESERVED PRB for Dedicated Slice is not available  \
               thus skipping PRB allocation for Dedicated Slice for this slot,\
               SharePRB number:%d.", ueCb.dlLcPrbEst.sharedNumPrb);

         /*PRB Alloc for Default LCs*/
         prbAllocUsingRRMPolicy(ueCb.dlLcPrbEst.defLcList, FALSE, \
               &(ueCb.dlLcPrbEst.sharedNumPrb), ueCb.dlLcPrbEst.dedLcInfo->rsvdDedicatedPRB);
      ...
      else
      {
         ...
         prbAllocUsingRRMPolicy(ueCb.dlLcPrbEst.dedLcInfo->dedLcList, TRUE, \
               &(ueCb.dlLcPrbEst.sharedNumPrb), ueCb.dlLcPrbEst.dedLcInfo->rsvdDedicatedPRB);
         /*PRB Alloc for Default LCs*/
         prbAllocUsingRRMPolicy(ueCb.dlLcPrbEst.defLcList, FALSE, \
               &(ueCb.dlLcPrbEst.sharedNumPrb), ueCb.dlLcPrbEst.dedLcInfo->rsvdDedicatedPRB);
      ...
      for(lcIdx = 0; lcIdx < dlMsgAlloc->numLc; lcIdx ++)
      {
         ...
         dlMsgAlloc->lcSchInfo[lcIdx].schBytes = calculateAllocBOSize(lcLL, \
               dlMsgAlloc->lcSchInfo[lcIdx].lcId, mcsIdx, pdschSymbols);

         /*Calculate the Total Payload/BO size allocated*/
         accumalatedSize += dlMsgAlloc->lcSchInfo[lcIdx].schBytes;
      }
