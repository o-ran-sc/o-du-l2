...
bool schFillBoGrantDlSchedInfo(SchCellCb *cell, SlotTimingInfo currTime, uint8_t ueId, SchDlHqProcCb* retxHqCb)
{
   ...
   HQ_LOG(0,hqP->procId,"\nANKIT schFillBoGrantDlSchedInfo start ueId[%d] isRetx[%d] proc[%2d] \n", ueId, isRetx, hqP->procId);
   ...
   UNSET_ONE_BIT(ueId, cell->boIndBitMap);
   HQ_LOG(0,hqP->procId,"\nANKIT schFillBoGrantDlSchedInfo end\n");
   return true;
}
...
uint8_t schProcessSlotInd(SlotTimingInfo *slotInd, Inst schInst)
{
   ...
   if(pendingUeNode)
   {
      if(pendingUeNode->node)
      {
         ...
         {
            isMsg4Pending = true;
            isMsg4Scheduled = schProcessMsg4Req(cell, *slotInd, ueId, TRUE, cell->ueCb[ueId-1].msg4Proc);
         }
         else
         {
            ...
            {
               isMsg4Pending = true;
               isMsg4Scheduled = schProcessMsg4Req(cell, *slotInd, ueId, FALSE, cell->ueCb[ueId-1].msg4Proc);
            }
         }
