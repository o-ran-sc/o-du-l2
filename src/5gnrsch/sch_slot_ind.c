...
bool schFillBoGrantDlSchedInfo(SchCellCb *cell, SlotTimingInfo currTime, uint8_t ueId, bool isRetx, SchDlHqProcCb **hqP)
{
   ...
   else
   {
      ...
      dciSlotAlloc->bwp = dciSlotAlloc->bwp;
      memcpy(dlMsgAlloc->dlMsgPdschCfg, dciSlotAlloc->dlMsgPdschCfg, sizeof(PdschCfg));
...
uint8_t schProcDlPageAlloc(SchCellCb *cell, SlotTimingInfo currTime, Inst schInst)
{
   ...
   while(true)
   {
      ...
      memcpy(&dlPageAlloc.pagePdcchCfg.dci.pdschCfg, &dlPageAlloc.pagePdschCfg, sizeof(PdschCfg));
