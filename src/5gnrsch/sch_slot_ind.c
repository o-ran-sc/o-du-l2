...
uint8_t schProcDlPageAlloc(SchCellCb *cell, SlotTimingInfo currTime, Inst schInst)
{
   ...
   pageInfoNode = schPageInfoSearchFromPageList(currTime, &(cell->pageCb.pageReqInfoRecord[currTime.sfn]));
   
   if(pageInfoNode == NULLP)
   {
      DU_LOG("\nERROR  --> SCH: Page not exist for SFN:%d, SLOT:%d", currTime.sfn, currTime.slot);
	  return RFAILED;
   }
   ...
   while(1)
   {
      ...
      if(maxFreePRB != 0)
      {
         pageInfo->tbsSize = calculateEstimateTBSize(pageInfo->msgLen, pageInfo->mcs, NUM_PDSCH_SYMBOL, maxFreePRB, &(pageInfo->nPRB));
      }
      else
      {
      ...
      /*Fill PDSCH*/
      if(schFillPagePdschCfg(cell, &dlPageAlloc.pagePdschCfg, pdschTime, pageInfo->tbsSize, pageInfo->mcs, startPrb) != ROK)
      {
   ...
   schDeleteFromPageInfoList(&(cell->pageCb.pageReqInfoRecord[currTime.sfn]), pageInfoNode);
