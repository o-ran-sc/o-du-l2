 ...
 *      -# void
 **/
void findNextOndurationOccurance(SchCellCb *cell, SchDrxUeCb *ueDrxCb, SlotTimingInfo  *nxtOnDur, uint8_t delta, uint8_t mu)
{
   uint16_t   resultX, resultY, idx, sfn, subframeNum;
   ...
   {
      sfn = tmpTime.sfn;
      subframeNum = (tmpTime.slot/(mu + 1));

      resultX = (sfn *10+subframeNum) % cycleLen;
      if (TRUE != ueDrxCb->shortCyclePresent)
         resultY = ueDrxCb->drxStartOffset;
      else
         resultY = ueDrxCb->drxStartOffset % cycleLen;
      if(resultX == resultY)
      {
...
void schDrxUeReCfgTimer(SchCellCb *cell, SchUeCb *ueCb)
{
   ...
   findNextOndurationOccurance(cell,  &ueCb->drxUeCb, &onDurationOccurance, 0, cell->cellCfg.numerology);
...
void schAddUeInOndurationAndShortCycleList(SchCellCb *cell, SchUeCb *ueCb, uint8_t delta)
{
   ...
   {
      findNextOndurationOccurance(cell,  &ueCb->drxUeCb, &onDurationOccurance, delta, cell->cellCfg.numerology);
      onDurTime = onDurationOccurance.sfn*MAX_SLOTS+onDurationOccurance.slot;
      onDurIndx = onDurTime%MAX_DRX_SIZE;
      ueCb->drxUeCb.onDurationStartIndex = onDurIndx;
      ueCb->drxUeCb.onDurationStartDistance = SCH_CALC_SLOT_DIFF(onDurationOccurance, cell->slotInfo, MAX_SLOTS)/MAX_DRX_SIZE;
