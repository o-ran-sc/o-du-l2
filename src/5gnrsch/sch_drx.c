...
void findNextOndurationOccurance(SchCellCb *cell, SchDrxUeCb *ueDrxCb, SlotTimingInfo  *nxtOnDur, uint8_t delta)
{
   ...
   if ( curTime <= ueDrxCb->drxStartOffset)
   {
      ...
      nxtOnDur->sfn = ((ueDrxCb->drxStartOffset / MAX_SLOTS)) & (MAX_SFN - 1);
      nxtOnDur->slot = (ueDrxCb->drxStartOffset % MAX_SLOTS);
   ...
   else
   {
      ...
      if (0 == (tmpDistance % cycleLen))
      {
         ...
         nxtOnDur->sfn      = ((curTime / MAX_SLOTS)) & (MAX_SFN - 1);
         nxtOnDur->slot = (curTime % MAX_SLOTS);
         nxtDist = ((nxtOnDur->sfn * MAX_SLOTS) + nxtOnDur->slot);
      }
      else
      {
         nxtDist            = ueDrxCb->drxStartOffset + (numOfCycles + 1) * cycleLen;
         nxtOnDur->sfn      = ((nxtDist / MAX_SLOTS) & (MAX_SFN - 1));
         nxtOnDur->slot = (nxtDist % MAX_SLOTS);
      }
   }
   ...
   DU_LOG("\nPBORLA [%d:%d] [%d:%d]", cell->slotInfo.sfn, cell->slotInfo.slot, nxtOnDur->sfn,  nxtOnDur->slot);
}
