...
void schDeleteDlHarqDrxTimer(SchCellCb  *cell, SchDlHqEnt *dlHqEnt)
{
   ...
   for(idx =0; idx<numHqPrcs; idx++)
   {
      ...
      if(dlProc->dlDrxHarqCb.rttIndex != SCH_DRX_INVALID_INDEX)
      {
         cmLListDelFrm(&cell->drxCb[tmrIdx].dlHarqRttExpiryList, dlProc->dlDrxHarqCb.rttNode);
         SCH_FREE(dlProc->dlDrxHarqCb.rttNode, sizeof(CmLList));
      }

      if(dlProc->dlDrxHarqCb.retxIndex != SCH_DRX_INVALID_INDEX)
      {
...
void schDeleteUlHarqDrxTimer(SchCellCb  *cell, SchUlHqEnt *ulHqEnt)
{
   ...
   for(idx =0; idx<numHqPrcs; idx++)
   {
      ...
      if(ulProc->ulDrxHarqCb.rttIndex != SCH_DRX_INVALID_INDEX)
      {
         cmLListDelFrm(&cell->drxCb[tmrIdx].ulHarqRttExpiryList, ulProc->ulDrxHarqCb.rttNode);
         SCH_FREE(ulProc->ulDrxHarqCb.rttNode, sizeof(CmLList));
      }

      if(ulProc->ulDrxHarqCb.retxIndex != SCH_DRX_INVALID_INDEX)
      {
