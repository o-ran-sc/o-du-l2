...
uint8_t fillPucchFormat0(SchPucchInfo *ulSchedPucch, SchPucchResrcInfo *resrcInfo)
{
   if(resrcInfo->SchPucchFormat.format0)
   {
      ...
      SCH_ALLOC(ulSchedPucch->initialCyclicShift, sizeof(uint8_t));
      if(!ulSchedPucch->initialCyclicShift)
      {
         DU_LOG("\nSCH: Failed to allocate memory for cyclic shift in fillPucchFormat0()"); 
         return RFAILED;
      }
      memset(ulSchedPucch->initialCyclicShift, 0, sizeof(uint8_t));
...
uint8_t fillPucchFormat1(SchPucchInfo *ulSchedPucch, SchPucchResrcInfo *resrcInfo)
{
   if(resrcInfo->SchPucchFormat.format1)
   {
      ...
      SCH_ALLOC(ulSchedPucch->initialCyclicShift, sizeof(uint8_t));
      if(ulSchedPucch->initialCyclicShift == NULLP)
      {
         DU_LOG("\nSCH: Failed to allocate memory for cyclic shift in fillPucchFormat1()"); 
         return RFAILED;
      }
      memset(ulSchedPucch->initialCyclicShift, 0, sizeof(uint8_t));
      *ulSchedPucch->initialCyclicShift =  resrcInfo->SchPucchFormat.format1->initialCyclicShift;
      ...
      SCH_ALLOC(ulSchedPucch->timeDomOCC, sizeof(uint8_t));
      if(ulSchedPucch->timeDomOCC == NULLP)
      {
         DU_LOG("\nSCH: Failed to allocate memory for time domain OCC in fillPucchFormat1()"); 
         return RFAILED;
      }
      memset(ulSchedPucch->timeDomOCC, 0, sizeof(uint8_t));
   ...
   SchPucchResrcInfo *resrcInfo, SchPucchFormatCfg *formatCfg)
{
   ...
   switch(pucchFormat)
   {
      case PUCCH_FORMAT_0:
         {
	    if(resrcInfo)
               ret = fillPucchFormat0(ulSchedPucch, resrcInfo);
	    else
	       ret = RFAILED;
            return ret;
	 }
      case PUCCH_FORMAT_1:
	    ...
	    if(resrcInfo)
	    {
               ret = fillPucchFormat1(ulSchedPucch, resrcInfo);
	    }
	    if(formatCfg)
	    {
      ...
      default:
         DU_LOG("\n Invalid PUCCH format[%d] in fillUlSchedPucchFormatCfg()", pucchFormat);
	 ret = RFAILED;
         return ret;
   }
   return ret;
...
uint8_t fillUlSchedPucchDedicatedCfg(SchPucchCfg *pucchDedCfg, SlotIndInfo *slotInfo, SchPucchInfo *ulSchedPucch)
{
   ...
   {
      SCH_ALLOC(ulSchedPucch->cmnFormatCfg, sizeof(SchPucchFormatCfg));
      if(ulSchedPucch->cmnFormatCfg == NULLP)
      {
...
/**
 * @brief Function to free Ul Sched Pucch Cfg
 *
 * @details
 *
 *     Function : freeUlSchedPucchCfg
 *     
 *     Function to free Ul Sched Pucch Cfg
 *     
 *  @param[in]  SchPucchInfo *schPucchInfo
 *  @return  void
 **/

void freeUlSchedPucchCfg(SchPucchInfo *pucchInfo)
{
   if(pucchInfo->intraFreqHop)
      SCH_FREE(pucchInfo->intraFreqHop, sizeof(uint8_t));
   if(pucchInfo->secondPrbHop)
      SCH_FREE(pucchInfo->secondPrbHop, sizeof(uint16_t));
   if(pucchInfo->initialCyclicShift)
      SCH_FREE(pucchInfo->initialCyclicShift, sizeof(uint8_t));
   if(pucchInfo->occLen)
      SCH_FREE(pucchInfo->occLen, sizeof(uint8_t));
   if(pucchInfo->occIdx)
      SCH_FREE(pucchInfo->occIdx, sizeof(uint8_t));
   if(pucchInfo->timeDomOCC)
      SCH_FREE(pucchInfo->timeDomOCC, sizeof(uint8_t));
   if(pucchInfo->cmnFormatCfg)
      SCH_FREE(pucchInfo->cmnFormatCfg, sizeof(SchPucchFormatCfg));
   
   memset(pucchInfo, 0, sizeof(SchPucchInfo));
}
...
uint16_t fillPucchResourceInfo(SchPucchInfo *schPucchInfo, Inst inst)
{
   ...
   if(cell->ueCb[ueIdx].ueCfg.spCellCfg.servCellCfg.initUlBwp.pucchCfgPres)
   {
      ...
      ret = fillUlSchedPucchDedicatedCfg(&cell->ueCb[ueIdx].ueCfg.spCellCfg.servCellCfg.initUlBwp.pucchCfg,\
               &cell->slotInfo, schPucchInfo);
      if(ret == RFAILED)
      {
         freeUlSchedPucchCfg(schPucchInfo);
	 return ret;
      }
   }
   else
