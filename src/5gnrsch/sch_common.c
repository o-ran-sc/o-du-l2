...
void schCmnDlAlloc(SchCellCb *cell, DlBrdcstAlloc *dlBrdcstAlloc)
{
	...
	if(dlBrdcstAlloc->ssbTrans)
	{
		...
		SCH_ALLOC(ssbStartSymb, SCH_MAX_SSB_BEAM*sizeof(uint8_t));
		if(!ssbStartSymb)
		{
			DU_LOG("\nMemory allocation failure in schCmnDlAlloc");
			return RFAILED;
		}
		ssbDlTdAlloc(scs, ssbStartSymbArr);
		ssbStartSymb = ssbStartSymbArr[dlBrdcstAlloc->ssbIdxSupported]; /*since we are supporting only 1 ssb beam */
		...
		for(idx=ssbStartSymb; idx<SCH_SSB_SYMB_DURATION; idx++)
		{
