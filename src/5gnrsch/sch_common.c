...
#if 0
   SCH_ALLOC(msg4Alloc->dlMsgPdschCfg, sizeof(PdschCfg));
   if(msg4Alloc->dlMsgPdschCfg == NULLP)
   {
   ...
   else
   {
      ...
      for(slotIdx = 0; slotIdx < cell->numSlots; slotIdx++)
      {
         ...
         for(k0Index = 0; ((k0Index < numTimeDomAlloc) && (k0Index < MAX_NUM_K0_IDX)); k0Index++)
         {
            if(pdschCfgCmnPres == true)
            {
               k0TmpVal = cmnTimeDomRsrcAllocList[k0Index].k0 + 4;
            ...
            else
            {
               if(dedTimeDomRsrcAllocList[k0Index].k0 != NULLP)
               {
                  k0TmpVal =  *(dedTimeDomRsrcAllocList[k0Index].k0) + 6;
               }
               else
               {
                  k0TmpVal = DEFAULT_K0_VALUE +6;
               }
