...
bool schProcessMsg4Req(SchCellCb *cell, SlotTimingInfo currTime, uint8_t ueId, bool isRetxMsg4, SchDlHqProcCb *msg4HqProc)
{
   ...
   HQ_LOG(0,-,"\nANKIT schProcessMsg4Req  start isRetx[%d] msg4HqProc[%d]\n",isRetxMsg4,msg4HqProc);
   if(cell == NULL)
   {
      DU_LOG("\nERROR  -->  SCH: schDlRsrcAllocMsg4() : Cell is NULL");
      return RFAILED;
   ...
   {
      schDlReleaseHqProcess(msg4HqProc);
   ...
   HQ_LOG(0,msg4HqProc->procId,"\nANKIT schProcessMsg4Req  end\n");
   return ROK;
}
 ...
 *
 *******************************************************************/
bool schProcessSrOrBsrReq(SchCellCb *cell, SlotTimingInfo currTime, uint8_t ueId, SchUlHqProcCb* retxHqP)
{
   ...
   HQ_LOG(1,hqP->procId,"\nANKIT schProcessSrOrBsrReq start ueId[%d] isRetx[%d] proc[%2d]\n",ueId, isRetx, hqP->procId);
   ...
   if(k2Found == true)
   {
      ...
      if(totDataReq > 0 && ret == TRUE)
      {
         ...
         if(!dciInfo)
         {
            ...
            schUlReleaseHqProcess(hqP, FALSE);
            return false;
         }
         ...
         if(schFillPuschAlloc(ueCb, puschTime, totDataReq, startSymb, symbLen, startPrb, isRetx, hqP) == ROK)
         {
            if(cell->schUlSlotInfo[puschTime.slot]->schPuschInfo)
            {
               ...
               if(puschInfo != NULLP)
               {
                  ...
                  HQ_LOG(1,hqP->procId,"\nANKIT schProcessSrOrBsrReq end ret[%d]\n",ret);
                  return true;
               }
            }
         }
