...
uint8_t schBroadcastSsbAlloc(SchCellCb *cell, DlBrdcstAlloc *dlBrdcstAlloc, uint16_t slot)
{
   ...
   if((allocatePrbPerSymbol(schDlSlotInfo->prbAllocPerSymbol, ssbStartSymb, SCH_SSB_NUM_SYMB, \
       ssbStartPrb, (SCH_SSB_NUM_PRB+1))) != ROK)
   {
       DU_LOG("\nERROR  -->  SCH: PRB allocation failed for SSB in slot [%d]", slot);
       return RFAILED;
   }
...
uint8_t schBroadcastSib1Alloc(SchCellCb *cell, DlBrdcstAlloc *dlBrdcstAlloc, uint16_t slot)
{
   ...
   if((allocatePrbPerSymbol(schDlSlotInfo->prbAllocPerSymbol, timeAlloc.startSymb, timeAlloc.numSymb, \
       freqAlloc.startPrb, freqAlloc.numPrb)) != ROK)
   {
       DU_LOG("\nERROR  -->  SCH: PRB allocation failed for SIB1 in slot [%d]", slot);
       return RFAILED;
   }
