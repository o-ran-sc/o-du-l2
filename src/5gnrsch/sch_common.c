                  ...
                  uint16_t *sharedPRB, uint16_t *reservedPRB, bool *isTxPayloadLenAdded, bool *srRcvd)
{
   ...
   while(node)
   {
      ...
      {
         DU_LOG("\nINFO --> SCH: LC:%d is the First node to be allocated which includes UL_GRANT_SIZE",\
               lcNode->lcId);
         *srRcvd = FALSE;
 ...
 *
 * ****************************************************************/
uint32_t schCalculateUlTBS(SchCellCb *cell, SchUeCb *ueCb, SlotTimingInfo puschTime, uint8_t symbLen, uint16_t *startPrb)
{
   uint32_t totTBS = 0; /* in bytes */
   uint16_t mcsIdx = 0;
   ...
   *startPrb = 0;
   ...
   maxFreePRB = searchLargestFreeBlock(cell, puschTime, startPrb, DIR_UL);
   ...
   if(maxFreePRB != 0)
   {
      ...
      { 
         ...
         DU_LOG("\nWARNING  --> SCH : UL Only Default Slice is scheduled, sharedPRB Count:%d",\
...
bool schProcessSrOrBsrReq(SchCellCb *cell, SlotTimingInfo currTime, uint8_t ueId)
{
   ...
   if(k2Found == true)
   {
      totDataReq = schCalculateUlTBS(cell, ueCb, puschTime, symbLen, &startPrb);
      if(totDataReq > 0)
      {
