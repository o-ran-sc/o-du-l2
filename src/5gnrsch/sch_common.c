 ...
 *
 * @return uint32_t totalTBS size
 *
 * ****************************************************************/
uint32_t schCalculateUlTbs(SchUeCb *ueCb, SlotTimingInfo puschTime, uint8_t symbLen, uint16_t *startPrb)
{
...
bool schProcessSrOrBsrReq(SchCellCb *cell, SlotTimingInfo currTime, uint8_t ueId)
{
   ...
   if(k2Found == true)
   {
      totDataReq = schCalculateUlTbs(ueCb, puschTime, symbLen, &startPrb);
      if(totDataReq > 0)
      {
