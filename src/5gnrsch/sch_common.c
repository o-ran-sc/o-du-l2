      ...
      if(lcNode != NULLP)
      {
         ...
         if(dlMsgAlloc != NULLP)
         {
            ...
            *accumalatedBOSize += dlMsgSchInfo->lcSchInfo[dlMsgSchInfo->numLc].schBytes; 
            ...
            /* TODO : MUST check for corner cases
             * The entry from list should be deleted even if not fully allocated because in case of segmentation, 
             * RLC will segmented DATA PDU along with remainig BO request. In this case, a new entry will be added
             * to this list. Hence, the old entry must be cleared beforehand */
            //if(lcNode->reqBO == 0)
            {
