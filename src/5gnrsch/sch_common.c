...
uint8_t schDlRsrcAllocMsg4(SchCellCb *cell, SlotTimingInfo msg4Time, uint8_t ueIdx, DlMsgAlloc *msg4Alloc, uint8_t k0Idx)
{
   ...
   memcpy(&msg4Alloc->dlMsgInfo, &cell->raCb[ueIdx].dlMsgInfo, sizeof(DlMsgInfo));
   return ROK;
}
 ...
 *
 * @params[in] SlotTimingInfo currTime, SchCellCb *cell 
 * @return ROK     - success
 *         RFAILED - failure
 *
 *******************************************************************/

uint8_t schProcessMsg4Req(SlotTimingInfo currTime, SchCellCb *cell)
{
   bool k1Found;  
   uint16_t dciSlot = 0, pdschSlot = 0;
   uint8_t ueIdx=0,numK0,k0TblIdx,k0Index,k0Val;
   uint8_t k1TblIdx,k1Index,k1Val,numK1;
   SchK0K1TimingInfoTbl *k0K1InfoTbl;
   SlotTimingInfo pdschTime, pucchTime;
   ...
   while(ueIdx < MAX_NUM_UE)
   {
...
#ifdef NR_TDD
      totalCfgSlot = calculateSlotPatternLength(cell->cellCfg.ssbSchCfg.scsCommon, cell->cellCfg.tddCfg.tddPeriod);
      if(schGetSlotSymbFrmt(currTime.slot%totalCfgSlot, cell->slotFrmtBitMap) != DL_SLOT)
      {
         continue;
      }
#endif
      ...
      numK0 = k0K1InfoTbl->k0k1TimingInfo[currTime.slot].numK0;
      for(k0TblIdx = 0; k0TblIdx < numK0; k0TblIdx++)
      {
         k0Index = k0K1InfoTbl->k0k1TimingInfo[currTime.slot].k0Indexes[k0TblIdx].k0Index;
         k0Val = cell->cellCfg.schInitialDlBwp.pdschCommon.timeDomRsrcAllocList[k0Index].k0;

         ADD_DELTA_TO_TIME(currTime, pdschTime, k0Val);
#ifdef NR_TDD
         if(schGetSlotSymbFrmt(currTime.slot%totalCfgSlot, cell->slotFrmtBitMap) != DL_SLOT)
         {
            continue;
         ...
         numK1 = k0K1InfoTbl->k0k1TimingInfo[currTime.slot].k0Indexes[k0TblIdx].k1TimingInfo.numK1;
         for(k1TblIdx = 0; k1TblIdx < numK1; k1TblIdx++)
         {
            k1Index = k0K1InfoTbl->k0k1TimingInfo[currTime.slot].k0Indexes[k0TblIdx].k1TimingInfo.k1Indexes[k1TblIdx];
      ...
      cell->schDlSlotInfo[currTime.slot]->dlMsgAlloc = dciSlotAlloc;
      ...
      */
      ...
      else
      {
         ...
         if(msg4SlotAlloc == NULLP)
         {
            ...
            SCH_FREE(dciSlotAlloc, sizeof(DlMsgAlloc));
            cell->schDlSlotInfo[currTime.slot]->rarAlloc = NULLP;
            return 0;
         }
         cell->schDlSlotInfo[currTime.slot]->dlMsgAlloc = msg4SlotAlloc;
      ...
      /* PUCCH resource */
      schAllocPucchResource(cell, cell->raCb[ueIdx].tcrnti, pucchTime.slot);
      cell->raCb[ueIdx].msg4recvd = FALSE;
      ueIdx++;
      DU_LOG("\nINFO  -->  SCH : MSG4 doneeeeeeeeeeeeeeeeeeeeeeeeeee msg4SlotAlloc");
   }
   return ROK;
}
/**********************************************************************
