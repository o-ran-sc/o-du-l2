...
SchPdschConfig pdschDedCfg, uint8_t ulAckListCount, uint8_t *UlAckTbl)
{
   ...
   /*memseting the structre and storing the total slots values*/
   memset(k0K1InfoTbl, 0, sizeof(SchK0K1TimingInfoTbl));
   k0K1InfoTbl->tblSize = cell->numSlots;
   
   /*stroing time domain resource allocation list based on common or dedicated 
    * configuration*/
   ...
   {
      numK0 = 0;
      /* stroing the values of k0 based on time domain resource
...
SchPdschConfig pdschDedCfg, uint8_t ulAckListCount, uint8_t *UlAckTbl)
{
   ...
   else
   {
      ...
      /*memseting the K0K1 strucutre, storing total num if slot and calculating the slot pattern length*/
      memset(k0K1InfoTbl, 0, sizeof(SchK0K1TimingInfoTbl));
      k0K1InfoTbl->tblSize = cell->numSlots;
      totalCfgSlot = calculateSlotPatternLength(cell->cellCfg.ssbSchCfg.scsCommon, cell->cellCfg.tddCfg.tddPeriod);
      
      /*Storing time domain resorce allocation list based on common and
      ...
      for(slotIdx = 0; slotIdx < cell->numSlots; slotIdx++)
      {
         /*If persent slot is UL or FLEXI then Skip the slot else continue*/
         ...
         for(k0Index = 0; ((k0Index < numTimeDomAlloc) && (k0Index < MAX_NUM_K0_IDX)); k0Index++)
         {
             ...
             * else if it is UL slot then store the infomation
             * else if it is FLEXI slot then check the symbols, it must have
             * atleast on UL symbol */
            numK1 = 0;
            for(k1Index = 0; k1Index < ulAckListCount; k1Index++)
            {
               k1TmpVal = UlAckTbl[k1Index];
