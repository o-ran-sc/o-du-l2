...
uint8_t MacSchRachRsrcReq(Pst *pst, SchRachRsrcReq *schRachRsrcReq)
{
 ...
 /* Fill RACH resource response to MAC */
   ...
   /* Send failure response to MAC in case of invalid cell ID or CRNTI */
   if(rachRsrcRsp->result == RSP_NOK)
   {
      SCH_FREE(schRachRsrcReq, sizeof(SchRachRsrcReq));
      /* Return failure in RACH resource response */
   ...
   /* Return Success RACH resource response to MAC */
