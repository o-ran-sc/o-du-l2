...
uint8_t *msg3NumRb)
{
   ...
   cell = schCb[schInst].cells[schInst];
	...
	{
		cell->ulSchInfo[msg3SlotAlloc]->assignedPrb[idx] = startRb + numRb;
	}
	ulSchInfo = cell->ulSchInfo[msg3SlotAlloc];

   SCH_ALLOC(ulSchInfo->schPuschInfo, sizeof(SchPuschInfo));
	if(!ulSchInfo->schPuschInfo)
	{
      DU_LOG("SCH: Memory allocation failed in schAllocMsg3Pusch");
		return RFAILED;
	}
	ulSchInfo->schPuschInfo->harqProcId        = SCH_HARQ_PROC_ID;
	ulSchInfo->schPuschInfo->resAllocType      = SCH_ALLOC_TYPE_1;
	ulSchInfo->schPuschInfo->fdAlloc.startPrb  = startRb;
	ulSchInfo->schPuschInfo->fdAlloc.numPrb    = numRb;
	ulSchInfo->schPuschInfo->tdAlloc.startSymb = startSymb;
	ulSchInfo->schPuschInfo->tdAlloc.numSymb   = symbLen;
	ulSchInfo->schPuschInfo->tbInfo.mcs	     = 4;
	ulSchInfo->schPuschInfo->tbInfo.ndi        = 1; /* new transmission */
	ulSchInfo->schPuschInfo->tbInfo.rv	        = 0;
	ulSchInfo->schPuschInfo->tbInfo.tbSize     = 24; /*Considering 2 PRBs */
...
uint8_t schProcessRachInd(RachIndInfo *rachInd, Inst schInst)
{
   SchCellCb *cell = schCb[schInst].cells[schInst];
	uint16_t raRnti = 0;
	uint16_t rarSlot = (rachInd->timingInfo.slot+SCHED_DELTA+RAR_DELAY+PHY_DELTA)%SCH_NUM_SLOTS;
	uint16_t msg3StartRb;
	uint8_t  msg3NumRb;
   uint8_t  ret = ROK;

   DlSchInfo *dlSchInfo = cell->dlSchInfo[rarSlot]; /* RAR will sent in the next slot */
	RarInfo *rarInfo = &(dlSchInfo->rarInfo);

   /* rar message presense in next slot ind and will be scheduled */
   dlSchInfo->rarPres = true;
...
uint8_t schFillRar(RarAlloc *rarAlloc, uint16_t raRnti, uint16_t pci, uint8_t offsetPointA)
{
   ...
   SchBwpDlCfg *initialBwp = &schCb[inst].cells[inst]->cellCfg.schInitialUlBwp;
   ...
   pdsch->freqAlloc.rbSize = schCalcNumPrb(tbSize,4/*MCS*/,numPdschSymbols);
