...
void schProcessRaReq(SlotTimingInfo currTime, SchCellCb *cell)
{
   ...
   while(ueIdx < MAX_NUM_UE)
   {
...
#endif
      {
         ...
         if((allocatePrbPerSymbol(schDlSlotInfo->prbAllocPerSymbol, pdschTimeAlloc.startSymb, pdschTimeAlloc.numSymb, \
                     pdschFreqAlloc.startPrb, pdschFreqAlloc.numPrb)) != ROK)
         {
            DU_LOG("\nERROR  -->  SCH: PRB allocation failed for RAR in slot [%d]", rarSlot);
            return;
         }
