 ...
 *  @return  false
 **/
bool isInRaRspWindow(SchRaReq *raReq, SlotTimingInfo frameToCheck, uint16_t numSlotsPerSystemFrame)
{
...
void schProcessRaReq(SlotTimingInfo currTime, SchCellCb *cell)
{
   ...
   SchPuschInfo *msg3PuschInfo;          /* Stores MSG3 PUSCH scheduling information */
   ...
   while(ueIdx < MAX_NUM_UE)
   {
...
#endif
      {
         ...
         if(isInRaRspWindow(cell->raReq[ueIdx], dciTime, cell->numSlots))
         {
            ...
            for(k0TblIdx = 0; k0TblIdx < k0K1InfoTbl->k0k1TimingInfo[dciSlot].numK0; k0TblIdx++)
            {
               ...
               rarSlot = (dciSlot + k0) % cell->numSlots;

               for(k2TblIdx = 0; k2TblIdx < k2InfoTbl->k2TimingInfo[rarSlot].numK2; k2TblIdx++)
               {
                  k2Index = k2InfoTbl->k2TimingInfo[rarSlot].k2Indexes[k2TblIdx];
...
#ifdef NR_TDD
                     if(schGetSlotSymbFrmt(msg3Slot, cell->slotFrmtBitMap) == DL_SLOT)
                        continue;
#endif
                     k2Found = true;
                     break;
