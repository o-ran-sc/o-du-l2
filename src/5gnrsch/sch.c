...
uint8_t MacSchDlRlcBoInfo(Pst *pst, DlRlcBoInfo *dlBoInfo)
{
         ...
         (lcId >= MIN_DRB_LCID && lcId <= MAX_DRB_LCID))
   {
      ...
      /*searching for Lc to for which BO has been receieved*/
      for(ueLcIdx = 0; ueLcIdx < ueCb->ulInfo.numUlLc; ueLcIdx++)
      {
         if(ueCb->ulInfo.ulLcCtxt[ueLcIdx].lcId == lcId)
         {
            ueCb->dlInfo.dlLcCtxt[ueLcIdx].bo = dlBoInfo->dataVolume;
            break;
         }
      }
      if(ueLcIdx == ueCb->ulInfo.numUlLc)
      {
         DU_LOG("\nERROR  -->  SCH : MacSchDlRlcBoInfo: LC Id %d has no context in SCH UE CB", lcId);
         return RFAILED;
      }
...
#endif
   DU_LOG("\nINFO --> SCH : VS slot:%d, lcId:%d, bo:%d",slot, lcId, dlBoInfo->dataVolume);
