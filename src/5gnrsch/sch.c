...
/* ue bit map */
uint64_t gActvUeBitMap;
uint64_t gBoIndBitMap;
 ...
 *      -# ROK
 **/
int schActvInit(Ent entity, Inst instId, Region region, Reason reason)
{
...
uint8_t macSchDlRlcBoInfo(Pst *pst, DlRlcBOInfo *dlBoInfo)
{
   ...
   else if(lcId == SRB1_LCID || lcId == SRB2_LCID)
   {
   ...
   else if(lcId >= MIN_DRB_LCID && lcId <= MAX_DRB_LCID)
   {
      GET_UE_IDX(crnti,ueIdx);
      SET_ONE_BIT(ueIdx,gBoIndBitMap);
      ueCb->dlLcCtxt[lcId].bo = dlBoInfo->dataVolume;
