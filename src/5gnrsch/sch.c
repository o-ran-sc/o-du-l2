 ...
 *
 * ********************************************************************************/
uint8_t addSliceCfgInSchDb(CmLListCp *sliceCfgInDb, SchRrmPolicyOfSlice *rrmPolicyOfSlices, SchRrmPolicyOfSlice *cfgReq, bool sliceRecfg)
{
   ...
   for(cfgIdx = 0; cfgIdx<schSliceCfgReq->numOfConfiguredSlice; cfgIdx++)
   {
      ...
      for(plmnIdx = 0; plmnIdx < MAX_PLMN; plmnIdx++)
      {
         for(sliceIdx = 0; sliceIdx<cellCb->cellCfg.plmnInfoList[plmnIdx].numSliceSupport; sliceIdx++)
         {
            ...
            {
               if(addSliceCfgInSchDb(storedSliceCfg, NULL, schSliceCfgReq->listOfSlices[cfgIdx], false) == ROK)
               {
                  sliceFound = RSP_OK;
               }
               else
