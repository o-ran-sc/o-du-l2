 ...
 *
 * ****************************************************************/
uint8_t addUeToBeScheduled(SchCellCb *cell, uint8_t ueIdxToAdd)
{
   uint8_t *ueIdx;
   CmLList *node;
   ...
   SCH_ALLOC(ueIdx, sizeof(uint8_t));
   if(!ueIdx)
   {
   ...
   *ueIdx = ueIdxToAdd;
   if(addNodeToLList(&cell->ueToBeScheduled, ueIdx, NULLP) != ROK)
   {
