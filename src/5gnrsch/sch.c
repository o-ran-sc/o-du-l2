 ...
 *
 * ****************************************************************/
uint8_t schAddToKpiActiveList(CmLListCp *prbUseList, PTR schStatsGrp)
{
   CmLList  *node = NULLP;
   ...
   if(node)
   {
      node->node = schStatsGrp; 
      cmLListAdd2Tail(prbUseList, node);
      return ROK;
   }
 ...
 *          we need to into the list as a node.
 *    [Step 2] - If allocation is successful then move start traversing 
 *          each measurment cfg index of received group info.
 *       [Step 2.1] Validate all the measurement information.
 *          [Step 2.1.1] If validation is successful, go to [step 2.2].
 *          [Step 2.1.2] Else add the related info in stats rsp's
 *             rejected list.
 *       [Step 2.2] If DL related report received then add the KPI into
 *          Dl related active KPI list.
 *          [Step 2.2.1] If the KPI is not successfully added, add the 
 *             stats related info in stats rsp's rejected list.
 *       [Step 2.3] If UL related report received then add the KPI into
 *          Ul related active KPI list.
 *          [Step 2.3.1] If the KPI is not successfully added, add the 
 *             stats related info in stats rsp's rejected list.
 *       [Step 2.4] Fill other group related information
 *       [Step 2.5] Initialise and start timer
 *       [Step 2.6] Now create a node for stats group list and add 
 *             All the stats related information in link list.
 *          [Step 2.6.1] If node successfully added to the list, then 
 *             fill the group related info in stats rsp's accepted list.
 *          [Step 2.6.2] Else add the related info in stats rsp's
 *             rejected list.
 *    [Step 3] - Else fill the group related info in stats rsp's 
 *          rejected list.
 *
 * @params[in]  
 *          Inst
...
uint8_t schAddToStatsGrpList(Inst inst, struct schStatsRsp *statsRsp, uint64_t subscriptionId, SchStatsGrpInfo *grpInfo)
{
   ...
   bool successfulDlAlloc=false;
   bool successfulUlAlloc=false;
   ...
   for(reqGrpIdx=0; reqGrpIdx<statsReq->numStatsGroup && grpIdx<MAX_NUM_STATS; reqGrpIdx++)
   {
      ...
      else
      {
         ...
         if(schAddToStatsGrpList(inst, &schStatsRsp, statsReq->subscriptionId, grpInfo) != ROK)
         {
            ...
            { 
               if(schStatsRsp.statsGrpRejectedList[schStatsRsp.numGrpRejected-1].cause == RESOURCE_UNAVAILABLE)
               {
                  allocFailed = true;
               }
            }
...
void deleteStatsGrp(Inst inst, CmLList *grpNode)
{
   ...
   CmLList *dlTotPrbUseNode=NULLP;
   CmLList *ulTotPrbUseNode=NULLP;
   
   if(grpNode)
   {
      statsGrpInfo = (SchStatsGrp*)grpNode->node;
      node = (PTR) statsGrpInfo->kpiStats.dlTotalPrbUsage;
      CM_LLIST_FIRST_NODE(&schCb[inst].statistics.activeKpiList.dlTotPrbUseList, dlTotPrbUseNode);
      while(dlTotPrbUseNode)
      {
         if(memcmp(&dlTotPrbUseNode->node, &node, sizeof(TotalPrbUsage)) == 0)
         {
            cmLListDelFrm(&schCb[inst].statistics.activeKpiList.dlTotPrbUseList, dlTotPrbUseNode);
            SCH_FREE(statsGrpInfo->kpiStats.dlTotalPrbUsage, sizeof(TotalPrbUsage));
            break;
         }
         dlTotPrbUseNode = dlTotPrbUseNode->next;
      }

      node = (PTR) statsGrpInfo->kpiStats.ulTotalPrbUsage;
      CM_LLIST_FIRST_NODE(&schCb[inst].statistics.activeKpiList.ulTotPrbUseList, ulTotPrbUseNode);
      while(ulTotPrbUseNode)
      {
         if(memcmp(&ulTotPrbUseNode->node, &node, sizeof(TotalPrbUsage)) == 0)
         {
            cmLListDelFrm(&schCb[inst].statistics.activeKpiList.ulTotPrbUseList, ulTotPrbUseNode);
            SCH_FREE(statsGrpInfo->kpiStats.ulTotalPrbUsage, sizeof(TotalPrbUsage));
            break;
         }
         ulTotPrbUseNode = ulTotPrbUseNode->next;
      }
...
/*******************************************************************
 *
 * @brief Update active KPI pointers to KPI-Active-List
 *
 * @details
 *
 *    Function : schUpdateToKpiActiveList
 *
 *    Functionality: For each active statistics group for which timer
 *       is running, add its KPI pointer to KPI-Active-List.
 *       Use case :
 ...
 * @params[in]  Inst
 *              Stats group info
 *              Boolean variable for DL or UL
 * @return ROK     - success
 *         RFAILED - failure
 *
 * ****************************************************************/
uint8_t schUpdateToKpiActiveList(Inst inst, SchStatsGrp *grpInfo, bool isDlUpdate)
{
   CmLList  *node = NULLP;
   
   if(isDlUpdate == true)
   {
      /* If DL Kpi needs to update */
      SCH_ALLOC(node, sizeof(CmLList));
      if(node)
      {
...
*           [Step -4.1.2] Reconfigure stats group. If reconfigured successfuly,
*              then  goto Step -4.1.4.
*           [Step -4.1.3] If reconfiguration failed, then restart the timer
*           which we have stopped at step 4.1.1.  
*           [Step -4.1.4] Clean all statistics stored so far for this stats group.
*        [Step -4.2] Else fe fill the group related info in ine stats modification rsp's 
*           rejected list.
