...
/*******************************************************************
 *
 * @brief Fills DL MSg info
 *
 * @details
 *
 *    Function : schFillDlMsgInfo
 *
 *    Functionality: Fills DL MSg info
 *
 * @params[in] Cell Cb, UE Cb, DL BO Info
 * @return ROK     - success
 *         RFAILED - failure
 *
 * ****************************************************************/
uint8_t schFillDlMsgInfo(SchCellCb *cell, SchUeCb *ueCb, DlRlcBoInfo *dlBoInfo)
{
   uint16_t slot;
   SchDlSlotInfo *schDlSlotInfo = NULLP;

   SET_ONE_BIT(ueCb->ueIdx, cell->boIndBitMap);
   ueCb->dlLcCtxt[dlBoInfo->lcId].bo = dlBoInfo->dataVolume;
...
uint8_t MacSchDlRlcBoInfo(Pst *pst, DlRlcBoInfo *dlBoInfo)
{
   ...
   {
      schFillDlMsgInfo(cell, ueCb, dlBoInfo);
#if 0
