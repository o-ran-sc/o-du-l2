 ...
 *          [Step 2.2.1] If KPI is not successfully added, goto step 3
 *       [Step 2.3] If UL related report received then add the KPI into
 *          Ul related active KPI list.
 *          [Step 2.3.1] If KPI is not successfully added, goto step 3.
 ...
 * @details
 *
 *    Function :deleteNodeFrmActiveKpiList 
 *
 *    Functionality:
...
void deleteNodeFrmActiveKpiList(CmLListCp actvKpiList, PTR cmpNode)
{
   CmLList *kpiNode=NULLP;

   CM_LLIST_FIRST_NODE(&actvKpiList, kpiNode);
   while(kpiNode)
   {
      if(memcmp(&kpiNode->node, &cmpNode, sizeof(TotalPrbUsage)) == 0)
      {
         cmLListDelFrm(&actvKpiList, kpiNode);
         break;
      }
...
void deleteStatsGrpInfo(Inst inst, SchStatsGrp *statsGrpInfo)
{
   ...
   if(statsGrpInfo)
   {
      if(statsGrpInfo->kpiStats.dlTotalPrbUsage)
      {
         ...
         deleteNodeFrmActiveKpiList(schCb[inst].statistics.activeKpiList.dlTotPrbUseList, node);
      ...
      if(statsGrpInfo->kpiStats.ulTotalPrbUsage)
      {
         ...
         deleteNodeFrmActiveKpiList(schCb[inst].statistics.activeKpiList.ulTotPrbUseList, node);
