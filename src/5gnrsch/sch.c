...
void deleteStatsGrpInfo(Inst inst, SchStatsGrp *statsGrpInfo)
{
   if(statsGrpInfo)
   {
      ...
      {
         deleteNodeFrmActiveKpiList(&schCb[inst].statistics.activeKpiList.dlTotPrbUseList, (PTR) statsGrpInfo->kpiStats.dlTotalPrbUsage);
         SCH_FREE(statsGrpInfo->kpiStats.dlTotalPrbUsage, sizeof(TotalPrbUsage));
      }

      if(statsGrpInfo->kpiStats.ulTotalPrbUsage)
      {
         deleteNodeFrmActiveKpiList(&schCb[inst].statistics.activeKpiList.ulTotPrbUseList, (PTR) statsGrpInfo->kpiStats.ulTotalPrbUsage);
