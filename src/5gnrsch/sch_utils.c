...
{   
   ...
   uint16_t numDmrsRes = NUM_DMRS_SYMBOLS;
   uint16_t tbSf = 0;
//   uint16_t numPrbOvrHead = 0;
   ...
   * Nre = min(156,Nre') . nPrb                                      */
   ...
   nre = nreDash * numPrb;
   if(tbSf == 2)
      nInfo = ceil(nre * qm * numLayer * rValue/(1024.0 * 4));
   else if(tbSf == 1)
      nInfo = ceil(nre * qm * numLayer * rValue/(1024.0*2));
   else
      nInfo = ceil(nre * qm * numLayer * rValue/1024.0);
