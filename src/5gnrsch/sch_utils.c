...
void schUpdValY(SchUeCb *ueCb, SchPdcchInfo *pdcchInfo)
{
   ...
   SCH_ALLOC(ueCb->pdcchInfo[0].y, (sizeof(uint32_t) * numSlotsPerRadioFrame));
   ...
   for(slotIdx= 0 ; slotIdx < numSlotsPerRadioFrame; slotIdx++)
   {
      if(slotIdx == 0)
      {
         pdcchInfo->y[slotIdx] = schCalY(ueCb->pdcchInfo[0].cRSetRef->cRSetId, ueCb->crnti);
      }
      else
      {
         pdcchInfo->y[slotIdx] = schCalY(ueCb->pdcchInfo[0].cRSetRef->cRSetId, pdcchInfo->y[slotIdx - 1]);
      }
   }
}
/**********************************************************************
