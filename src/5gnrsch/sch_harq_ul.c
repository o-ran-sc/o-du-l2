...
{
   HQ_LOG(1,hqP->procId,"\nANKIT schUlReleaseHqProcess proc[%2d] togNdi[%d]\n",hqP->procId, togNdi);
...
{
   HQ_LOG(1,hqP->procId,"\nANKIT schUlHqProcessNack txCntr[%2d] maxTx[%2d] node[%d] \n",hqP->tbInfo.txCntr , hqP->maxHqTxPerHqP,hqP->ulHqProcLink.node);
