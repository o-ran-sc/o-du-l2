...
dlrate_kwu += sdu->sduSz;
   ...
   if (rbCb->ueCb->tenbStats)
   {
      ...
      kwWinSz = RLC_AM_TRANS_WIN_SIZE(&AMDL);
...
#ifdef LTE_L2_MEAS_RLC
         kwUtlUpdSduSnMap(rbCb, sdu, rlcDatReq, TRUE);
#endif /*  LTE_L2_MEAS */
...
#ifdef LTE_L2_MEAS_RLC
         kwUtlUpdSduSnMap(rbCb, sdu, rlcDatReq, FALSE);
#endif /*  LTE_L2_MEAS */
...
#endif
{
   ...
   RlcTx *txBuf = kwUtlGetTxBuf(AMDL.txBufLst, sn);
