...
#endif
{
   ...
   if(gCb->genCfg.rlcMode == LKW_RLC_MODE_DL)
   {
      ...
      RLC_ALLOC(gCb,gCb->u.dlCb->rlcKwuDlSap, kwSapSize);
      ...
      {
         RLC_FREE(gCb,gCb->u.dlCb->rlcKwuDlSap, kwSapSize);
         RLC_FREE(gCb,gCb->u.dlCb, sizeof (RlcDlCb));
         RLOG0(L_FATAL,"Memory Allocation failed");   
         return (LCM_REASON_MEM_NOAVAIL);
      }
...
/*Pradeep: changing the SRegTmrMt() to SRegTmr()*/
   ...
   else if(gCb->genCfg.rlcMode == LKW_RLC_MODE_UL)
   {
      ...
      RLC_ALLOC(gCb,gCb->u.ulCb->rlcKwuUlSap, kwSapSize);
      ...
      if(gCb->u.ulCb->rlcKwuUlSap == NULLP)
      {
         RLC_FREE(gCb,gCb->u.ulCb->rlcKwuUlSap, kwSapSize);
         RLC_FREE(gCb,gCb->u.ulCb, sizeof (RlcUlCb));
         RLOG0(L_FATAL,"Memory Allocation failed");   
         return (LCM_REASON_MEM_NOAVAIL);
      }
   ...
   gCb->kwTqCp.tmrLen = RLC_TMR_LEN;

   cmMemset((U8*)gCb->kwTq, NULLP, sizeof(CmTqType) * RLC_TMR_LEN);
...
#endif
{
   ...
   {
      RlcL2MeasEvtCb* measEvtCb = &(tRlcCb->u.ulCb->kwL2Cb.kwL2EvtCb[cntr]);
...
/*Pradeep: changing the SDeregTmrMt() to SDeregTmr*/
      ...
      kwSapSize = (Size)(gCb->genCfg.maxKwuSaps * sizeof(RlcKwuSapCb));

      if (gCb->genCfg.rlcMode == LKW_RLC_MODE_DL)
      {
         kwUdxSapSize = (Size)(gCb->genCfg.maxUdxSaps * sizeof(RlcUdxDlSapCb));
         if (gCb->u.dlCb)
         {
      ...
      {
         kwUdxSapSize = (Size)(gCb->genCfg.maxUdxSaps * sizeof(RlcUdxUlSapCb));
         if (gCb->u.ulCb)
         {
            if (gCb->u.ulCb->rlcKwuUlSap != NULLP)
            {
