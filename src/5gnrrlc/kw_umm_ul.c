...
#endif
{
   ...
   while (count < pduCount)
   {
    ...
    /* Changed the condition to TRUE from ROK  */
	 ...
	 tmrRunning = rlcChkTmr(gCb,(PTR)rbCb, RLC_EVNT_UMUL_REASSEMBLE_TMR);
	 ...
	 if (tmrRunning) 
	 {
	    ...
	    recBuf = rlcUtlGetUmRecBuf(RLC_UMUL.recBufLst,*vrUr);
            ...
            {
               rlcStopTmr(gCb,(PTR)rbCb, RLC_EVNT_UMUL_REASSEMBLE_TMR);             
               tmrRunning = FALSE;
	 ...
	 if (!tmrRunning)
	 {
                 ...
                 && (recBuf && (!recBuf->noMissingSeg))))
            {
                ...
                rlcStartTmr(gCb, (PTR)rbCb, RLC_EVNT_UMUL_REASSEMBLE_TMR); 
                *vrUx = *vrUh;
            }
	 }
      }
...
)
{
   ...
   if(TRUE == rlcChkTmr(gCb,(PTR)rbCb,RLC_EVNT_UMUL_REASSEMBLE_TMR))
   {
       rlcStopTmr(gCb,(PTR)rbCb,RLC_EVNT_UMUL_REASSEMBLE_TMR);
   }
