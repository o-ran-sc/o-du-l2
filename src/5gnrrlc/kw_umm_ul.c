 ...
 * @return  Void
*/ 
static void rlcUmmFindNextVRUR (RlcCb *gCb, RlcUmUl* umUl, RlcSn nextSn)
{
#ifdef NR_RLC_UL
    ...
    /* Changed the condition to TRUE from ROK  */
	 ...
	 continue;
      ...
      if (seqNum < ur)
      {
         ...
         DU_LOG("\nRLC : rlcUmmProcessPdus: Received an unexpected pdu with sn %d \
      ...
      if(rlcUmmAddRcvdSeg(gCb, rbCb, &umHdr, pdu, pduSz) == TRUE)      
      {
         ...
         if (recBuf == NULLP)
         {
            DU_LOG("\nERROR  -->  RLC: rlcUmmProcessPdus: recBuf is NULLP UEID:%d CELLID:%d", \
               rbCb->rlcId.ueId, rbCb->rlcId.cellId);
            continue;
         }
         ...
         if(recBuf != NULLP && recBuf->allSegRcvd)
         {
            ...
            DU_LOG("\nDEBUG  -->  RLC: rlcUmmProcessPdus: Assembled the Sdus for  sn = %d UEID:%d CELLID:%d",\
            ...
            if (seqNum == ur)
            {
               ...
               rlcUmmFindNextVRUR(gCb, &RLC_UMUL, nextVrUr);
            }
         }
         /* If curSn is outside re-assembly window */
         else if (!rlcUmmCheckSnInReassemblyWindow(curSn,&RLC_UMUL))
         {  
            DU_LOG("\nDEBUG  -->  RLC: rlcUmmProcessPdus: curent sn is outSide the re-Assembly window. \
	    ...
	    /* update RX_NEXT_Highest */
            *vrUh  = (curSn + 1) & RLC_UMUL.modBitMask;
            ...
            if (!rlcUmmCheckSnInReassemblyWindow(*vrUr,&RLC_UMUL))
            {
               ...
               RlcSn nextVrUr = (*vrUh - RLC_UMUL.umWinSz) &  RLC_UMUL.modBitMask;
	       rlcUmmFindNextVRUR(gCb, &RLC_UMUL, nextVrUr);
	 ...
	 tmrRunning = rlcChkTmr(gCb,(PTR)rbCb, RLC_EVT_UMUL_REASSEMBLE_TMR);
         RlcSn tRxNextReassembly = RLC_UM_GET_VALUE(*vrUr ,RLC_UMUL);
         RlcSn tRxNextReassemblyNxt = (*vrUr + 1) & rbCb->m.umUl.modBitMask;
         RlcSn tRxNextHighest = RLC_UM_GET_VALUE(*vrUh, RLC_UMUL);
	 ...
	 if (tmrRunning) 
	 {
	    ...
	    uint8_t ret = rlcUmmCheckSnInReassemblyWindow(*vrUx, &RLC_UMUL);
                ...
                (tRxNextHighest ==  tRxNextReassemblyNxt && recBuf && recBuf->noMissingSeg))
            {
               ...
               tmrRunning = FALSE;
               DU_LOG("\nINFO  --> RLC: rlcUmmProcessPdus: Stopped ReAssembly Timer rxTimerTigger = %d \
	 ...
	 if (!tmrRunning)
	 {
            ...
            {
                DU_LOG("\nDEBUG  -->  RLC: rlcUmmProcessPdus: Start ReAssembly Timer tRxNextReassemblyNxt = %d \
...
#endif      
      {
         DU_LOG("\nERROR  -->  RLC: rlcUmmProcessPdus:Failed to assemble the PDU for SN  = %d UEID:%d CELLID:%d",\
            umHdr.sn, rbCb->rlcId.ueId, rbCb->rlcId.cellId);
...
)
{
   ...
   rlcUmmFindNextVRUR(gCb, &RLC_UMUL, RLC_UMUL.vrUx);
