...
uint8_t rlcUtlSendUlDataToDu(RlcCb *gCb, RlcUlRbCb *rbCb, Buffer *sdu)
{
   ...
   if (datIndInfo == NULLP)
   {
      DU_LOG("\nERROR  -->  RLC UL: rlcUtlSendUlDataToDu: Memory allocation failed UEID:%d \
   ...
   if(rbCb->rlcId.rbType == RB_TYPE_SRB)
   {
	 ...
	 {
	    DU_LOG("\nERROR  -->  RLC UL: rlcUtlSendUlDataToDu: Memory allocation failed for rrcMsg");
      ...
      {
	 DU_LOG("\nERROR  -->  RLC UL: rlcUtlSendUlDataToDu: Memory allocation failed for ulRrcMsgInfo");
	 return RFAILED;
      }
   }
   else if(rbCb->rlcId.rbType == RB_TYPE_DRB)
   {
	 ...
	 ulUserDatInfo->lcId = rbCb->lch.lChId;
	 ...
	 {
	    DU_LOG("\nERROR  -->  RLC UL: rlcUtlSendUlDataToDu: Memory allocation failed for user data");
      ...
      {
	 DU_LOG("\nERROR  -->  RLC UL: rlcUtlSendUlDataToDu: Memory allocation failed for ulUserDatInfo");
	 return RFAILED;
      }
   }
   return ROK;
...
void rlcUtlDelUmRecBuf(RlcCb *gCb, CmLListCp *recBufLst, RlcUmRecBuf  *recBuf)
{
   ...
   recBufLstCp = &recBufLst[hashKey];
   cmLListDelFrm(recBufLstCp, &recBuf->lnk);
   RLC_FREE_WC(gCb, recBuf, sizeof(RlcUmRecBuf));
