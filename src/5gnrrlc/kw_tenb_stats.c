...
#endif
{
   ...
   for (cnt=0; cnt < L2_STATS_MAX_CELLS; cnt++)
   {
        ...
        memset((U8 *)l2CellStats[cnt], 0x00, (Size)sizeof(TSL2CellStatsCb));
   }
   ...
   for (cnt=0; cnt < L2_STATS_MAX_UES; cnt++)
   {
        ...
        memset((U8 *)statsCb, 0x00, (Size)sizeof(TSL2UeStatsCb));
...
#endif
{
   ...
   for (idx = 0; idx < L2_STATS_MAX_UES; idx++)
   {
      ...
      memset((U8 *)&statsCb->stats.nonPersistent, 0x00, (Size)sizeof(statsCb->stats.nonPersistent));
      /* memset((U8 *)&statsCb->stats, 0x00, (Size)sizeof(TSInfL2UeStats)); */
   ...
   for (idx = 0; idx < L2_STATS_MAX_CELLS; idx++)
   {
      ...
      memset((U8 *)l2CellStats[idx], 0x00, (Size)sizeof(TSInfL2CellStats));
