...
/* kw005.201 added support for L2 Measurement */
      ...
      case EVENT_RLC_UE_THROUGHPUT_TMR:
      {
         ...
         TMR_CALCUATE_WAIT(arg.wait, gConfigInfo.ueThrptTimeIntervl, gCb->genCfg.timeRes);
      ...
      {
         RlcThpt *thptCb = (RlcThpt *)cb;
         TMR_CALCUATE_WAIT(arg.wait, gConfigInfo.snssaiThrptTimeIntervl, gCb->genCfg.timeRes);
   ...
   if(rlcThptCb->ueTputInfo.numActvUe)
   {
      for(ueIdx = 0; ueIdx < MAX_NUM_UE; ueIdx++)
      {
         if(rlcThptCb->ueTputInfo.thptPerUe[ueIdx].ueId)
         {
             ...
             */
             tpt = (double)(rlcThptCb->ueTputInfo.thptPerUe[ueIdx].dataVol * 8)/(double)gConfigInfo.ueThrptTimeIntervl;
