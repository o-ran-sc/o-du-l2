...
EXTERN S16 kwHdlCkwUlCfgReq ARGS((RlcCb  *gCb,RlcUlCfgTmpData *cfgTmpData,
...
#endif
{
   ...
   kwHdlCkwUlCfgReq(tRlcCb,cfgTmpData, cfmInfo, cfgCfm);
...
#endif
{
   ...
   if(status.status == CKW_CFG_CFM_OK)
   {
       ...
       {
      kwCfgApplyUlUeIdChng(tRlcCb, cfgTmpData->ueInfo, cfgTmpData->newUeInfo, cfgTmpData);
   }
   }
