...
)
{
   ...
   switch (udxSap->state)
   {
      ...
      {
         if(TRUE == rlcChkTmr(tRlcCb,(PTR)udxSap,RLC_EVNT_WAIT_BNDCFM))
         {
             rlcStopTmr (tRlcCb,(PTR)udxSap, RLC_EVNT_WAIT_BNDCFM);
         }
         udxSap->retryCnt = 0;
...
)
{
   ...
   /* Start staProhTmr */
   rlcStartTmr(tRlcCb,(PTR)rbCb, RLC_EVNT_AMUL_STA_PROH_TMR);
