 ...
 *
 * @params[in] uint8_t cellId, uint8_t ueId, CausesOfResult cause 
 *
 * @return ROK     - success
 *         RFAILED - failure
 *
 * ****************************************************************/
uint8_t sendRlcUeDeleteRspToDu(uint16_t cellId,uint8_t ueId, CausesOfResult cause)
{
   Pst pst;  
...
uint8_t RlcProcUeDeleteReq(Pst *pst, RlcUeDelete *ueDelete)
{
   ...
   if(ueDelete != NULLP)
   {
      ...
      if(cause != SUCCESSFUL)
      {
         ret = sendRlcUeDeleteRspToDu(ueDelete->cellId, ueDelete->ueId, cause );
         if(ret != ROK)
         {
