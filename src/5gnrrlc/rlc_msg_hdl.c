 ...
 * @details
 *
 *    Function : RlcMacProcSchResultRpt
 *
 *    Functionality:
 ...
 *
 * ****************************************************************/
uint8_t RlcMacProcSchResultRpt(Pst *pst, RlcSchResultRpt *schRep)
{
   ...
   if(pst->selector == ODU_SELECTOR_LWLC)
   {
      KW_FREE_SHRABL_BUF(pst->region, pst->pool, schRep, sizeof(RlcSchResultRpt));
   }
   return ROK;
}
