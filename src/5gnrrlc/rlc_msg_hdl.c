 ...
 * @details
 *
 *    Function : sendRlcUeReestablishmentRspToDu 
 *
 *    Functionality:
 ...
 *
 * ****************************************************************/
uint8_t sendRlcUeReestablishmentRspToDu(uint16_t cellId,uint8_t ueId, CauseOfResult  status)
{
   Pst pst;  
   RlcUeReestablishmentRsp *ueReestablishmentRsp = NULLP;
   
   FILL_PST_RLC_TO_DUAPP(pst, RLC_UL_INST, EVENT_RLC_UE_REESTABLISHMENT_RSP);

   RLC_ALLOC_SHRABL_BUF(pst.region, pst.pool, ueReestablishmentRsp, sizeof(RlcUeReestablishmentRsp));
   if(!ueReestablishmentRsp)
   {
      DU_LOG("\nERROR  -->  RLC: sendRlcUeReestablishmentRspToDu(): Memory allocation failed ");
      return RFAILED;
   }
   else
   {
      ...
      {
         DU_LOG("\nERROR  -->  RLC: SendRlcUeReestablishmentRspToDu():Failed to send UE Reestablishment response to DU");
         RLC_FREE_SHRABL_BUF(pst.region, pst.pool, ueReestablishmentRsp, sizeof(RlcUeReestablishmentRsp));
         return RFAILED;
      }
   }
   return ROK;
...
uint8_t RlcProcUeReestablishReq(Pst *pst, RlcUeReestablishReq *ueReestablishmentReq)
{
   uint8_t ret = RFAILED;
   RlcCb *gRlcCb = NULLP;
   RlcUlUeCb *ueCb = NULLP;
   ...
   if(ueReestablishmentReq != NULLP)
   {
      gRlcCb = RLC_GET_RLCCB(pst->dstInst);
      rlcDbmFetchUlUeCb(gRlcCb,ueReestablishmentReq->ueId, ueReestablishmentReq->cellId, &ueCb);
      if(ueCb != NULLP)
      {
         if(ueReestablishmentReq->cellId == ueCb->cellId)
         {
            /* TODO : 
             * Step 1: Fill the RlcCfgInfo structure with data from the ueReestablishmentReq, just as we did in fillRlcCfg function and set 
         ...
         {
            status = CELLID_INVALID;
            DU_LOG("\nERROR  -->  SCH : RlcProcUeReestablishReq(): cell Id[%d] not found", ueReestablishmentReq->cellId);
         }
      }
      else
      {
         status = UEID_INVALID;
         DU_LOG("\nERROR  -->  SCH : RlcProcUeReestablishReq(): ue Id[%d] not found", ueReestablishmentReq->cellId);
      }

      if(status != SUCCESSFUL)
      {
         if(sendRlcUeReestablishmentRspToDu(ueReestablishmentReq->cellId, ueReestablishmentReq->ueId, status)  != ROK)
         {
            DU_LOG("\nERROR  -->  RLC: RlcProcUeReestablishReq():Failed to send UE Reestablishment response to DU");
         }
      }
      RLC_FREE_SHRABL_BUF(pst->region, pst->pool, ueReestablishmentReq, sizeof(RlcUeDelete));
   }
   else
   {
