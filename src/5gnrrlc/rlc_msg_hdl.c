...
uint8_t sendRlcUeDeleteRspToDu(Pst *pst, uint8_t ueIdx, uint16_t cellId, UeDeleteResult result)
{
   ...
   else
   {
      ...
      FILL_PST_RLC_TO_DUAPP(post, RLC_UL_INST, EVENT_RLC_UE_DELETE_RSP);
      if(rlcSendUeDeleteRspToDu(&post, ueDeleteRsp) == ROK)
      {
...
uint8_t RlcProcUeDeleteReq(Pst *pst, RlcUeDelete *ueDelete)
{
   ...
   if(ueDelete != NULLP)
   {
      ...
      {
         ret = sendRlcUeDeleteRspToDu(pst, ueDelete->ueIdx, ueDelete->cellId, result);
         if(ret != ROK)
         {
