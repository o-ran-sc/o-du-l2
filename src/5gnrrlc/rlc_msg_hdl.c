...
uint8_t RlcUlProcUeCreateReq(Pst *pst, RlcUeCfg *ueCfg)
{
   ...
   if(rlcUeCfg == NULLP)
   {
      DU_LOG("\nRLC: Failed to allocate memory at RlcUlProcUeCreateReq()");
      ret = RFAILED;
   }
   else
   {
      ...
      if(ret != ROK)
         DU_LOG("\nRLC: Failed to configure Add/Mod/Del entities at RlcUlProcUeCreateReq()");
...
uint8_t RlcUlProcUeReconfigReq(Pst *pst, RlcUeCfg *ueCfg)
{
   ...
   if(rlcUeCfg == NULLP)
   {
      DU_LOG("\nRLC: Failed to allocate memory at RlcUlProcUeReconfigReq()");
      ret = RFAILED;
   }
   else
   {
      ...
      ret = RlcProcCfgReq(pst, rlcUeCfg);
      if(ret != ROK)
         DU_LOG("\nRLC: Failed to configure Add/Mod/Del entities at RlcUlProcUeReconfigReq()");
   }
