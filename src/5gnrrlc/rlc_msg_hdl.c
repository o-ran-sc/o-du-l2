...
uint8_t RlcUlHdlUeCreateReq(Pst *pst, RlcUeCfg *ueCfg)
{
   ...
   KwCb *rlcCb = NULLP;
   rlcCb = RLC_GET_RLCCB(pst->dstInst);
   RLC_ALLOC(rlcCb, rlcUeCfg, sizeof(CkwCfgInfo));
   if(rlcUeCfg)
   {
      ...
      for(idx = 0; idx < ueCfg->numLcs; idx++)
      {
         ...
         switch(rlcUeCfg->entCfg[idx].entMode)
         {
            ...
            case CM_LTE_MODE_AM:
            {
               ...
               /* UL AM INFO */
               rlcUeCfg->entCfg[idx].lCh[1].lChId   = ueCfg->rlcBearerCfg[idx].lcId;   
               rlcUeCfg->entCfg[idx].lCh[1].type    = ueCfg->rlcBearerCfg[idx].lcType;
