 ...
 *
 * ****************************************************************/
void fillRlcCfgCfmInfo(RlcCfgCfmInfo *cfgRsp, RlcUeCfg *ueCfg)
{
   uint8_t cfgIdx =0;
   ...
   for(cfgIdx =0; cfgIdx<ueCfg->numLcs; cfgIdx++)
   {
      ...
      cfgRsp->entCfgCfm[cfgIdx].status.status = CKW_CFG_CFM_NOK;
...
uint8_t RlcProcUeCreateReq(Pst *pst, RlcUeCfg *ueCfg)
{
   ...
   else
   {
      ...
      if(ret != ROK)
      {
         ...
         fillRlcCfgCfmInfo(&cfgRsp, ueCfg);
