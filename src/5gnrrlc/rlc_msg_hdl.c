 ...
 *
 * ****************************************************************/
uint8_t fillLcCfg(RlcEntCfgInfo *rlcUeCfg, RlcBearerCfg *duRlcUeCfg, uint8_t cfgType)
{
   uint8_t lChRbIdx = 0;
...
uint8_t fillRlcCfg(RlcCfgInfo *rlcUeCfg, RlcUeCfg *ueCfg)
{
   uint8_t ret, lcIdx;
   ret = ROK;
   ...
   for(lcIdx = 0; lcIdx < rlcUeCfg->numEnt; lcIdx++)
   {
      ...
      {
         ret  = fillLcCfg(&rlcUeCfg->entCfg[lcIdx], &ueCfg->rlcLcCfg[lcIdx], CKW_CFG_ADD);
         if(ret == RFAILED)
         {
      ...
      {
         ret  = fillLcCfg(&rlcUeCfg->entCfg[lcIdx], &ueCfg->rlcLcCfg[lcIdx], CKW_CFG_MODIFY);
         if(ret == RFAILED)
         {
      ...
      {
         ret  = fillLcCfg(&rlcUeCfg->entCfg[lcIdx], &ueCfg->rlcLcCfg[lcIdx], CKW_CFG_DELETE);
         if(ret == RFAILED)
         {
...
uint8_t RlcUlProcUeCreateReq(Pst *pst, RlcUeCfg *ueCfg)
{
   ...
   if(rlcUeCfg)
   {
      ...
      ret = fillRlcCfg(rlcUeCfg, ueCfg); 
      if(ret == ROK)
         ret = RlcProcCfgReq(pst, rlcUeCfg);
      else
         DU_LOG("\nRLC: Failed at fillRlcCfg");
