 ...
 * @details
 *
 *    Function : BuildAndSendRlcMaxRetransInfoToDu 
 *
 *    Functionality:
 ...
 *
 * ****************************************************************/
uint8_t BuildAndSendRlcMaxRetransInfoToDu(uint16_t cellId,uint8_t ueId, uint8_t lcId, uint8_t lcType)
{
   Pst pst;  
   RlcMaxRetransInfo *retransmissionInfo = NULLP;
   
   FILL_PST_RLC_TO_DUAPP(pst, RLC_UL_INST, EVENT_RLC_MAX_RETRANSMISSION_INFO);

   RLC_ALLOC_SHRABL_BUF(pst.region, pst.pool, retransmissionInfo, sizeof(RlcMaxRetransInfo));
   if(!retransmissionInfo)
   {
      DU_LOG("\nERROR  -->  RLC: sendRlcMaxRetransInfoToDu(): Memory allocation failed ");
      return RFAILED;
   }
   else
   {
      retransmissionInfo->cellId = cellId;
      retransmissionInfo->ueId = ueId;
      retransmissionInfo->lcId = lcId;
      retransmissionInfo->lcType = lcType;
  
      if(rlcSendMaxRetransmissionReachedInfoToDu(&pst, retransmissionInfo) == ROK)
      {
         DU_LOG("\nDEBUG  -->  RLC: UE [ %d] max retransmission information sent successfully",ueId);
      }
      else
      {
         DU_LOG("\nERROR  -->  RLC: SendRlcMaxRetransInfoToDu(): fail to send UeId's[%d] maximum retransmission information", ueId);
         RLC_FREE_SHRABL_BUF(pst.region, pst.pool, retransmissionInfo, sizeof(RlcMaxRetransInfo));
         return RFAILED;
      }
   }
   return ROK;
