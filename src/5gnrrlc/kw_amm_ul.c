               ...
               (recBuf->segLst.count > 0))
      {
         ...
         while (seg != NULLP && nackCnt < RLC_MAX_NACK_CNT)
         {
            ...
            if (seg->amHdr.so != seqSo)
            {
               ...
               DU_LOG("\nERROR  -->  RLC_UL : rlcAmmUlAssembleCntrlInfo: Missing byte segment's" 
                  " SN:%d UEID:%d CELLID:%d", sn, rbCb->rlcId.ueId, rbCb->rlcId.cellId);
               DU_LOG("\nINFO  -->  RLC_UL : rlcAmmUlAssembleCntrlInfo: soStart and soEnd = %d, %d \
             ...
             (seg->amHdr.si != RLC_SI_LAST_SEG && nackCnt < RLC_MAX_NACK_CNT))
         {
            ...
            DU_LOG("\nERROR  -->  RLC_UL : rlcAmmUlAssembleCntrlInfo: Missing (last) byte " 
...
#endif
{
   ...
   DU_LOG("\nINFO  -->  RLC_UL : rlcAmmProcessPdus: numPdu[%d],numPduToProcess[%d] UEID:%d CELLID:%d",
	 ...
	 continue;
      ...
      if (rlcAmmUlPlacePduInRecBuf(gCb,pdu, rbCb, &amHdr) == TRUE)
      {
         ...
         if (mSn >= mrxNextHighestRcvd)
         {
            ...
            DU_LOG("\nINFO  -->  RLC_UL : rlcAmmProcessPdus: Updated rxNextHighestRcvd = %d UEID:%d CELLID:%d",
         ...
         if (!tmrRunning)
         {
            if (amUl->rxNextHighestRcvd > amUl->rxNext)
            {
               ...
               DU_LOG("\nINFO  -->  RLC_UL : rlcAmmProcessPdus: Updated rxNextStatusTrig = %d \
...
static uint8_t rlcAmmExtractHdr(RlcCb *gCb, RlcUlRbCb *rbCb, Buffer *pdu, RlcAmHdr *amHdr, uint8_t *fByte)
{
   ...
   {
   //DU_LOG ("INFO  -->  ++++++++++++ 5GNRLOG HDR extracted CTRL : \n");
      return ROK;
   }
...
static void rlcAmmUlHndlStatusPdu(RlcCb *gCb, RlcUlRbCb *rbCb, Buffer *cntrlPdu, uint8_t *fByte)
{
   ...
   //DU_LOG ("INFO  -->  ++++++++++++ 5GNRLOG HNDL STATUS acksn %d : \n",  pStaPdu->ackSn);
   /* Check if NACK Exists */
   hdrInfo.len = RLC_E1_LEN;
   rlcAmmExtractElmnt(gCb, cntrlPdu, &hdrInfo);
   e1 = (uint8_t)hdrInfo.val;
   DU_LOG("\nINFO  -->  RLC_UL : rlcAmmUlHndlStatusPdu: ACK SN = %d UEID:%d CELLID:%d",
   ...
   while (e1 && (pStaPdu->nackCnt < RLC_MAX_NACK_CNT))
   {
      ...
      if (pStaPdu->nackInfo[pStaPdu->nackCnt].isSegment)
      {
         ...
         DU_LOG("\nINFO  -->  RLC_UL : rlcAmmUlHndlStatusPdu: soStart and soEnd = %d %d"
