    ...
    /* Changed the condition to TRUE from ROK  */
	 ...
	 continue;
      ...
      if (rlcAmmUlPlacePduInRecBuf(gCb,pdu, rbCb, &amHdr) == TRUE)
      {
         ...
         tmrRunning = rlcChkTmr(gCb,(PTR)rbCb, RLC_EVNT_AMUL_REORD_TMR);
         if (tmrRunning)
         {
            ...
            {
               rlcStopTmr(gCb,(PTR)rbCb, RLC_EVNT_AMUL_REORD_TMR);
               tmrRunning = FALSE;
            }
         }
         ...
         if (!tmrRunning)
         {
            ...
            {
               rlcStartTmr(gCb,(PTR)rbCb, RLC_EVNT_AMUL_REORD_TMR);
...
static void rlcAmmTriggerStatus(RlcCb *gCb, RlcUlRbCb *rbCb, RlcSn sn, bool discFlg)
{
   ...
   if ((discFlg) || (tSn < trxHighestStatus) || (tSn >= tVrMr))
   {
      ...
      tmrRunning = rlcChkTmr(gCb,(PTR) rbCb, RLC_EVNT_AMUL_STA_PROH_TMR);
...
Void rlcAmmUlReEstablish(RlcCb *gCb,CmLteRlcId rlcId,Bool sendReEst,RlcUlRbCb  *rbCb)
{
   ...
   if(TRUE == rlcChkTmr(gCb,(PTR)rbCb,RLC_EVNT_AMUL_REORD_TMR))
   {
       rlcStopTmr(gCb,(PTR)rbCb, RLC_EVNT_AMUL_REORD_TMR);
   }
   if(TRUE == rlcChkTmr(gCb,(PTR)rbCb,RLC_EVNT_AMUL_STA_PROH_TMR))
   {
       rlcStopTmr(gCb,(PTR)rbCb, RLC_EVNT_AMUL_STA_PROH_TMR);
   }
