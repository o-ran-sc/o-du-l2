...
S16 rlcUtlL2MeasDlInit(RlcCb *gCb)
{
   ...
   {
      cmMemset((U8 *)&(gCb->u.dlCb->kwL2Cb.kwL2EvtCb[cntr]), 0, sizeof(RlcL2MeasEvtCb));
   }
   gCb->u.dlCb->kwL2Cb.kwL2EvtCb[RLC_L2MEAS_DL_DISC].measCb.measType = LKW_L2MEAS_DL_DISC;
   gCb->u.dlCb->kwL2Cb.kwL2EvtCb[RLC_L2MEAS_DL_IP].measCb.measType = LKW_L2MEAS_DL_IP;
   gCb->u.dlCb->kwL2Cb.kwL2EvtCb[RLC_L2MEAS_DL_DELAY].measCb.measType= LKW_L2MEAS_DL_DELAY;
   gCb->u.dlCb->kwL2Cb.kwL2EvtCb[RLC_L2MEAS_UU_LOSS].measCb.measType= LKW_L2MEAS_UU_LOSS;
   gCb->u.dlCb->kwL2Cb.kwL2EvtCb[RLC_L2MEAS_ACT_UE].measCb.measType= LKW_L2MEAS_ACT_UE;
...
#ifdef ANSI
RlcL2MeasTb * kwUtlGetCurMeasTb
(
RlcCb     *gCb,
RlcDlRbCb *rbCb
)
#else
RlcL2MeasTb * kwUtlGetCurMeasTb(gCb, rbCb)
RlcCb     *gCb;
RlcDlRbCb *rbCb;
#endif
{
...
#endif
{
   ...
   for(lcIdx = 0; ((lcIdx < l2MeasTb->numLcId) && (lcIdx < RLC_MAX_ACTV_DRB)); lcIdx++)
   {
      ...
      /* fix for DL IP stop*/
      if (!gCb->u.dlCb->kwL2Cb.measOn[rlcRbCb->qci]
...
Void rlcUtlResetDlL2MeasInKwRb
(
RlcCb       *gCb,
RlcL2MeasCb *measCb,
U8             measType
...
#else
Void rlcUtlResetDlL2MeasInKwRb(gCb, measCb, measType)
RlcCb       *gCb;
RlcL2MeasCb *measCb;
U8             measType;
#endif
...
PRIVATE Void dumpRLCDlRbInformation(RlcDlRbCb* dlRbCb)
{
   ...
   else if(dlRbCb->mode == CM_LTE_MODE_AM)
   {
      ...
      {
         RlcTx *txBuf = kwUtlGetTxBuf(dlRbCb->m.amDl.txBufLst, j);
         if(txBuf != NULLP)
         {
            numTxPdus++;
         }
...
#endif
{
   RlcuFlowCntrlIndInfo    *flowCntrlInfo;
   RlcKwuSapCb*            rlckwuSap;
...
#else
RlcTx* kwUtlGetTxBuf(txBufLst, sn)
CmLListCp        *txBufLst;
RlcSn             sn;
#endif
{
