...
Void rlcDlUtlSetReestInProgressForRB(RlcCb *gCb,RlcDlRbCb *rbCb)
{
   ...
   if(rbCb->mode == RLC_MODE_AM )
   {
      ...
      if(rlcChkTmr(gCb, (PTR)rbCb, RLC_EVNT_AMDL_POLL_RETX_TMR))
      {
         rlcStopTmr(gCb, (PTR)rbCb, RLC_EVNT_AMDL_POLL_RETX_TMR);
      }
   }
   else
   {
   ...
   for(rbIdx = 0;rbIdx < RLC_MAX_SRB_PER_UE;rbIdx++)
   {
      if(ueCb->srbCb[rbIdx] != NULLP)
      {
         ...
         else
         {
            ...
            if(rlcChkTmr(gCb, (PTR)ueCb->srbCb[rbIdx], RLC_EVNT_AMDL_POLL_RETX_TMR))
            {
               rlcStopTmr(gCb, (PTR)ueCb->srbCb[rbIdx], RLC_EVNT_AMDL_POLL_RETX_TMR);
            }
         }
      }
   }
