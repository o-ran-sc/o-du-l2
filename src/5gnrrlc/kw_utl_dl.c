...
Void ResetRLCStats(Void)
{
   ...
   cmMemset((uint8_t*)&gRlcStats, 0, sizeof(RLCStats));
   cmMemset((uint8_t*)&dlInst->genSts,0,sizeof(RlcGenSts));
   cmMemset((uint8_t*)&ulInst->genSts,0,sizeof(RlcGenSts));
}
...
//Debug
   ...
   for(idx = 0; idx < staIndInfo->nmbOfUeGrantPerTti; idx++)
   {
      ...
      cmMemset( (uint8_t *)&datReq, 0, sizeof(RlcDatReq) ); 
      totNumPdu = 0;
...
S16 rlcUtlL2MeasDlInit(RlcCb *gCb)
{
   ...
   {
      cmMemset((uint8_t *)&(gCb->u.dlCb->rlcL2Cb.rlcL2EvtCb[cntr]), 0, sizeof(RlcL2MeasEvtCb));
   }
