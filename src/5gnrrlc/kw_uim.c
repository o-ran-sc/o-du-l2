...
)
{
   ...
   DU_LOG("DEBUG  -->  RLC : spId(%d), suId(%d)", spId, suId);
   ...
   switch(ckwSap->state)
   {
      ...
      case RLC_SAP_UBND:
      {
         ...
         DU_LOG("DEBUG  -->  RLC : RlcUiCkwBndReq: state (%d)", ckwSap->state);
         break;
      }
      case RLC_SAP_BND:
      {
             ...
             ckwSap->suId != suId)
         {
            ...
            DU_LOG("ERROR  -->  RLC : CKW SAP already Bound");
      ...
      {
#if (ERRCLASS & ERRCLS_INT_PAR)
         DU_LOG("ERROR  -->  RLC : Invalid CKW SAP State in Bind Req");
         RLC_SEND_SAPID_ALARM(tRlcCb,
                             spId, 
                             LKW_EVENT_CKW_BND_REQ, 
                             LCM_CAUSE_INV_STATE);
...
)
{
   ...
   DU_LOG("DEBUG  -->  RLC : spId(%d), reason(%d)", 
                spId, 
                reason);
...
)
{
   ...
   {
      DU_LOG("ERROR  -->  RLC : Addition to UL transId Lst Failed");
      RLC_PST_FREE(pst->region, pst->pool, cfg, sizeof(RlcCfgInfo));
...
)
{
   ...
   do
   {
      ...
      tRlcCb = RLC_GET_RLCCB(pst->dstInst);
#ifndef ALIGN_64BIT
      DU_LOG("DEBUG  -->  RLC : RlcUiCkwUeIdChgReq(pst, spId(%d), transId(%ld))", 
                   spId, 
                   transId);
#else
      DU_LOG("DEBUG  -->  RLC : RlcUiCkwUeIdChgReq(pst, spId(%d), transId(%d))\n", 
                 spId, transId);
#endif
   ...
   {
      DU_LOG("ERROR  -->  RLC : Validation Failure for UeId change");
   }
      ...
      switch(cfg->entCfg[idx].cfgType)
      {
         ...
         case CKW_CFG_DELETE:
         {
            ...
            if(cfg->entCfg[idx].dir & RLC_DIR_UL)
            {
               ...
               if(ROK != rlcCfgValidateUlRb(gCb,
                                           ...
                                           cfgTmpData))
               {
                  DU_LOG("ERROR  -->  RLC : CELLID [%u]:Validation Failure for UL RB [%d]",
         ...
         case CKW_CFG_REESTABLISH:   
         {
            if(cfg->entCfg[idx].dir & RLC_DIR_UL)
            {
               if(ROK != rlcCfgValidateReEstRb(gCb,
                                              ...
                                              &cfgTmpData->cfgEntData[idx]))
               {
                  DU_LOG("ERROR  -->  RLC : CellID [%u]:Validation Failure for Reest UL RB [%d]",
                        cfg->cellId,cfg->entCfg[idx].rbId);
                  cfgTmpData->cfgEntData[idx].entUlCfgCfm.status.status = CKW_CFG_CFM_NOK;
         ...
         case CKW_CFG_DELETE_UE :   
         {
            if(ROK != rlcCfgValidateDelUlUe(gCb,  
                                           ...
                                           cfgTmpData))
            {
               DU_LOG("ERROR  -->  RLC : UL UEID [%d]:Validation Failure",
                     cfgTmpData->ueId);
               cfgTmpData->cfgEntData[idx].entUlCfgCfm.status.status = CKW_CFG_CFM_NOK;
               /* Setting dir as UL, so that no configuration is done at DL */
               cfg->entCfg[idx].dir = RLC_DIR_UL;
         ...
         case CKW_CFG_DELETE_CELL :   
         {
            if(ROK != rlcCfgValidateDelUlCell(gCb,
                                             ...
                                             cfgTmpData))
            {
               DU_LOG("ERROR  -->  RLC : Del UL Cell Validation Failure");
               cfgTmpData->cfgEntData[idx].entUlCfgCfm.status.status = CKW_CFG_CFM_NOK;
               /* Setting dir as UL, so that no configuration is done at DL */
               cfg->entCfg[idx].dir = RLC_DIR_UL;
            }
...
)
{
...
#endif
   tRlcCb = RLC_GET_RLCCB(pst->dstInst);
   DU_LOG("DEBUG  -->  RL : RlcUiKwuBndReq(pst, spId(%d), suId(%d))", spId, suId);

    /* Validation of input parameters */
#if (ERRCLASS & ERRCLS_INT_PAR)
   if(!((spId < (S16) tRlcCb->genCfg.maxKwuSaps) && (spId >=0))) 
   {
      DU_LOG("ERROR  -->  RLC : Invalid spId");
   ...
   switch(rlckwuSap->state)
   {
      ...
      case RLC_SAP_UBND:
      {
         ...
         DU_LOG("DEBUG  -->  RLC : RlcUiKwuBndReq: state (%d)", rlckwuSap->state);
         break;
      }
      case RLC_SAP_BND:
      {
             ...
             rlckwuSap->suId != suId)
         {
            RLC_SEND_SAPID_ALARM(tRlcCb,
                                ...
                                LCM_CAUSE_INV_PAR_VAL);
            DU_LOG("ERROR  -->  RLC : RLC Mode [%d] : KWU SAP already Bound",
     ...
     default:
      {
#if (ERRCLASS & ERRCLS_INT_PAR)
         DU_LOG("ERROR  -->  RLC : RLC Mode [%d]:Invalid KWU SAP State in Bind Req",
               tRlcCb->genCfg.rlcMode);
         RLC_SEND_SAPID_ALARM(tRlcCb,
                             spId,
                             LKW_EVENT_KWU_BND_REQ, 
...
uint8_t rlcProcDlData(Pst *pst, KwuDatReqInfo *datReq, Buffer *mBuf)
{
   ...
   if(!rbCb)
   {
      DU_LOG("\nERROR  -->  RLC : CellId[%u]:DL RbId [%d] not found",
