...
S16 RlcLiRguBndCfm(Pst *pst,SuId suId,uint8_t status)
{
   ...
   DU_LOG("DEBUG  -->  RLC : RlcLiRguBndCfm(suId(%d), status(%d)", suId, status);

#if (ERRCLASS & ERRCLS_INT_PAR)
   if (tRlcCb->init.cfgDone != TRUE)
   {
      DU_LOG("ERROR  -->  RLC : General configuration not done");
   ...
   DU_LOG("DEBUG  -->  RLC : RlcLiRguBndCfm: For RGU SAP state=%d", rguSap->state);
...
uint8_t rlcProcCommLcUlData(Pst *pst, SuId suId, RguCDatIndInfo *datInd)
{
   ...
   {
      DU_LOG("\nERROR  -->  RLC : rlcProcCommLcUlData : Invalid LcId [%d], Max is [%d]",\
   ...
   if (!rbCb)
   {
      DU_LOG("\nERROR  -->  RLC : rlcProcCommLcUlData : LcId [%d] not found",
         datInd->lcId);
      RLC_SHRABL_STATIC_BUF_FREE(RLC_MEM_REGION_UL, RLC_POOL, datInd, sizeof(RguCDatIndInfo));
      return RFAILED;
   }
...
uint8_t rlcProcDedLcUlData(Pst *pst, SuId suId, RguDDatIndInfo *datInd)
{
   ...
   {
       DU_LOG("\nINFO  -->  RLC : rlcProcDedLcUlData : suId(%d))recieved in DL Inst", suId);
       RLC_SHRABL_STATIC_BUF_FREE(RLC_MEM_REGION_UL, RLC_POOL, datInd, sizeof(RguDDatIndInfo));
       return RFAILED;
   }
#endif
...
uint8_t rlcProcCommLcSchedRpt(Pst *pst, SuId suId, RguCStaIndInfo *staInd)
{
   ...
   if ((suId >= tRlcCb->genCfg.maxRguSaps) || (suId < 0))
   {
      DU_LOG("\nERROR  -->  RLC: rlcProcCommLcSchedRpt: Invalid RGU suId %d\n", suId);
      return RFAILED; 
   }
   if (tRlcCb->genCfg.rlcMode == LKW_RLC_MODE_UL)
   {
       DU_LOG("\nINFO  -->  RLC: rlcProcCommLcSchedRpt: Received in RLC UL CELLID:%d",
             staInd->cellId);
       RLC_SHRABL_STATIC_BUF_FREE(RLC_MEM_REGION_DL, RLC_POOL, staInd, sizeof(RguCStaIndInfo));
       return RFAILED;
   }
   ...
   {
      DU_LOG("\nERROR  -->  RLC: rlcProcCommLcSchedRpt: Invalid LcId, Max is [%d] CELLID:%d",
   ...
   if(!rbCb)                                               
   {
      DU_LOG("\nERROR  -->  RLC: rlcProcCommLcSchedRpt: LcId [%d] not found CELLID:%d",
...
uint8_t rlcProcDedLcSchedRpt(Pst *pst, SuId suId, RguDStaIndInfo *staInd)
{
   ...
   {
       DU_LOG("\nINFO  -->  RLC: rlcProcDedLcSchedRpt: Received in RLC UL ");
       RLC_SHRABL_STATIC_BUF_FREE(RLC_MEM_REGION_DL, RLC_POOL, staInd, sizeof(RguDStaIndInfo));
       return RFAILED;
   }
   if ((suId >= gCb->genCfg.maxRguSaps) || (suId < 0))
   {
      DU_LOG("\nERROR  -->  RLC: rlcProcDedLcSchedRpt: Invalid RGU suId %d\n", suId);
      return (RFAILED); 
   }
#endif
