...
void ProcE2NodeConfigUpdate(uint32_t duId, E2nodeConfigurationUpdate_t *e2NodeConfigUpdate)
{
   ...
   if(elementCnt == ieIdx)
   {
     if(BuildAndSendE2NodeConfigUpdateAck(duDb, transId, &tmpE2NodeList) !=ROK)
      ...
      BuildAndSendConnectionUpdate(duId);
   }
   else
   {
...
uint8_t fillE2connectionUpdateItem(E2connectionUpdate_Item_t *connectionModifyItem)
{
   ...
   connectionModifyItem->tnlInformation.tnlAddress.buf[0] = 192;
   connectionModifyItem->tnlInformation.tnlAddress.buf[1] = 168;
   connectionModifyItem->tnlInformation.tnlAddress.buf[2] = 130;
   connectionModifyItem->tnlInformation.tnlAddress.buf[3] = 81;
   connectionModifyItem->tnlInformation.tnlAddress.bits_unused = 0;
   connectionModifyItem->tnlUsage = TNLusage_both; 
   return ROK;
}
