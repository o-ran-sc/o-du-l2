...
/*******************************************************************
 *
 * @brief deallocate the memory allocated in E2Node Config UpdateFailure
 *
 * @details
 *
 *    Function : FreeE2ConfigUpdateFail
 *
 *    Functionality: deallocate the memory allocated in E2Node Config UpdateFailure
 *
...
void FreeE2ConfigUpdateFail(E2AP_PDU_t *e2apMsg)
{
   ...
   if(e2apMsg)
   {
      ...
      {
         e2NodeCfgUpdFail = &e2apMsg->choice.unsuccessfulOutcome->value.choice.E2setupFailure;
         if(e2NodeCfgUpdFail->protocolIEs.list.array)
         {
            for(arrIdx=0; arrIdx<e2NodeCfgUpdFail->protocolIEs.list.count; arrIdx++)
            {
               RIC_FREE(e2NodeCfgUpdFail->protocolIEs.list.array[arrIdx], sizeof(E2setupFailureIEs_t));
            }
...
uint8_t BuildAndSendE2NodeConfigUpdateFailure(uint32_t duId, uint8_t transId)
{
   ...
   while(true)
   {
      ...
      elementCnt = 3;
      e2NodeCfgUpdFail->protocolIEs.list.count = elementCnt;
      e2NodeCfgUpdFail->protocolIEs.list.size  = elementCnt * sizeof(struct E2nodeConfigurationUpdateFailure_IEs *);

      RIC_ALLOC(e2NodeCfgUpdFail->protocolIEs.list.array, e2NodeCfgUpdFail->protocolIEs.list.size);
      if(e2NodeCfgUpdFail->protocolIEs.list.array == NULLP)
      {
      ...
      {
         RIC_ALLOC(e2NodeCfgUpdFail->protocolIEs.list.array[arrIdx], sizeof(struct E2nodeConfigurationUpdateFailure_IEs));
         if(e2NodeCfgUpdFail->protocolIEs.list.array[arrIdx] == NULLP)
         {
            DU_LOG("\nERROR  -->  E2AP : Memory allocation for E2FailureIEs failed");
            memAllocFailed = true;
            break;
      ...
      /* Trans Id */
      arrIdx = 0;
      e2NodeCfgUpdFail->protocolIEs.list.array[arrIdx]->id = ProtocolIE_IDE2_id_TransactionID;
      e2NodeCfgUpdFail->protocolIEs.list.array[arrIdx]->criticality = CriticalityE2_reject;
      e2NodeCfgUpdFail->protocolIEs.list.array[arrIdx]->value.present = E2setupFailureIEs__value_PR_TransactionID;
      e2NodeCfgUpdFail->protocolIEs.list.array[arrIdx]->value.choice.TransactionID  = transId;

      arrIdx++;
      e2NodeCfgUpdFail->protocolIEs.list.array[arrIdx]->id = ProtocolIE_IDE2_id_CauseE2;
      e2NodeCfgUpdFail->protocolIEs.list.array[arrIdx]->criticality = CriticalityE2_reject;
      e2NodeCfgUpdFail->protocolIEs.list.array[arrIdx]->value.present = E2setupFailureIEs__value_PR_CauseE2;
      e2NodeCfgUpdFail->protocolIEs.list.array[arrIdx]->value.choice.CauseE2.present = CauseE2_PR_protocol;
      e2NodeCfgUpdFail->protocolIEs.list.array[arrIdx]->value.choice.CauseE2.choice.protocol = CauseE2Protocol_unspecified;

      arrIdx++;
      e2NodeCfgUpdFail->protocolIEs.list.array[arrIdx]->id = ProtocolIE_IDE2_id_TimeToWaitE2;
      e2NodeCfgUpdFail->protocolIEs.list.array[arrIdx]->criticality = CriticalityE2_ignore;
      e2NodeCfgUpdFail->protocolIEs.list.array[arrIdx]->value.present = E2setupFailureIEs__value_PR_TimeToWaitE2;
      e2NodeCfgUpdFail->protocolIEs.list.array[arrIdx]->value.choice.TimeToWaitE2 = TimeToWaitE2_v5s;
...
void ProcE2NodeConfigUpdate(uint32_t duId, E2nodeConfigurationUpdate_t *e2NodeConfigUpdate)
{
   ...
   uint8_t transId = 0, cause = 0, e2NodeUpdateListIdx;
   DuDb    *duDb = NULLP;
                        ...
                        if(e2NodeUpdateList->list.array)
                        {
                           for(e2NodeUpdateListIdx = 0; e2NodeUpdateListIdx< e2NodeUpdateList->list.count; e2NodeUpdateListIdx++)
                           {
                              if(e2NodeUpdateList->list.array[e2NodeUpdateListIdx])
                              {
                                 ...
                                 if(e2NodeUpdateItem->value.choice.E2nodeComponentConfigUpdate_Item.e2nodeComponentID.choice.\
                                       e2nodeComponentInterfaceTypeF1)
                                 {
                                    ...
                                    if(duDb != NULLP)
                                    {
                                       ...
                                       duDb->e2NodeComponent.componentId = e2NodeUpdateItem->value.choice.E2nodeComponentConfigUpdate_Item.e2nodeComponentID.choice.e2nodeComponentInterfaceTypeF1->gNB_DU_ID.buf[0]; 
                                    }
                                 }
                                 else
                                 {
                                    invalidE2NodeComponentFound = true;
                                 }
                              }
                           }
                        }
