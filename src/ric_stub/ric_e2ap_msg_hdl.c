...
/*******************************************************************
 *
 * @brief deallocate the memory allocated in RicServiceQuery(
 *
 * @details
 *
 *    Function : FreeE2SetupRsp 
 *
 *    Functionality: deallocate the memory allocated in RicServiceQuery
 *
 * @params[in] E2AP_PDU_t *e2apMsg
 *
 * @return void
...
uint8_t BuildAndSendRicServiceQuery(DuDb *duDb, uint8_t transId)
{
   ...
   while(true)
   {
      ...
      ricServiceQuery->protocolIEs.list.array[arrIdx]->value.choice.TransactionID  = transId;
#if 0
      ...
      if(BuildRanFunctionAddedList(duDb, &ricServiceQuery->protocolIEs.list.array[arrIdx]->value.choice.RANfunctionsID_List)!=ROK)
      {
         DU_LOG("\nERROR  -->  E2AP : Failed to build Ran function added list");
         break;         
      }
