...
void FreeE2NodeConfigUpdateAck(E2AP_PDU_t *e2apMsg)
{
   ...
   E2nodeComponentConfigAdditionAck_ItemIEs_t *e2NodeConfigAdditionAckItemIte=NULL;
         ...
         if(e2NodeConfigUpdateAck->protocolIEs.list.array != NULLP)
         {
            for(arrIdx = 0; arrIdx < e2NodeConfigUpdateAck->protocolIEs.list.count; arrIdx++)
            {
               if(e2NodeConfigUpdateAck->protocolIEs.list.array[arrIdx])
               {
                     ...
                     case ProtocolIE_IDE2_id_E2nodeComponentConfigAdditionAck:
                        {
                           ...
                           if(e2NodeConfigAdditionAckList->list.array)
                           {
                              ...
                              {
                                 e2NodeConfigAdditionAckItemIte = (E2nodeComponentConfigAdditionAck_ItemIEs_t*) e2NodeConfigAdditionAckList->list.array[e2NodeConfigIdx];
                                 if(e2NodeConfigAdditionAckItemIte)
                                 {
                                    freeE2NodeConfigItem(ProtocolIE_IDE2_id_E2nodeComponentConfigAdditionAck, (PTR)&e2NodeConfigAdditionAckItemIte->value.choice.E2nodeComponentConfigAdditionAck_Item);
                                    RIC_FREE(e2NodeConfigAdditionAckItemIte, sizeof(E2nodeComponentConfigAdditionAck_ItemIEs_t));
                                 }
                              }
                  ...
                  RIC_FREE(e2NodeConfigUpdateAck->protocolIEs.list.array[arrIdx], sizeof(E2nodeConfigurationUpdateAcknowledge_IEs_t));
               }
            }
      ...
      case ProtocolIE_IDE2_id_E2nodeComponentConfigAdditionAck:
         {
            ...
            e2nodeComponentInterfaceType = &additionAckItem->e2nodeComponentInterfaceType;
...
uint8_t BuildE2nodeComponentConfigRemovalAck(E2nodeComponentConfigRemovalAck_List_t *e2NodeConfigRemovalAckList, uint16_t removalE2NodeCount, E2NodeConfigItem *removaldE2Node)
{
   uint8_t arrIdx = 0;
...
void handleE2NodeComponentAction(DuDb *duDb, PTR e2NodeCfg, uint8_t protocolId, E2NodeConfigItem *storeCfg)
{
   uint8_t idx=0;
   bool configFound;
   ...
   /* Only if protocolId is  ProtocolIE_IDE2_id_E2nodeComponentConfigAddition,
    * adding the new element in the e2NodeComponent list */
   if(protocolId == ProtocolIE_IDE2_id_E2nodeComponentConfigAddition)
   {
      /* Adding the new ran function in DB*/
      duDb->e2NodeComponent[duDb->numOfE2NodeComponent].interfaceType = storeCfg->componentInfo.interfaceType;
      duDb->e2NodeComponent[duDb->numOfE2NodeComponent].componentId   = storeCfg->componentInfo.componentId;
      duDb->numOfE2NodeComponent++;
   }

   for(idx =0;idx< duDb->numOfE2NodeComponent; idx++)
   {
      if(duDb->e2NodeComponent[idx].interfaceType == storeCfg->componentInfo.interfaceType)
      {
         switch(protocolId)
         {
            case ProtocolIE_IDE2_id_E2nodeComponentConfigAddition:
         ...
         case ProtocolIE_IDE2_id_E2nodeComponentConfigAddition:
            {
               ...
               if(e2NodeAddList->list.array)
               {
                  for(arrIdx = 0; arrIdx< e2NodeAddList->list.count; arrIdx++)
                  {
                     ...
                     handleE2NodeComponentAction(duDb, (PTR)e2NodeAddItem, ProtocolIE_IDE2_id_E2nodeComponentConfigAddition,\
                     &tmpE2NodeList.addedE2Node[tmpE2NodeList.addedE2NodeCount++]);
                  }
               }
               break;
            }
         ...
         case ProtocolIE_IDE2_id_E2nodeComponentConfigUpdate:
            {
               ...
               if(e2NodeUpdateList->list.array)
               {
                  for(arrIdx = 0; arrIdx< e2NodeUpdateList->list.count; arrIdx++)
                  {
                     ...
                     handleE2NodeComponentAction(duDb, (PTR)e2NodeUpdateItem, ProtocolIE_IDE2_id_E2nodeComponentConfigUpdate,\
                     &tmpE2NodeList.updatedE2Node[tmpE2NodeList.updatedE2NodeCount++]);
                  }
               }
               break;
            }
         ...
         case ProtocolIE_IDE2_id_E2nodeComponentConfigRemoval:
            {
               ...
               if(e2NodeRemoveList->list.array)
               {
                  for(arrIdx = 0; arrIdx< e2NodeRemoveList->list.count; arrIdx++)
                  {
                     ...
                     &tmpE2NodeList.removedE2Node[tmpE2NodeList.removedE2NodeCount++]);
                  }
               }
               break;
            }
