...
{
   uint8_t arrIdx = 0, e2NodeConfigListIdx=0, ranFuncAddedListIdx=0;
            ...
            for(arrIdx=0; arrIdx<e2SetupRsp->protocolIEs.list.count; arrIdx++)
            {
               switch(e2SetupRsp->protocolIEs.list.array[arrIdx]->id)
               {
                  case ProtocolIE_IDE2_id_RANfunctionsAccepted:
                  {
                     ...
                     {
                        for(ranFuncAddedListIdx=0;ranFuncAddedListIdx<ranFuncAddedList->list.count; ranFuncAddedListIdx++)
                        {
                  ...
                  case ProtocolIE_IDE2_id_E2nodeComponentConfigAdditionAck:
                  {
                     ...
                     {
                        for(e2NodeConfigListIdx=0; e2NodeConfigListIdx<e2NodeConfigAdditionAckList->list.count; e2NodeConfigListIdx++)
                        {
                           if(e2NodeConfigAdditionAckList->list.array[arrIdx])
                           {
                              e2NodeAddAckItem = (E2nodeComponentConfigAdditionAck_ItemIEs_t*) e2NodeConfigAdditionAckList->list.array[e2NodeConfigListIdx];
                              RIC_FREE(e2NodeAddAckItem->value.choice.E2nodeComponentConfigAdditionAck_Item.e2nodeComponentID.choice.e2nodeComponentInterfaceTypeF1,\
                              sizeof(E2nodeComponentInterfaceF1_t));
                              RIC_FREE(e2NodeAddAckItem->value.choice.E2nodeComponentConfigAdditionAck_Item.e2nodeComponentID.choice.e2nodeComponentInterfaceTypeF1->gNB_DU_ID.buf,\
                              ...
                              RIC_FREE(e2NodeConfigAdditionAckList->list.array[arrIdx], sizeof(E2nodeComponentConfigAdditionAck_ItemIEs_t));
                           }
                        }
...
uint8_t BuildRanFunctionAddedList(DuDb *duDb, RANfunctionsID_List_t *ranFuncAddedList)
{
   ...
   RANfunctionID_Item_t  *ranFuncAddedItem;
   ...
   if(ranFuncAddedList->list.array)
   {
      for(arrIdx = 0; arrIdx< ranFuncAddedList->list.count; arrIdx++)
      {
         RIC_ALLOC(ranFuncAddedList->list.array[arrIdx], sizeof(RANfunction_ItemIEs_t));
         if(ranFuncAddedList->list.array[arrIdx] == NULLP)
         {
            DU_LOG("\nERROR  -->  E2AP : Memory allocation for RAN function added list array item");
            return RFAILED;
         }
         ranFuncAddedItemIe = (RANfunctionID_Item_t*)ranFuncAddedList->list.array[arrIdx];
         ...
         ranFuncAddedItemIe->value.choice.RANfunctionID_Item.ranFunctionID =duDb->ranFunction[arrIdx].id;
         ranFuncAddedItemIe->value.choice.RANfunctionID_Item.ranFunctionRevision=duDb->ranFunction[arrIdx].revisionCounter;
      }
   }
   else
   {
                  ...
                  case ProtocolIE_IDE2_id_GlobalE2node_ID:
                     ...
                     case ProtocolIE_IDE2_id_RANfunctionsAdded:
                     {
                        ...
                        if(ranFunctionsList->list.array)
                        {
                           ...
                           {
                              ranFuncAddItemIe = (RANfunction_ItemIEs_t *) ranFunctionsList->list.array[ranFuncIdx]; 
                              ranFunAddItem = &ranFuncAddItemIe->value.choice.RANfunction_Item;
                              duDb->ranFunction[duDb->numOfRanFunction].id =  ranFunAddItem->ranFunctionID; 
                              duDb->ranFunction[ duDb->numOfRanFunction].revisionCounter =  ranFunAddItem->ranFunctionRevision; 
