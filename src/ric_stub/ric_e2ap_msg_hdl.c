...
uint8_t BuildRicRequestId(RICrequestID_t *ricReqId, RicRequestId *reqIdDb)
{
   if(ricReqId != NULLP)
   {
...
uint8_t BuildAndSendRicSubscriptionReq(DuDb *duDb)
{
   ...
   while(true)
   {
      ...
      if(BuildRicRequestId(&ricSubscriptionReq->protocolIEs.list.array[idx]->value.choice.RICrequestID, \
