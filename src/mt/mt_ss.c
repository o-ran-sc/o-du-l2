...
#ifdef SS_USE_WLS_MEM
PRIVATE MtRegMemSz mtDynMemSz[MT_MAX_BKTS];
PRIVATE S16 SPartitionWlsDynMem();
PRIVATE S16 SAllocateWlsDynMem();
#endif
#ifdef INTEL_WLS
...
#endif
#else
   hdl = WLS_Open(WLS_DEVICE_NAME, WLS_MASTER_CLIENT, 0x7ec00000);
#endif
...
{
   U32 i;
   U8 *bktMemStrtAddr = (U8 *)(((U8*)osCp.wls.allocAddr) + (4 * 1024 * 1024));
