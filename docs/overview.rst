...
.. figure:: ODU-O1-Arch.jpg
  :width: 600
  :alt: Figure 2 O1 Module Architecture

  Figure 2 - O-DU O1 Architecture 
...
.. figure:: ODU-O1-GetAlarmListFlow.jpg
  :width: 720
  :alt: Figure 6 get Alarm List code flow  

  Figure 6 - O-DU O1 get Alarm List code flow

 
As seen in the Figure 6,

- On the cell state change from de-active to activate, ODU High raises cell up alarm message and sends it over the TCP socket using the TCP client Alarm Interface API.

- On other side a TCP server, running as a thread, in O1 module recieves the cell up alarm message and it passes the alarm information to the Alarm Manager.
...
- The callback function fetches the alarm list from Alarm Manager and sends it back to the Netconf client (SMO/OAM). 
