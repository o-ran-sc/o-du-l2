...
O-DU High send alarm messages through Alarm Interface APIs
...
Cell specific alarm APIs 

1. **Raise Cell up/down Alarm**
   
   *uint8_t raiseCellAlrm(uint16_t alrmId, uint16_t cellId)*
   
      a. Description
         
         - Fill the Alarm structure with cell information setting the action as RAISE alarm and send it to O1 module over TCP interface.

      b. Input

         - Alarm Id and cell Id

      c. Returns 0 on success

2. **Clear Cell up/down Alarm**
   
   *uint8_t clearCellAlrm(uint16_t alrmId)*

      a. Description
         
         - Fill the Alarm structure with cell information setting the action as CLEAR alarm and send it to O1 module over TCP interface.
