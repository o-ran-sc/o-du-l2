...
void decode_fapi_msg(fapi_msg_t* msg) 
{
   printf("\n");
#ifdef OAI_TESTING
   printf("numMsg = %u\t", msg->numMsg);
   printf("opaque = %u\t", msg->opaque);
#endif
...
void decode_fapi_uint8_tlv(fapi_uint8_tlv_t* tlv) 
{
   ...
   printf("tl.length = %u\t", tlv->tl.length);
...
{
   printf("\n");
   printf("tl.tag = %u\t", config_num->tl.tag);
   printf("tl.length = %u\t", config_num->tl.length);
...
void decode_fapi_cell_parms(fapi_cell_parms_t* cell_parms) 
{
   ...
   for (size_t i = 0; i < FAPI_MAX_NUM_TLVS_CONFIG; i++) {
      printf("tlvStatus[%zu].tag = %u\t", i, cell_parms->tlvStatus[i].tag);
      printf("tlvStatus[%zu].length = %u\t", i, cell_parms->tlvStatus[i].length);
   }
}
...
void decode_fapi_param_resp(fapi_param_resp_t* param_resp) 
{
   ...
   for (size_t i = 0; i < FAPI_MAX_NUM_TLVS_PARAMS; i++) {
      printf("tlvs[%zu].tl.tag = %u\t", i, param_resp->tlvs[i].tl.tag);
      printf("tlvs[%zu].tl.length = %u\t", i, param_resp->tlvs[i].tl.length);
   }
}
...
void decode_fapi_precoderWeight_t(fapi_precoderWeight_t* precoder)
{
   ...
   printf("\n");
   printf("PrecoderRe = %u\t", precoder->preCoderWeightRe);
   printf("PrecoderIm = %u\t", precoder->preCoderWeightIm);
...
void decode_fapi_precoding_bmform(const fapi_precoding_bmform_t* precodingBfForm) 
{
   ...
   for (size_t i = 0; i < FAPI_MAX_NUM_PRGS_PER_TTI; i++) {
      printf("pmi_bfi[%zu] pmIdx = %u\t", i, precodingBfForm->pmi_bfi[i].pmIdx);
      for (size_t j = 0; j < FAPI_MAX_NUM_DIGBFINTERFACES; j++) {
         printf("pmi_bfi[%zu] beamIdx[%zu] beamidx = %u\t", i, j, precodingBfForm->pmi_bfi[i].beamIdx[j].beamidx);
      }
   }
}
